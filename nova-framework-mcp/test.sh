#!/bin/bash
# Test script for Nova Framework MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/nova-framework-mcp:latest"
PORT=8299
BINARY="nova-framework"
CONTAINER_NAME="nova-framework-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Nova Framework MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_nova_framework)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_nova_framework","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_nova_framework returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_nova_framework returned content"
else
    fail "tools/call run_nova_framework did not return expected response"
