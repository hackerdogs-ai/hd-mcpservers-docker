#!/bin/bash
# Test script for Social-Analyzer MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/social-analyzer-mcp:latest"
PORT=8255
BINARY="social-analyzer"
CONTAINER_NAME="social-analyzer-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Social-Analyzer MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_social_analyzer)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_social_analyzer","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_social_analyzer returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_social_analyzer returned content"
else
    fail "tools/call run_social_analyzer did not return expected response"
