#!/bin/bash
# Test script for Pwntools MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/pwntools-mcp:latest"
PORT=8245
BINARY="python3"
CONTAINER_NAME="pwntools-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Pwntools MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_pwntools)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_pwntools","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_pwntools returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_pwntools returned content"
else
    fail "tools/call run_pwntools did not return expected response"
