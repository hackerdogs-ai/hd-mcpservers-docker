#!/usr/bin/env python3
"""Pwntools MCP Server — CTF framework and exploit development (use -c with pwntools).

Wraps the python3 CLI (Gallopsled/pwntools) to expose
...
logger = logging.getLogger("pwntools-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8245"))

mcp = FastMCP(
    "Pwntools MCP Server",
    instructions=(
        "CTF framework and exploit development (use -c with pwntools)."
    ),
)

PWNTOOLS_BIN = os.environ.get("PWNTOOLS_BIN", "python3")


def _find_binary() -> str:
    """Locate the python3 binary, raising a clear error if missing."""
    path = shutil.which(PWNTOOLS_BIN)
    if path is None:
        logger.error("python3 binary not found on PATH")
        raise FileNotFoundError(
            f"python3 binary not found. Ensure it is installed and available "
            f"on PATH, or set PWNTOOLS_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a python3 command and return structured output.
...
@mcp.tool()
async def run_pwntools(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run python3 with the given arguments.
    ...
    logger.info("run_pwntools called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("python3 command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"python3 failed (exit code {result['return_code']})",
...
def main():
    logger.info("Starting pwntools-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
