#!/bin/bash
# Test script for Augustus MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/augustus-mcp:latest"
PORT=8101
BINARY="augustus"
CONTAINER_NAME="augustus-mcp-test"
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${YELLOW}NOTE: Augustus requires LLM provider API keys to run scans.${NC}"
echo -e "${YELLOW}   Tests 1-6 only check MCP protocol compliance (no API key needed).${NC}"
echo -e "${YELLOW}   To test actual scans: export OPENAI_API_KEY=... then use scan_llm tool.${NC}"
echo ""
...
echo "================================================================================="
echo -e "${BLUE}Augustus MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (get_version)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"get_version","arguments":{}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call get_version returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call get_version returned content"
else
    fail "tools/call get_version did not return expected response"
