#!/usr/bin/env python3
"""
Test suite for Blackbird MCP Server.
Tests the Blackbird username search tool.
"""
...
def main():
    """Run tests for Blackbird."""
    print("="*60)
    print("Blackbird MCP Server Test Suite")
    print("="*60)
    
    docker_cmd = ["docker", "run", "--rm", "-i", "hackerdogs/blackbird-mcp-server:latest"]
    ...
    if "result" in response and response["result"].get("serverInfo", {}).get("name") == "blackbird-mcp-server":
    ...
    if "result" in response and "tools" in response["result"]:
        ...
        if len(tools) > 0 and tools[0].get("name") == "blackbird_username_search":
    ...
    # Test 3: Blackbird username search
    tests_total += 1
    if test_tool(
        docker_cmd,
