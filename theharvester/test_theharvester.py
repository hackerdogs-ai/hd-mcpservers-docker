#!/usr/bin/env python3
"""
Test suite for theHarvester MCP Server.
Tests the theHarvester domain/email enumeration tool.
"""
...
def main():
    """Run tests for theHarvester."""
    print("="*60)
    print("theHarvester MCP Server Test Suite")
    print("="*60)
    
    docker_cmd = ["docker", "run", "--rm", "-i", "hackerdogs/theharvester-mcp-server:latest"]
    ...
    if "result" in response and response["result"].get("serverInfo", {}).get("name") == "theharvester-mcp-server":
    ...
    if "result" in response and "tools" in response["result"]:
        ...
        if len(tools) > 0 and tools[0].get("name") == "theharvester_domain_search":
    ...
    # Test 3: theHarvester domain search
    tests_total += 1
    if test_tool(
        docker_cmd,
