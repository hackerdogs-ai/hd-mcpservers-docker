#!/bin/bash
# Test script for Boofuzz MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/boofuzz-mcp:latest"
PORT=8333
BINARY="boofuzz"
...
echo "================================================================================="
echo -e "${BLUE}Boofuzz MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_boofuzz)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_boofuzz","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_boofuzz returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_boofuzz returned content"
else
    fail "tools/call run_boofuzz did not return expected response"
