#!/bin/bash
# Test script for Smuggler MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/smuggler-mcp:latest"
PORT=8291
BINARY="smuggler"
CONTAINER_NAME="smuggler-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Smuggler MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_smuggler)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_smuggler","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_smuggler returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_smuggler returned content"
else
    fail "tools/call run_smuggler did not return expected response"
