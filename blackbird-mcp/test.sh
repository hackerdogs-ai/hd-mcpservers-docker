#!/bin/bash
# Test script for Blackbird MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/blackbird-mcp:latest"
PORT=8220
BINARY="blackbird"
CONTAINER_NAME="blackbird-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Blackbird MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_blackbird --help)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_blackbird","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_blackbird returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_blackbird returned content"
else
    fail "tools/call run_blackbird did not return expected response"
