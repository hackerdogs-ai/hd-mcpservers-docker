#!/usr/bin/env python3
"""Horusec MCP Server — Static application security testing (SAST) tool.

Wraps the horusec CLI (Checkmarx/Horusec) to expose
...
logger = logging.getLogger("horusec-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8359"))
...
BIN_NAME = os.environ.get("HORUSEC_BIN", "horusec")


def _find_binary() -> str:
    """Locate the horusec binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("horusec binary not found on PATH")
        raise FileNotFoundError(
            f"horusec binary not found. Ensure it is installed and available "
            f"on PATH, or set HORUSEC_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a horusec command and return structured output.
...
@mcp.tool()
async def run_horusec(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run horusec with the given arguments.
    ...
    logger.info("run_horusec called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("horusec command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"horusec failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"horusec {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting horusec-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
