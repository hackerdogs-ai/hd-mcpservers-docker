#!/bin/bash
# Test script for Horusec MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/horusec-mcp:latest"
PORT=8359
BINARY="horusec"
CONTAINER_NAME="horusec-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Horusec MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_horusec)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_horusec","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_horusec returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_horusec returned content"
else
    fail "tools/call run_horusec did not return expected response"
