#!/bin/bash
# Test script for SSLyze MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/sslyze-mcp:latest"
PORT=8280
BINARY="sslyze"
CONTAINER_NAME="sslyze-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}SSLyze MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_sslyze)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_sslyze","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_sslyze returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_sslyze returned content"
else
    fail "tools/call run_sslyze did not return expected response"
