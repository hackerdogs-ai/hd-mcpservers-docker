#!/bin/bash
# Test script for GDB-GEF MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/gef-mcp:latest"
PORT=8238
BINARY="gdb"
CONTAINER_NAME="gef-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}GDB-GEF MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_gef)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_gef","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_gef returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_gef returned content"
else
    fail "tools/call run_gef did not return expected response"
