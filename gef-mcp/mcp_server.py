...
Wraps the gdb CLI (hugsy/gef) to expose capabilities through the Model Context Protocol (MCP).
"""
...
mcp = FastMCP(
    "GDB-GEF MCP Server",
    instructions=(
        "GDB Enhanced Features (GEF) for exploit development and dynamic analysis. "
        "IMPORTANT: GDB is highly interactive! You MUST use the '--batch' flag to run it headlessly, "
        "otherwise it will hang. Execute commands using '-ex'. "
        "Example: gdb -q --batch -ex 'checksec' -ex 'info functions' ./target_binary "
        "Place all target binaries in the mounted /app/output directory."
    ),
)

BIN_NAME = os.environ.get("GEF_BIN", "/usr/bin/gdb")
...
async def run_gef(
    ...
    try:
        ...
        logger.info("run_gef called with arguments=%s", arguments)
        ...
        return json.dumps({
            "success": True,
            "message": "GDB-GEF executed successfully.",
            "stdout": stdout
        }, indent=2)

    except Exception as e:
        logger.error("Unhandled exception in run_gef: %s", e)
        return json.dumps({
            "error": True,
