...
PASS=0
FAIL=0
IMAGE="hackerdogs/opencti-mcp:latest"
PORT=8370
CONTAINER_NAME="opencti-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}OpenCTI MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 2] pycti library inside container"
PYCTI_OUTPUT=$(docker run --rm "$IMAGE" python -c "from pycti import OpenCTIApiClient; print('pycti OK')" 2>&1 || true)
if echo "$PYCTI_OUTPUT" | grep -q "pycti OK"; then
    pass "pycti library imports successfully"
else
    fail "pycti library not available"
    echo "       ${PYCTI_OUTPUT:0:200}"
fi
echo ""
...
# Test 6: MCP HTTP — tools/call (opencti_search_indicators — expects config error without valid creds)
info "[Test 6] MCP HTTP — tools/call (opencti_search_indicators)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"opencti_search_indicators","arguments":{"query":"test"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call opencti_search_indicators returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call opencti_search_indicators returned content"
else
    fail "tools/call opencti_search_indicators did not return expected response"
