#!/bin/bash
# Test script for Hakrawler MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/hakrawler-mcp:latest"
PORT=8217
BINARY="hakrawler"
CONTAINER_NAME="hakrawler-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Hakrawler MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_hakrawler)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_hakrawler","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_hakrawler returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_hakrawler returned content"
else
    fail "tools/call run_hakrawler did not return expected response"
