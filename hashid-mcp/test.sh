#!/bin/bash
# Test script for HashID MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/hashid-mcp:latest"
PORT=8264
BINARY="hashid"
CONTAINER_NAME="hashid-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}HashID MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_hashid)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_hashid","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_hashid returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_hashid returned content"
else
    fail "tools/call run_hashid did not return expected response"
