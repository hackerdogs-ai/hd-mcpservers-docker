#!/bin/bash
# Test script for AWS S3 MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/aws-s3-mcp:latest"
PORT=8346
BINARY="aws-s3-mcp"
...
echo "================================================================================="
echo -e "${BLUE}AWS S3 MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_aws_s3_mcp)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_aws_s3","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_aws_s3_mcp returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_aws_s3_mcp returned content"
else
    fail "tools/call run_aws_s3_mcp did not return expected response"
