#!/bin/bash
# Test script for AlienVault OTX MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/otx-mcp:latest"
PORT=8368
CONTAINER_NAME="otx-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}AlienVault OTX MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
# Test 2: OTXv2 Python SDK available inside container
info "[Test 2] OTXv2 Python SDK inside container"
SDK_OUTPUT=$(docker run --rm "$IMAGE" python -c "import OTXv2; print('OTXv2 version:', OTXv2.__version__ if hasattr(OTXv2, '__version__') else 'installed')" 2>&1 || true)
if echo "$SDK_OUTPUT" | grep -qi "OTXv2"; then
    pass "OTXv2 Python SDK is available"
    echo "       ${SDK_OUTPUT%%$'\n'*}"
else
    fail "OTXv2 Python SDK not found or not responding"
    echo "       Output: $SDK_OUTPUT"
fi
echo ""
...
# Test 6: MCP HTTP — tools/call (otx_domain_report — expects API key error)
info "[Test 6] MCP HTTP — tools/call (otx_domain_report)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"otx_domain_report","arguments":{"domain":"example.com"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call otx_domain_report returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call otx_domain_report returned content"
else
    fail "tools/call otx_domain_report did not return expected response"
