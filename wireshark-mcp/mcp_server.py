#!/usr/bin/env python3
"""Wireshark (tshark) MCP Server — Network protocol analyzer for deep packet inspection.

Wraps the tshark CLI (wireshark/wireshark) to expose
...
MCP_PORT = int(os.environ.get("MCP_PORT", "8315"))
...
BIN_NAME = os.environ.get("TSHARK_BIN", "tshark")


def _find_binary() -> str:
    """Locate the tshark binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("tshark binary not found on PATH")
        raise FileNotFoundError(
            f"tshark binary not found. Ensure it is installed and available "
            f"on PATH, or set TSHARK_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a tshark command and return structured output.
...
async def run_tshark(
    ...
    logger.info("run_tshark called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("tshark command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"tshark failed (exit code {result['return_code']})",
...
def main():
    logger.info("Starting wireshark-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
