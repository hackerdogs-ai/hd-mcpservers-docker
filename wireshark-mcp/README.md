...
MCP server wrapper for [Wireshark (tshark)](https://github.com/wireshark/wireshark) — Network protocol analyzer for deep packet inspection.

## What is Wireshark (tshark)?

Wireshark (tshark) (tshark) is a security tool that provides: **Network protocol analyzer for deep packet inspection.**

See [wireshark/wireshark](https://github.com/wireshark/wireshark) for full documentation.

**No API keys required** — Wireshark (tshark) runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Wireshark (tshark)](https://github.com/wireshark/wireshark) — Network protocol analyzer for deep packet inspection.

**Tools:**
- `run_tshark` — Run tshark with the given arguments. Returns structured JSON output.
...
Run tshark with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "tshark output will appear here"
}
```
...
- "Run tshark with --help to see all available options."
- "Use tshark to scan the target 192.168.1.1."
- "What options does tshark support? Show me its help output."
- "Run tshark against example.com with default settings."
- "Execute tshark with verbose output enabled."
- "Use the tshark tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/wireshark-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8315:8315 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8315 \
  hackerdogs/wireshark-mcp:latest
```
...
```json
{
  "mcpServers": {
    "wireshark-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/wireshark-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "wireshark-mcp": {
      "url": "http://localhost:8485/wireshark-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8315` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/wireshark-mcp:latest .
```
...
```bash
docker run -d --rm --name wireshark-mcp-test -p 8315:8315 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/wireshark-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8315/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8315/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8315/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_tshark","arguments":{"arguments":"--help"}}}'
```
