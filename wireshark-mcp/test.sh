#!/bin/bash
# Test script for Wireshark (tshark) MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/wireshark-mcp:latest"
PORT=8315
BINARY="tshark"
CONTAINER_NAME="wireshark-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Wireshark (tshark) MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_tshark)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_tshark","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_tshark returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_tshark returned content"
else
    fail "tools/call run_tshark did not return expected response"
