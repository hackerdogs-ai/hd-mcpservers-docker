...
MCP server wrapper for [Ropgadget](https://github.com/JonathanSalwan/ROPgadget) — ROP/JOP gadget finder.

## What is Ropgadget?

Ropgadget (ROPgadget) is a security tool that provides: **ROP/JOP gadget finder.**

See [JonathanSalwan/ROPgadget](https://github.com/JonathanSalwan/ROPgadget) for full documentation.

**No API keys required** — Ropgadget runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Ropgadget](https://github.com/JonathanSalwan/ROPgadget) — ROP/JOP gadget finder.

**Tools:**
- `run_ropgadget` — Run ROPgadget with the given arguments. Returns structured JSON output.
...
Run ROPgadget with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "ROPgadget output will appear here"
}
```
...
- "Run ROPgadget with --help to see all available options."
- "Use ropgadget to scan the target 192.168.1.1."
- "What options does ROPgadget support? Show me its help output."
- "Run ropgadget against example.com with default settings."
- "Execute ROPgadget with verbose output enabled."
- "Use the ropgadget tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/ropgadget-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8242:8242 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8242 \
  hackerdogs/ropgadget-mcp:latest
```
...
```json
{
  "mcpServers": {
    "ropgadget-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/ropgadget-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
| `MCP_PORT` | `8242` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/ropgadget-mcp:latest .
```
...
```bash
docker run -d --rm --name ropgadget-mcp-test -p 8242:8242 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/ropgadget-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8242/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8242/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8242/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_ropgadget","arguments":{"arguments":"--help"}}}'
```
...
```json
{
  "mcpServers": {
    "ropgadget-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "hackerdogs/ropgadget-mcp:latest"],
      "env": {}
    }
  }
}
...
```bash
docker run -d -p 8242:8242 -e MCP_TRANSPORT=streamable-http hackerdogs/ropgadget-mcp:latest
```
