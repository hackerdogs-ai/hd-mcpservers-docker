#!/bin/bash
# Test script for SlowHTTPTest MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/slowhttptest-mcp:latest"
PORT=8316
BINARY="slowhttptest"
CONTAINER_NAME="slowhttptest-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}SlowHTTPTest MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_slowhttptest)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_slowhttptest","arguments":{"arguments":"-h"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_slowhttptest returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_slowhttptest returned content"
else
    fail "tools/call run_slowhttptest did not return expected response"
