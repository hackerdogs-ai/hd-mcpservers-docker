...
docker run -i --rm \
  -e STRIPE_SECRET_KEY \
  hackerdogs/stripe-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8669:8669 \
...
```bash
docker build -t hackerdogs/stripe-mcp:latest .
```
...
```bash
docker run -d --rm --name stripe-mcp-test -p 8669:8669 \
  -e MCP_TRANSPORT=streamable-http \
  -e STRIPE_SECRET_KEY \
  hackerdogs/stripe-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8669/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8669/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
