#!/bin/bash
# Test script for RASN MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/rasn-mcp:latest"
PORT=8353
BINARY="rasn"
CONTAINER_NAME="rasn-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}RASN MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_rasn)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_rasn","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_rasn returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_rasn returned content"
else
    fail "tools/call run_rasn did not return expected response"
