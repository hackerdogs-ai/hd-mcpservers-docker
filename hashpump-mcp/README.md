...
Run HashPump with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "HashPump output will appear here"
}
```
...
- "Run HashPump with --help to see all available options."
- "Use hashpump to scan the target 192.168.1.1."
- "What options does HashPump support? Show me its help output."
- "Run hashpump against example.com with default settings."
- "Execute HashPump with verbose output enabled."
- "Use the hashpump tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/hashpump-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8274:8274 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8274 \
  hackerdogs/hashpump-mcp:latest
```
...
```json
{
  "mcpServers": {
    "hashpump-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/hashpump-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "hashpump-mcp": {
      "url": "http://localhost:8485/hashpump-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8274` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/hashpump-mcp:latest .
```
...
```bash
docker run -d --rm --name hashpump-mcp-test -p 8274:8274 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/hashpump-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8274/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8274/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8274/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_hashpump","arguments":{"arguments":"--help"}}}'
```

**4. Clean up:**

```bash
docker stop hashpump-mcp-test
```
