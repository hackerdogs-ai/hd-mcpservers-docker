#!/bin/bash
# Test script for HashPump MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/hashpump-mcp:latest"
PORT=8274
BINARY="HashPump"
CONTAINER_NAME="hashpump-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}HashPump MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_hashpump)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_hashpump","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_hashpump returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_hashpump returned content"
else
    fail "tools/call run_hashpump did not return expected response"
