#!/usr/bin/env python3
"""HashPump MCP Server — Hash length extension attacks.

Wraps the HashPump CLI (bwall/HashPump) to expose
...
logger = logging.getLogger("hashpump-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8274"))
...
HASHPUMP_BIN = os.environ.get("HASHPUMP_BIN", "HashPump")


def _find_binary() -> str:
    """Locate the HashPump binary, raising a clear error if missing."""
    path = shutil.which(HASHPUMP_BIN)
    if path is None:
        logger.error("HashPump binary not found on PATH")
        raise FileNotFoundError(
            f"HashPump binary not found. Ensure it is installed and available "
            f"on PATH, or set HASHPUMP_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a HashPump command and return structured output.
...
@mcp.tool()
async def run_hashpump(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run HashPump with the given arguments.
    ...
    logger.info("run_hashpump called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("HashPump command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"HashPump failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"HashPump {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting hashpump-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
