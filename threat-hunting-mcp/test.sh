#!/bin/bash
# Test script for Threat Hunting MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/threat-hunting-mcp:latest"
PORT=8345
BINARY="threat-hunting"
CONTAINER_NAME="threat-hunting-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Threat Hunting MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_threat_hunting)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_threat_hunting","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_threat_hunting returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_threat_hunting returned content"
else
    fail "tools/call run_threat_hunting did not return expected response"
