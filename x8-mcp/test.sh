...
PASS=0
FAIL=0
IMAGE="hackerdogs/x8-mcp:latest"
PORT=8275
BINARY="x8"
CONTAINER_NAME="x8-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}X8 MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_x8)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_x8","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_x8 returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_x8 returned content"
else
    fail "tools/call run_x8 did not return expected response"
