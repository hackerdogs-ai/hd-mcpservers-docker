#!/bin/bash
# Test script for VulnX MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/vulnx-mcp:latest"
PORT=8116
BINARY="vulnx"
CONTAINER_NAME="vulnx-mcp-test"
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

if [ -z "${PDCP_API_KEY:-}" ]; then
    echo -e "${YELLOW}⚠️  NOTE: PDCP_API_KEY not set. Vulnx works without it but is rate-limited.${NC}"
    echo -e "${YELLOW}   Get a free key at https://cloud.projectdiscovery.io${NC}"
    echo ""
fi
...
echo "================================================================================="
echo -e "${BLUE}VulnX MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
# Test 3: MCP stdio — initialize + tools/list
...
# Test 4: MCP HTTP streamable — initialize
...
info "[Test 6] MCP HTTP — tools/call (list_search_filters)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"list_search_filters","arguments":{}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call list_search_filters returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call list_search_filters returned content"
else
    fail "tools/call list_search_filters did not return expected response"
