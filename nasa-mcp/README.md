...
MCP server wrapper for [NASA](https://github.com/programcomputer/nasa-mcp-server) — upstream package `@programcomputer/nasa-mcp-server`.
...
```bash
docker build -t hackerdogs/nasa-mcp:latest .
```
...
```bash
docker run -d --rm --name nasa-mcp-test -p 8651:8651 \
  -e MCP_TRANSPORT=streamable-http \
  -e NASA_API_KEY \
  hackerdogs/nasa-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8651/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8651/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
