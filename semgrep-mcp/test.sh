...
PASS=0
FAIL=0
IMAGE="hackerdogs/semgrep-mcp:latest"
PORT=8335
BINARY="semgrep"
CONTAINER_NAME="semgrep-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Semgrep MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_semgrep)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_semgrep","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_semgrep returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_semgrep returned content"
else
    fail "tools/call run_semgrep did not return expected response"
