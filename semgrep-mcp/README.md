...
MCP server wrapper for [Semgrep](https://github.com/semgrep/semgrep) — Lightweight static analysis for code security with 5000+ rules.

## What is Semgrep?

Semgrep (semgrep) is a security tool that provides: **Lightweight static analysis for code security with 5000+ rules.**

See [semgrep/semgrep](https://github.com/semgrep/semgrep) for full documentation.
...
Run semgrep with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "semgrep output will appear here"
}
```
...
- "Run semgrep with --help to see all available options."
- "Use semgrep to scan the target 192.168.1.1."
- "What options does semgrep support? Show me its help output."
...
```bash
docker run -i --rm hackerdogs/semgrep-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8335:8335 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8335 \
  hackerdogs/semgrep-mcp:latest
```
...
```json
{
  "mcpServers": {
    "semgrep-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/semgrep-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "semgrep-mcp": {
      "url": "http://localhost:8485/semgrep-mcp/mcp",
      "headers": {
...
```bash
docker build -t hackerdogs/semgrep-mcp:latest .
```
...
```bash
docker run -d --rm --name semgrep-mcp-test -p 8335:8335 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/semgrep-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8335/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8335/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8335/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_semgrep","arguments":{"arguments":"--help"}}}'
```
