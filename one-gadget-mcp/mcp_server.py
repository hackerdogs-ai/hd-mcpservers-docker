#!/usr/bin/env python3
"""One-Gadget MCP Server — Find one-shot RCE gadgets in libc.

Wraps the one_gadget CLI (david942j/one_gadget) to expose
...
logger = logging.getLogger("one-gadget-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8276"))
...
ONE_GADGET_BIN = os.environ.get("ONE_GADGET_BIN", "one_gadget")


def _find_binary() -> str:
    """Locate the one_gadget binary, raising a clear error if missing."""
    path = shutil.which(ONE_GADGET_BIN)
    if path is None:
        logger.error("one_gadget binary not found on PATH")
        raise FileNotFoundError(
            f"one_gadget binary not found. Ensure it is installed and available "
            f"on PATH, or set ONE_GADGET_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a one_gadget command and return structured output.
...
@mcp.tool()
async def run_one_gadget(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run one_gadget with the given arguments.
    ...
    logger.info("run_one_gadget called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("one_gadget command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"one_gadget failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"one_gadget {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting one-gadget-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
