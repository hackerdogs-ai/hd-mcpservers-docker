#!/bin/bash
# Test script for One-Gadget MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/one-gadget-mcp:latest"
PORT=8276
BINARY="one_gadget"
CONTAINER_NAME="one-gadget-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}One-Gadget MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_one_gadget)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_one_gadget","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_one_gadget returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_one_gadget returned content"
else
    fail "tools/call run_one_gadget did not return expected response"
