...
MCP server wrapper for [One Gadget](https://github.com/david942j/one_gadget) — Find one-shot RCE gadgets in libc.

## What is One Gadget?

One Gadget (one_gadget) is a security tool that provides: **Find one-shot RCE gadgets in libc.**

See [david942j/one_gadget](https://github.com/david942j/one_gadget) for full documentation.

**No API keys required** — One Gadget runs locally inside the Docker container.

**Summary.** MCP server wrapper for [One Gadget](https://github.com/david942j/one_gadget) — Find one-shot RCE gadgets in libc.

**Tools:**
- `run_one_gadget` — Run one_gadget with the given arguments. Returns structured JSON output.
...
Run one_gadget with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "one_gadget output will appear here"
}
```
...
- "Run one_gadget with --help to see all available options."
- "Use one-gadget to scan the target 192.168.1.1."
- "What options does one_gadget support? Show me its help output."
- "Run one-gadget against example.com with default settings."
- "Execute one_gadget with verbose output enabled."
- "Use the one-gadget tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/one-gadget-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8276:8276 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8276 \
  hackerdogs/one-gadget-mcp:latest
```
...
```json
{
  "mcpServers": {
    "one-gadget-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/one-gadget-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "one-gadget-mcp": {
      "url": "http://localhost:8485/one-gadget-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8276` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/one-gadget-mcp:latest .
```
...
```bash
docker run -d --rm --name one-gadget-mcp-test -p 8276:8276 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/one-gadget-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8276/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8276/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8276/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_one_gadget","arguments":{"arguments":"--help"}}}'
```
