...
PASS=0; FAIL=0
IMAGE="hackerdogs/crtsh-mcp:latest"
PORT=8381
CONTAINER_NAME="crtsh-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"crtsh","arguments":{"target":"example.com"}}}'

echo "========== crtsh-mcp test (compliance) =========="
...
info "[3] Stdio tools/call crtsh"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 5 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'result\|content\|\[\]' && pass "stdio tools/call crtsh" || fail "stdio tools/call crtsh"
...
info "[5] HTTP streamable tools/call crtsh"
...
echo "$CALL_HTTP" | grep -q 'result\|content' && pass "HTTP tools/call crtsh" || fail "HTTP tools/call crtsh"
