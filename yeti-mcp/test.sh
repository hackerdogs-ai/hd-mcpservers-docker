#!/bin/bash
# Test script for Yeti MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/yeti-mcp:latest"
PORT=8337
BINARY="yeti-mcp"
CONTAINER_NAME="yeti-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Yeti MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_yeti_mcp)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_yeti_mcp","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_yeti_mcp returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_yeti_mcp returned content"
else
    fail "tools/call run_yeti_mcp did not return expected response"
