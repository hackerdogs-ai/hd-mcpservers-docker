#!/bin/bash
# Test script for OpenVAS MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/openvas-mcp:latest"
PORT=8300
BINARY="openvas"
CONTAINER_NAME="openvas-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}OpenVAS MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_openvas)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_openvas","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_openvas returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_openvas returned content"
else
    fail "tools/call run_openvas did not return expected response"
