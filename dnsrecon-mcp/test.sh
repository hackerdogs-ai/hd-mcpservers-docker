#!/bin/bash
# Test script for DNSRecon MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/dnsrecon-mcp:latest"
PORT=8203
BINARY="dnsrecon"
CONTAINER_NAME="dnsrecon-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}DNSRecon MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_dnsrecon)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_dnsrecon","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_dnsrecon returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_dnsrecon returned content"
else
    fail "tools/call run_dnsrecon did not return expected response"
