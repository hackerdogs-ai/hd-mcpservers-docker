#!/bin/bash
# Test script for BloodHound AI MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/bloodhound-mcp-ai-mcp:latest"
PORT=8338
...
echo "================================================================================="
echo -e "${BLUE}BloodHound AI MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_bloodhound_mcp)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_bloodhound_mcp","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_bloodhound_mcp returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_bloodhound_mcp returned content"
else
    fail "tools/call run_bloodhound_mcp did not return expected response"
