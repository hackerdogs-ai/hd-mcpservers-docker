#!/usr/bin/env python3
"""SecureMCP MCP Server — MCP server security hardening and validation tool.

Wraps the securemcp CLI (makalin/SecureMCP) to expose
...
logger = logging.getLogger("securemcp-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8297"))
...
def _find_binary() -> str:
    """Locate the securemcp binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("securemcp binary not found on PATH")
        raise FileNotFoundError(
            f"securemcp binary not found. Ensure it is installed and available "
            f"on PATH, or set SECUREMCP_BIN to the full path."
        )
    return path
...
@mcp.tool()
async def run_securemcp(
    arguments: str,
    timeout_seconds: int = 600,
) -> str:
    """Run securemcp with the given arguments.
    ...
    if result["return_code"] != 0:
        logger.warning("securemcp command failed with exit code %d", result["return_code"])
        error_detail = result["stderr"] or result["stdout"] or "Unknown error"
        return json.dumps(
            {
                "error": True,
                "message": f"securemcp failed (exit code {result['return_code']})",
                "detail": error_detail.strip(),
                "command": f"securemcp {' '.join(args)}",
            },
            indent=2,
        )
...
def main():
    logger.info("Starting securemcp-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
