#!/bin/bash
# Test script for SecureMCP MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/securemcp-mcp:latest"
PORT=8297
BINARY="securemcp"
CONTAINER_NAME="securemcp-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}SecureMCP MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_securemcp)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_securemcp","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_securemcp returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_securemcp returned content"
else
    fail "tools/call run_securemcp did not return expected response"
