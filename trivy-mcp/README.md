...
MCP server wrapper for [Trivy](https://github.com/aquasecurity/trivy) — Comprehensive vulnerability scanner for containers, filesystems, and IaC.

## What is Trivy?

Trivy (trivy) is a security tool that provides: **Comprehensive vulnerability scanner for containers, filesystems, and IaC.**

See [aquasecurity/trivy](https://github.com/aquasecurity/trivy) for full documentation.

**No API keys required** — Trivy runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Trivy](https://github.com/aquasecurity/trivy) — Comprehensive vulnerability scanner for containers, filesystems, and IaC.

**Tools:**
- `run_trivy` — Run trivy with the given arguments. Returns structured JSON output.
...
Run trivy with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "trivy output will appear here"
}
```
...
- "Run trivy with --help to see all available options."
- "Use trivy to scan the target 192.168.1.1."
- "What options does trivy support? Show me its help output."
- "Run trivy against example.com with default settings."
- "Execute trivy with verbose output enabled."
- "Use the trivy tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/trivy-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8330:8330 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8330 \
  hackerdogs/trivy-mcp:latest
```
...
```json
{
  "mcpServers": {
    "trivy-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/trivy-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "trivy-mcp": {
      "url": "http://localhost:8485/trivy-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8330` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/trivy-mcp:latest .
```
...
```bash
docker run -d --rm --name trivy-mcp-test -p 8330:8330 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/trivy-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8330/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8330/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8330/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_trivy","arguments":{"arguments":"--help"}}}'
```
