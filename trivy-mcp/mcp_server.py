#!/usr/bin/env python3
"""Trivy MCP Server — Comprehensive vulnerability scanner for containers, filesystems, and IaC.

Wraps the trivy CLI (aquasecurity/trivy) to expose
...
MCP_PORT = int(os.environ.get("MCP_PORT", "8330"))
...
def _find_binary() -> str:
    ...
    if path is None:
        ...
        raise FileNotFoundError(
            f"trivy binary not found. Ensure it is installed and available "
...
async def run_trivy(
    ...
    logger.info("run_trivy called with arguments=%s source_url=%s", arguments, source_url)
...
def main():
    logger.info("Starting trivy-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
