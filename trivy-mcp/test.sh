#!/bin/bash
# Test script for Trivy MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/trivy-mcp:latest"
PORT=8330
BINARY="trivy"
CONTAINER_NAME="trivy-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Trivy MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_trivy)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_trivy","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_trivy returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_trivy returned content"
else
    fail "tools/call run_trivy did not return expected response"
