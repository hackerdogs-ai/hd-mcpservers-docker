...
docker run -i --rm \
  ...
  hackerdogs/aws-eks-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8614:8614 \
...
| `MCP_PORT` | `8614` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/aws-eks-mcp:latest .
```
...
```bash
docker run -d --rm --name aws-eks-mcp-test -p 8614:8614 \
  -e MCP_TRANSPORT=streamable-http \
  -e AWS_REGION \
  -e AWS_ACCESS_KEY_ID \
  -e AWS_SECRET_ACCESS_KEY \
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8614/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8614/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
```json
{
  "mcpServers": {
    "aws-eks-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "hackerdogs/aws-eks-mcp:latest"],
      "env": {}
    }
  }
}
...
```bash
docker run -d -p 8614:8614 -e MCP_TRANSPORT=streamable-http hackerdogs/aws-eks-mcp:latest
```
