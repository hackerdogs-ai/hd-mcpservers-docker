#!/bin/bash
# Test script for Kube-Hunter MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/kube-hunter-mcp:latest"
PORT=8252
BINARY="kube-hunter"
CONTAINER_NAME="kube-hunter-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Kube-Hunter MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_kube_hunter)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_kube_hunter","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_kube_hunter returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_kube_hunter returned content"
else
    fail "tools/call run_kube_hunter did not return expected response"
