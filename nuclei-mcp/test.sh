...
PASS=0; FAIL=0
IMAGE="hackerdogs/nuclei-mcp:latest"
PORT=8391
CONTAINER_NAME="nuclei-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_nuclei","arguments":{"arguments":"-h"}}}'
echo "========== nuclei-mcp test (compliance) =========="
info "[1] Install"
...
info "[3] Stdio tools/call run_nuclei"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 5 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'result\|content' && pass "stdio tools/call run_nuclei" || fail "stdio tools/call run_nuclei"
...
info "[5] HTTP streamable tools/call run_nuclei"
...
echo "$CALL_HTTP" | grep -q 'result\|content' && pass "HTTP tools/call run_nuclei" || fail "HTTP tools/call run_nuclei"
