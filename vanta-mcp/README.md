...
MCP server wrapper for [Vanta](https://github.com/VantaInc/vanta-mcp-server) — Vanta compliance and security monitoring MCP integration.

## What is Vanta?

Vanta (vanta-mcp) is a security tool that provides: **Vanta compliance and security monitoring MCP integration.**
...
Run vanta-mcp with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "vanta-mcp output will appear here"
}
```
...
- "Run vanta-mcp with --help to see all available options."
- "Use vanta-mcp to scan the target 192.168.1.1."
- "What options does vanta-mcp support? Show me its help output."
- "Run vanta-mcp against example.com with default settings."
- "Execute vanta-mcp with verbose output enabled."
- "Use the vanta-mcp tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/vanta-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8348:8348 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8348 \
  hackerdogs/vanta-mcp:latest
```
...
```json
{
  "mcpServers": {
    "vanta-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/vanta-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "vanta-mcp": {
      "url": "http://localhost:8485/vanta-mcp/mcp",
      "headers": {
...
```bash
docker build -t hackerdogs/vanta-mcp:latest .
```
...
```bash
docker run -d --rm --name vanta-mcp-test -p 8348:8348 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/vanta-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8348/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8348/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8348/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_vanta_mcp","arguments":{"arguments":"--help"}}}'
```

**4. Clean up:**

```bash
docker stop vanta-mcp-test
```
