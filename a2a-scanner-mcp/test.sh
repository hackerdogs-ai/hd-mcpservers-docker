#!/bin/bash
# Test script for A2A Scanner MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/a2a-scanner-mcp:latest"
PORT=8341
BINARY="a2a-scanner"
CONTAINER_NAME="a2a-scanner-mcp-test"
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON="$(command -v python3 2>/dev/null || command -v python 2>/dev/null || true)"
if [ -z "$PYTHON" ]; then
    echo "python3 or python is required on PATH (for MCP stdio helper)" >&2
    exit 1
fi
...
echo "================================================================================="
echo -e "${BLUE}A2A Scanner MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_a2a_scanner)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_a2a_scanner","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_a2a_scanner returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_a2a_scanner returned content"
else
    fail "tools/call run_a2a_scanner did not return expected response"
