#!/bin/bash
# Test script for NetExec MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/netexec-mcp:latest"
PORT=8212
BINARY="netexec"
CONTAINER_NAME="netexec-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}NetExec MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_netexec)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_netexec","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_netexec returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_netexec returned content"
else
    fail "tools/call run_netexec did not return expected response"
