#!/bin/bash
# Test script for RustScan MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/rustscan-mcp:latest"
PORT=8200
BINARY="rustscan"
CONTAINER_NAME="rustscan-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}RustScan MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_rustscan)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_rustscan","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_rustscan returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_rustscan returned content"
else
    fail "tools/call run_rustscan did not return expected response"
