#!/bin/bash
# Test script for SMBMap MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/smbmap-mcp:latest"
PORT=8211
BINARY="smbmap"
CONTAINER_NAME="smbmap-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}SMBMap MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_smbmap)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_smbmap","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_smbmap returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_smbmap returned content"
else
    fail "tools/call run_smbmap did not return expected response"
