#!/bin/bash
# Test script for Psudohash MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/psudohash-mcp:latest"
PORT=8287
BINARY="psudohash"
CONTAINER_NAME="psudohash-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Psudohash MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_psudohash)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_psudohash","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_psudohash returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_psudohash returned content"
else
    fail "tools/call run_psudohash did not return expected response"
