#!/bin/bash
# Test script for CORScanner MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/corscanner-mcp:latest"
PORT=8292
BINARY="cors_scan"
CONTAINER_NAME="corscanner-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}CORScanner MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_cors_scan)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_corscanner","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_cors_scan returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_cors_scan returned content"
else
    fail "tools/call run_cors_scan did not return expected response"
