...
MCP server wrapper for [Steghide](https://github.com/StefanHetze/steghide) — Steganography detection and extraction.

## What is Steghide?

Steghide (steghide) is a security tool that provides: **Steganography detection and extraction.**

See [StefanHetze/steghide](https://github.com/StefanHetze/steghide) for full documentation.

**No API keys required** — Steghide runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Steghide](https://github.com/StefanHetze/steghide) — Steganography detection and extraction.

**Tools:**
- `run_steghide` — Run steghide with the given arguments. Returns structured JSON output.
...
Run steghide with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "steghide output will appear here"
}
```
...
- "Run steghide with --help to see all available options."
- "Use steghide to scan the target 192.168.1.1."
- "What options does steghide support? Show me its help output."
- "Run steghide against example.com with default settings."
- "Execute steghide with verbose output enabled."
- "Use the steghide tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/steghide-mcp:latest
```
...
docker run -d -p 8250:8250 \
  ...
  -e MCP_PORT=8250 \
  hackerdogs/steghide-mcp:latest
```
...
```json
{
  "mcpServers": {
    "steghide-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/steghide-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "steghide-mcp": {
      "url": "http://localhost:8485/steghide-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8250` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/steghide-mcp:latest .
```
...
```bash
docker run -d --rm --name steghide-mcp-test -p 8250:8250 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/steghide-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8250/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8250/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8250/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_steghide","arguments":{"arguments":"--help"}}}'
```
