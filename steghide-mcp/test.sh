#!/bin/bash
# Test script for Steghide MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/steghide-mcp:latest"
PORT=8250
BINARY="steghide"
CONTAINER_NAME="steghide-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Steghide MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_steghide)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_steghide","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_steghide returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_steghide returned content"
else
    fail "tools/call run_steghide did not return expected response"
