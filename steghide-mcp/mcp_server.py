#!/usr/bin/env python3
"""Steghide MCP Server — Steganography detection and extraction.

Wraps the steghide CLI (StefanHetze/steghide) to expose
...
logger = logging.getLogger("steghide-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8250"))
...
STEGHIDE_BIN = os.environ.get("STEGHIDE_BIN", "steghide")


def _find_binary() -> str:
    """Locate the steghide binary, raising a clear error if missing."""
    path = shutil.which(STEGHIDE_BIN)
    if path is None:
        logger.error("steghide binary not found on PATH")
        raise FileNotFoundError(
            f"steghide binary not found. Ensure it is installed and available "
            f"on PATH, or set STEGHIDE_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a steghide command and return structured output.
...
async def run_steghide(
    ...
    """Run steghide with the given arguments.
    ...
    logger.info("run_steghide called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("steghide command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"steghide failed (exit code {result['return_code']})",
...
def main():
    logger.info("Starting steghide-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
