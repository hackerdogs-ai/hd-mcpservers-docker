#!/bin/bash
# Test script for Metasploit MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/metasploit-mcp:latest"
PORT=8236
BINARY="msfconsole"
CONTAINER_NAME="metasploit-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Metasploit MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_metasploit)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_metasploit","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_metasploit returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_metasploit returned content"
else
    fail "tools/call run_metasploit did not return expected response"
