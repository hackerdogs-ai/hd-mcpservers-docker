#!/bin/bash
# Test script for OSV MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/osv-mcp:latest"
PORT=8347
BINARY="osv-mcp"
CONTAINER_NAME="osv-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}OSV MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_osv_mcp)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_osv","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_osv_mcp returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_osv_mcp returned content"
else
    fail "tools/call run_osv_mcp did not return expected response"
