...
```bash
docker run -i --rm hackerdogs/terraform-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8670:8670 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8670 \
  hackerdogs/terraform-mcp:latest
```
...
```json
{
  "mcpServers": {
    "terraform-mcp": {
      "url": "http://localhost:8485/terraform-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8670` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/terraform-mcp:latest .
```
...
```bash
docker run -d --rm --name terraform-mcp-test -p 8670:8670 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/terraform-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8670/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8670/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
