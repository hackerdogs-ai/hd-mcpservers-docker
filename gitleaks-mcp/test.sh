#!/bin/bash
# Test script for Gitleaks MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/gitleaks-mcp:latest"
PORT=8332
BINARY="gitleaks"
CONTAINER_NAME="gitleaks-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Gitleaks MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_gitleaks)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_gitleaks","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_gitleaks returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_gitleaks returned content"
else
    fail "tools/call run_gitleaks did not return expected response"
