...
MCP server wrapper for [Gitleaks](https://github.com/gitleaks/gitleaks) — Secret detection tool for git repositories and files.

## What is Gitleaks?

Gitleaks (gitleaks) is a security tool that provides: **Secret detection tool for git repositories and files.**

See [gitleaks/gitleaks](https://github.com/gitleaks/gitleaks) for full documentation.

**No API keys required** — Gitleaks runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Gitleaks](https://github.com/gitleaks/gitleaks) — Secret detection tool for git repositories and files.

**Tools:**
- `run_gitleaks` — Run gitleaks with the given arguments. Returns structured JSON output.
...
Run gitleaks with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "gitleaks output will appear here"
}
```
...
- "Run gitleaks with --help to see all available options."
- "Use gitleaks to scan the target 192.168.1.1."
- "What options does gitleaks support? Show me its help output."
- "Run gitleaks against example.com with default settings."
- "Execute gitleaks with verbose output enabled."
- "Use the gitleaks tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/gitleaks-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8332:8332 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8332 \
  hackerdogs/gitleaks-mcp:latest
```
...
```json
{
  "mcpServers": {
    "gitleaks-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/gitleaks-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "gitleaks-mcp": {
      "url": "http://localhost:8485/gitleaks-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8332` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/gitleaks-mcp:latest .
```
...
```bash
docker run -d --rm --name gitleaks-mcp-test -p 8332:8332 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/gitleaks-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8332/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8332/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8332/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_gitleaks","arguments":{"arguments":"--help"}}}'
```
