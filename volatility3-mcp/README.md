...
MCP server wrapper for [Volatility3](https://github.com/volatilityfoundation/volatility3) — Next-generation memory forensics.

## What is Volatility3?

Volatility3 (volatility) is a security tool that provides: **Next-generation memory forensics.**

See [volatilityfoundation/volatility3](https://github.com/volatilityfoundation/volatility3) for full documentation.

**No API keys required** — Volatility3 runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Volatility3](https://github.com/volatilityfoundation/volatility3) — Next-generation memory forensics.

**Tools:**
- `run_volatility3` — Run volatility with the given arguments. Returns structured JSON output.
...
Run volatility with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "volatility output will appear here"
}
```
...
- "Run volatility with --help to see all available options."
- "Use volatility3 to scan the target 192.168.1.1."
- "What options does volatility support? Show me its help output."
- "Run volatility3 against example.com with default settings."
- "Execute volatility with verbose output enabled."
- "Use the volatility3 tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/volatility3-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8247:8247 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8247 \
  hackerdogs/volatility3-mcp:latest
```
...
```json
{
  "mcpServers": {
    "volatility3-mcp": {
      "url": "http://localhost:8485/volatility3-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8247` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/volatility3-mcp:latest .
```
...
```bash
docker run -d --rm --name volatility3-mcp-test -p 8247:8247 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/volatility3-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8247/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8247/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8247/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_volatility3","arguments":{"arguments":"--help"}}}'
```
