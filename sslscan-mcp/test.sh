#!/bin/bash
# Test script for SSLScan MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/sslscan-mcp:latest"
PORT=8323
BINARY="sslscan"
CONTAINER_NAME="sslscan-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}SSLScan MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_sslscan)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_sslscan","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_sslscan returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_sslscan returned content"
else
    fail "tools/call run_sslscan did not return expected response"
