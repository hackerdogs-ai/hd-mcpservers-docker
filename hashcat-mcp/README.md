...
MCP server wrapper for [Hashcat](https://github.com/hashcat/hashcat) — GPU-accelerated password recovery.

## What is Hashcat?

Hashcat (hashcat) is a security tool that provides: **GPU-accelerated password recovery.**

See [hashcat/hashcat](https://github.com/hashcat/hashcat) for full documentation.

**No API keys required** — Hashcat runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Hashcat](https://github.com/hashcat/hashcat) — GPU-accelerated password recovery.
...
```json
{
  "mcpServers": {
    "hashcat-mcp": {
      "url": "http://localhost:8485/hashcat-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8235` | HTTP port (only used with `streamable-http`) |
