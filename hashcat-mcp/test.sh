#!/bin/bash
# Test script for Hashcat MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/hashcat-mcp:latest"
PORT=8235
BINARY="hashcat"
CONTAINER_NAME="hashcat-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Hashcat MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_hashcat)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_hashcat","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_hashcat returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_hashcat returned content"
else
    fail "tools/call run_hashcat did not return expected response"
