#!/bin/bash
# Test script for Nikto MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/nikto-mcp:latest"
PORT=8219
BINARY="nikto"
CONTAINER_NAME="nikto-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Nikto MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_nikto)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_nikto","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_nikto returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_nikto returned content"
else
    fail "tools/call run_nikto did not return expected response"
