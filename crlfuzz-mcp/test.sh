#!/bin/bash
# Test script for CRLFuzz MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/crlfuzz-mcp:latest"
PORT=8290
BINARY="crlfuzz"
CONTAINER_NAME="crlfuzz-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}CRLFuzz MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_crlfuzz)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_crlfuzz","arguments":{"arguments":"-h"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_crlfuzz returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_crlfuzz returned content"
else
    fail "tools/call run_crlfuzz did not return expected response"
