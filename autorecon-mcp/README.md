...
MCP server wrapper for [Autorecon](https://github.com/Tib3rius/AutoRecon) — Automated reconnaissance. Run AutoRecon with target and options.

## What is Autorecon?

Autorecon (autorecon) is a security tool that provides: **Automated reconnaissance. Run AutoRecon with target and options.**

See [Tib3rius/AutoRecon](https://github.com/Tib3rius/AutoRecon) for full documentation.
...
Run autorecon with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "autorecon output will appear here"
}
```
...
- "Run autorecon with --help to see all available options."
- "Use autorecon to scan the target 192.168.1.1."
- "What options does autorecon support? Show me its help output."
- "Run autorecon against example.com with default settings."
- "Execute autorecon with verbose output enabled."
- "Use the autorecon tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/autorecon-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8201:8201 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8201 \
  hackerdogs/autorecon-mcp:latest
```
...
```json
{
  "mcpServers": {
    "autorecon-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/autorecon-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "autorecon-mcp": {
      "url": "http://localhost:8485/autorecon-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8201` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/autorecon-mcp:latest .
```
...
```bash
docker run -d --rm --name autorecon-mcp-test -p 8201:8201 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/autorecon-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8201/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8201/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8201/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_autorecon","arguments":{"arguments":"--help"}}}'
```
