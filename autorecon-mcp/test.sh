#!/bin/bash
# Test script for AutoRecon MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/autorecon-mcp:latest"
PORT=8201
BINARY="autorecon"
CONTAINER_NAME="autorecon-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}AutoRecon MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_autorecon)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_autorecon","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_autorecon returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_autorecon returned content"
else
    fail "tools/call run_autorecon did not return expected response"
