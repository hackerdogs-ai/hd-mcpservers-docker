#!/bin/bash
# Test script for Gobuster MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/gobuster-mcp:latest"
PORT=8213
BINARY="gobuster"
CONTAINER_NAME="gobuster-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Gobuster MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_gobuster)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_gobuster","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_gobuster returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_gobuster returned content"
else
    fail "tools/call run_gobuster did not return expected response"
