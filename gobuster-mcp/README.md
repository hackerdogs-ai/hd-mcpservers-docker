...
MCP server wrapper for [Gobuster](https://github.com/OWASP/gobuster) — Directory/file/DNS enumeration for web and vhosts.

## What is Gobuster?

Gobuster (gobuster) is a security tool that provides: **Directory/file/DNS enumeration for web and vhosts.**

See [OWASP/gobuster](https://github.com/OWASP/gobuster) for full documentation.

**No API keys required** — Gobuster runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Gobuster](https://github.com/OWASP/gobuster) — Directory/file/DNS enumeration for web and vhosts.

**Tools:**
- `run_gobuster` — Run gobuster with the given arguments. Returns structured JSON output.
...
Run gobuster with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "gobuster output will appear here"
}
```
...
- "Run gobuster with --help to see all available options."
- "Use gobuster to scan the target 192.168.1.1."
- "What options does gobuster support? Show me its help output."
- "Run gobuster against example.com with default settings."
- "Execute gobuster with verbose output enabled."
- "Use the gobuster tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/gobuster-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8213:8213 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8213 \
  hackerdogs/gobuster-mcp:latest
```
...
```json
{
  "mcpServers": {
    "gobuster-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/gobuster-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "gobuster-mcp": {
      "url": "http://localhost:8485/gobuster-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8213` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/gobuster-mcp:latest .
```
...
```bash
docker run -d --rm --name gobuster-mcp-test -p 8213:8213 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/gobuster-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8213/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8213/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8213/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_gobuster","arguments":{"arguments":"--help"}}}'
```
