#!/usr/bin/env python3
"""Gobuster MCP Server — Directory/file/DNS enumeration for web and vhosts.

Wraps the gobuster CLI (OWASP/gobuster) to expose
...
logger = logging.getLogger("gobuster-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8213"))

mcp = FastMCP(
    "Gobuster MCP Server",
    instructions=(
        "Directory/file/DNS enumeration for web and vhosts."
    ),
)
...
def _find_binary() -> str:
    """Locate the gobuster binary, raising a clear error if missing."""
    path = shutil.which(GOBUSTER_BIN)
    if path is None:
        logger.error("gobuster binary not found on PATH")
        raise FileNotFoundError(
            f"gobuster binary not found. Ensure it is installed and available "
            f"on PATH, or set GOBUSTER_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a gobuster command and return structured output.
...
async def run_gobuster(
    ...
    """Run gobuster with the given arguments.
    ...
    logger.info("run_gobuster called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("gobuster command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"gobuster failed (exit code {result['return_code']})",
...
def main():
    logger.info("Starting gobuster-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
