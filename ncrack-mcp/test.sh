#!/bin/bash
# Test script for Ncrack MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/ncrack-mcp:latest"
PORT=8306
BINARY="ncrack"
CONTAINER_NAME="ncrack-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Ncrack MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
if python3 "$PROJECT_DIR/../scripts/mcp_stdio_docker_tools_list.py" --check "$IMAGE" 2>/dev/null; then
...
info "[Test 6] MCP HTTP — tools/call (run_ncrack)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_ncrack","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_ncrack returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_ncrack returned content"
else
    fail "tools/call run_ncrack did not return expected response"
