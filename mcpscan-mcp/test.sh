#!/bin/bash
# Test script for MCPScan MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/mcpscan-mcp:latest"
PORT=8296
BINARY="mcpscan"
CONTAINER_NAME="mcpscan-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}MCPScan MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_mcpscan)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_mcpscan","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_mcpscan returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_mcpscan returned content"
else
    fail "tools/call run_mcpscan did not return expected response"
