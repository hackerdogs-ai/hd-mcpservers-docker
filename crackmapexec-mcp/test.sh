#!/bin/bash
# Test script for CrackMapExec MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/crackmapexec-mcp:latest"
PORT=8208
BINARY="crackmapexec"
CONTAINER_NAME="crackmapexec-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}CrackMapExec MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_crackmapexec)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_crackmapexec","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_crackmapexec returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_crackmapexec returned content"
else
    fail "tools/call run_crackmapexec did not return expected response"
