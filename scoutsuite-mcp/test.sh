#!/bin/bash
# Test script for Scout Suite MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/scoutsuite-mcp:latest"
PORT=8251
BINARY="scout"
CONTAINER_NAME="scoutsuite-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Scout Suite MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_scoutsuite)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_scoutsuite","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_scoutsuite returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_scoutsuite returned content"
else
    fail "tools/call run_scoutsuite did not return expected response"
