#!/bin/bash
# Test script for Julius MCP Server
# Tests MCP protocol compliance via JSON-RPC (stdio and HTTP streamable)
# Compatible with macOS (no coreutils required) and FastMCP 3.x (SSE + sessions)
...
PORT=8100
BINARY="julius"
CONTAINER_NAME="julius-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Julius MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (list_probes)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"list_probes","arguments":{}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call list_probes returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call list_probes returned content"
else
    fail "tools/call list_probes did not return expected response"
