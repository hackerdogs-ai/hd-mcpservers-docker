#!/bin/bash
# Build and Publish Julius MCP Docker Image to Docker Hub
...
if [ "$SHOW_HELP" = true ] || { [ "$DO_BUILD" = false ] && [ "$DO_PUBLISH" = false ]; }; then
    echo "Build and Publish Julius MCP Docker Image to Docker Hub"
    echo ""
    echo "Usage:"
    ...
    echo "  --platforms parallel|sequential  Push both platforms at once (default) or one at a time (better for 502 retries)"
    echo "  --help, -h   Show this help message"
    echo ""
    echo "Examples:"
    echo "  $0 --build hackerdogs                        # Build locally"
    echo "  $0 --publish hackerdogs                      # Publish existing image"
    echo "  $0 --build --publish hackerdogs              # Build and publish (tag: latest)"
    echo "  $0 --build --publish hackerdogs v1.0.0       # Build and publish with tag v1.0.0"
    echo "  $0 --build --publish --platforms sequential hackerdogs latest  # Sequential push (better for 502)"
    exit 0
fi
...
if [ "$DO_BUILD" = true ] && [ "$DO_PUBLISH" = true ]; then
    echo -e "${BLUE}Building and Publishing Julius MCP Docker Image${NC}"
elif [ "$DO_BUILD" = true ]; then
    echo -e "${BLUE}Building Julius MCP Docker Image${NC}"
else
    echo -e "${BLUE}Publishing Julius MCP Docker Image to Docker Hub${NC}"
fi
echo "================================================================================="
if [ "$DO_PUBLISH" = true ]; then
...
# Check Docker Hub auth for publishing
if [ "$DO_PUBLISH" = true ]; then
...
# Retry wrapper for 502 / transient network errors
MAX_RETRIES=5
...
if [ "$DO_BUILD" = true ]; then
    ...
    if [ "$DO_PUBLISH" = true ]; then
        ...
        if [ "$PLATFORMS_MODE" = "sequential" ]; then
            echo "Mode: sequential (push amd64 then arm64, then create manifest)"
        else
        ...
        for tag in "${TAGS[@]}"; do
            if [ "$PLATFORMS_MODE" = "sequential" ]; then
                ...
                if ! do_build_push_with_retry docker buildx build \
                    ...
                    echo "Tip: Try --platforms sequential for smaller uploads."
                    exit 1
                fi
            fi
