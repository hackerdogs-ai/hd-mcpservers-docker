#!/bin/bash
# Test script for SMTP User Enum MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/smtp-user-enum-mcp:latest"
PORT=8325
BINARY="smtp-user-enum"
CONTAINER_NAME="smtp-user-enum-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}SMTP User Enum MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_smtp_user_enum)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_smtp_user_enum","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_smtp_user_enum returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_smtp_user_enum returned content"
else
    fail "tools/call run_smtp_user_enum did not return expected response"
