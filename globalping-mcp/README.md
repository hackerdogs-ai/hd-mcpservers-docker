...
```bash
docker run -i --rm hackerdogs/globalping-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8643:8643 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8643 \
  hackerdogs/globalping-mcp:latest
```
...
| `MCP_PORT` | `8643` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/globalping-mcp:latest .
```
...
```bash
docker run -d --rm --name globalping-mcp-test -p 8643:8643 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/globalping-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8643/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8643/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
