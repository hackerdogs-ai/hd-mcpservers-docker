#!/bin/bash
# Test script for ZMap MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/zmap-mcp:latest"
PORT=8303
BINARY="zmap"
CONTAINER_NAME="zmap-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}ZMap MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_zmap)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_zmap","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_zmap returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_zmap returned content"
else
    fail "tools/call run_zmap did not return expected response"
