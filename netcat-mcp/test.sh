#!/bin/bash
# Test script for Netcat MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/netcat-mcp:latest"
PORT=8327
BINARY="nc"
CONTAINER_NAME="netcat-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Netcat MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_nc)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_netcat","arguments":{"arguments":"-h"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_nc returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_nc returned content"
else
    fail "tools/call run_nc did not return expected response"
