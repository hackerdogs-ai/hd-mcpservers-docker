#!/bin/bash
# Test script for Enum4linux MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/enum4linux-mcp:latest"
PORT=8209
BINARY="enum4linux"
CONTAINER_NAME="enum4linux-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Enum4linux MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_enum4linux)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_enum4linux","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_enum4linux returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_enum4linux returned content"
else
    fail "tools/call run_enum4linux did not return expected response"
