...
docker run -i --rm \
  ...
  hackerdogs/s3-mcp-server-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8662:8662 \
...
| `MCP_PORT` | `8662` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/s3-mcp-server-mcp:latest .
```
...
```bash
docker run -d --rm --name s3-mcp-server-mcp-test -p 8662:8662 \
  -e MCP_TRANSPORT=streamable-http \
  -e AWS_REGION \
  -e AWS_ACCESS_KEY_ID \
  -e AWS_SECRET_ACCESS_KEY \
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8662/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8662/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
