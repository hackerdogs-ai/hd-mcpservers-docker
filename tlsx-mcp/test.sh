#!/bin/bash
# Test script for tlsx MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/tlsx-mcp:latest"
PORT=8109
BINARY="tlsx"
CONTAINER_NAME="tlsx-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}tlsx MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (scan_tls)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"scan_tls","arguments":{"hosts":"example.com"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call scan_tls returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call scan_tls returned content"
else
    fail "tools/call scan_tls did not return expected response"
