...
docker run -i --rm \
  -e BRAVE_API_KEY \
  hackerdogs/brave-search-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8629:8629 \
...
| `MCP_PORT` | `8629` | HTTP port (only used with `streamable-http`) |
| `BRAVE_API_KEY` | — | Brave Search API key — get one at [brave.com/search/api](https://brave.com/search/api/) |
...
```bash
docker build -t hackerdogs/brave-search-mcp:latest .
```
...
```bash
docker run -d --rm --name brave-search-mcp-test -p 8629:8629 \
  -e MCP_TRANSPORT=streamable-http \
  -e BRAVE_API_KEY \
  hackerdogs/brave-search-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8629/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8629/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
