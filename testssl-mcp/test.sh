#!/bin/bash
# Test script for TestSSL MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/testssl-mcp:latest"
PORT=8279
BINARY="testssl.sh"
CONTAINER_NAME="testssl-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}TestSSL MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_testssl)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_testssl","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_testssl returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_testssl returned content"
else
    fail "tools/call run_testssl did not return expected response"
