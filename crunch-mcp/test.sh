#!/bin/bash
# Test script for Crunch MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/crunch-mcp:latest"
PORT=8324
BINARY="crunch"
CONTAINER_NAME="crunch-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Crunch MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_crunch)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_crunch","arguments":{"arguments":"-h"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_crunch returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_crunch returned content"
else
    fail "tools/call run_crunch did not return expected response"
