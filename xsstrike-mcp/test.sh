#!/bin/bash
# Test script for XSStrike MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/xsstrike-mcp:latest"
PORT=8349
BINARY="xsstrike"
CONTAINER_NAME="xsstrike-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}XSStrike MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_xsstrike)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_xsstrike","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_xsstrike returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_xsstrike returned content"
else
    fail "tools/call run_xsstrike did not return expected response"
