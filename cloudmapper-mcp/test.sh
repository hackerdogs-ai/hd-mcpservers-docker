#!/bin/bash
# Test script for CloudMapper MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/cloudmapper-mcp:latest"
PORT=8268
BINARY="cloudmapper"
CONTAINER_NAME="cloudmapper-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}CloudMapper MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_cloudmapper)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_cloudmapper","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_cloudmapper returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_cloudmapper returned content"
else
    fail "tools/call run_cloudmapper did not return expected response"
