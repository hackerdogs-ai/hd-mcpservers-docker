#!/bin/bash
# Test script for cvemap MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/cvemap-mcp:latest"
PORT=8106
BINARY="cvemap"
CONTAINER_NAME="cvemap-mcp-test"
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

if [ -z "${PDCP_API_KEY:-}" ]; then
    echo -e "${YELLOW}⚠️  NOTE: PDCP_API_KEY not set. Cvemap may be rate-limited.${NC}"
...
echo "================================================================================="
echo -e "${BLUE}cvemap MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (list_filters)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"list_filters","arguments":{}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call list_filters returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call list_filters returned content"
else
    fail "tools/call list_filters did not return expected response"
