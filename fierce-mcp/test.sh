#!/bin/bash
# Test script for Fierce MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/fierce-mcp:latest"
PORT=8202
BINARY="fierce"
CONTAINER_NAME="fierce-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Fierce MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_fierce)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_fierce","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_fierce returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_fierce returned content"
else
    fail "tools/call run_fierce did not return expected response"
