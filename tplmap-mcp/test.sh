#!/bin/bash
# Test script for Tplmap MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/tplmap-mcp:latest"
PORT=8267
BINARY="tplmap"
CONTAINER_NAME="tplmap-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Tplmap MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_tplmap)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_tplmap","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_tplmap returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_tplmap returned content"
else
    fail "tools/call run_tplmap did not return expected response"
