...
PASS=0; FAIL=0
IMAGE="file-operations-mcp"
PORT=8522
CONTAINER_NAME="file-operations-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"file_info","arguments":{"file_path":"/etc/hostname"}}}'
echo "========== file-operations-mcp test (compliance) =========="
info "[1] Install"
...
while [ "$WAITED" -lt "${MCP_HTTP_LIST_MAX_WAIT:-30}" ]; do
  curl -s -D "$MCP_HDR" -o /dev/null -X POST "http://localhost:${PORT}/mcp" -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" -d "$INIT_REQ" 2>/dev/null || true
