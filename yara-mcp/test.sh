#!/bin/bash
# Test script for YARA MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/yara-mcp:latest"
PORT=8328
BINARY="yara"
CONTAINER_NAME="yara-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}YARA MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_yara)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_yara","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_yara returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_yara returned content"
else
    fail "tools/call run_yara did not return expected response"
