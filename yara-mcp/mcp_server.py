#!/usr/bin/env python3
"""YARA MCP Server — Pattern matching tool for malware researchers and classification.

Wraps the yara CLI (VirusTotal/yara) to expose
...
MCP_PORT = int(os.environ.get("MCP_PORT", "8328"))
...
BIN_NAME = os.environ.get("YARA_BIN", "yara")


def _find_binary() -> str:
    """Locate the yara binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("yara binary not found on PATH")
        raise FileNotFoundError(
            f"yara binary not found. Ensure it is installed and available "
            f"on PATH, or set YARA_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a yara command and return structured output.
...
async def run_yara(
    ...
    """Run yara with the given arguments.
    ...
    logger.info("run_yara called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("yara command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"yara failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"yara {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting yara-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
