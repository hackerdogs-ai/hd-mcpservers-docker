#!/bin/bash
# Test script for Crowbar MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/crowbar-mcp:latest"
PORT=8307
BINARY="crowbar"
CONTAINER_NAME="crowbar-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Crowbar MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_crowbar)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_crowbar","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_crowbar returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_crowbar returned content"
else
    fail "tools/call run_crowbar did not return expected response"
