...
MCP server wrapper for [Wpscan](https://github.com/wpscanteam/wpscan) — WordPress security scanner.

## What is Wpscan?

Wpscan (wpscan) is a security tool that provides: **WordPress security scanner.**

See [wpscanteam/wpscan](https://github.com/wpscanteam/wpscan) for full documentation.
...
Run wpscan with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "wpscan output will appear here"
}
```
...
- "Run wpscan with --help to see all available options."
- "Use wpscan to scan the target 192.168.1.1."
- "What options does wpscan support? Show me its help output."
- "Run wpscan against example.com with default settings."
- "Execute wpscan with verbose output enabled."
- "Use the wpscan tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/wpscan-mcp:latest
```
...
```json
{
  "mcpServers": {
    "wpscan-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/wpscan-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "wpscan-mcp": {
      "url": "http://localhost:8485/wpscan-mcp/mcp",
      "headers": {
...
```bash
docker build -t hackerdogs/wpscan-mcp:latest .
```
...
```bash
docker run -d --rm --name wpscan-mcp-test -p 8220:8220 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/wpscan-mcp:latest
```
...
curl -s -X POST http://localhost:8220/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_wpscan","arguments":{"arguments":"--help"}}}'
```
