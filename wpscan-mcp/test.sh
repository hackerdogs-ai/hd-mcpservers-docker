#!/bin/bash
# Test script for WPScan MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/wpscan-mcp:latest"
PORT=8220
BINARY="wpscan"
CONTAINER_NAME="wpscan-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}WPScan MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_wpscan)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_wpscan","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_wpscan returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_wpscan returned content"
else
    fail "tools/call run_wpscan did not return expected response"
