...
PASS=0; FAIL=0
IMAGE="${CLOUDFLARE_IMAGE:-hackerdogs/cloudflare-mcp:latest}"
PORT=8633
CONTAINER_NAME="cloudflare-mcp-test"
...
MCP_BASE="http://${HTTP_HOST}:${PORT}/mcp"

echo "========== cloudflare-mcp test (compliance) =========="
info "[1] Install"
...
while [ "$WAITED" -lt "${MCP_HTTP_LIST_MAX_WAIT:-180}" ]; do
  curl -s --max-time "$CURL_INIT" -D /tmp/mcp_h -o /dev/null -X POST "$MCP_BASE" -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" -d "$INIT_REQ" 2>/dev/null || true
  ...
  curl -s --max-time "$CURL_INIT" -X POST "$MCP_BASE" -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" $SESS_HDR -d "$INIT_NOTIF" >/dev/null 2>&1 || true
  TOOLS_RESP=$(curl -s --max-time "$CURL_MCP" -X POST "$MCP_BASE" -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" $SESS_HDR -d "$LIST_REQ" 2>/dev/null) || true
