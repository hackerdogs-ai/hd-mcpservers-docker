...
docker run -i --rm \
  -e PINECONE_API_KEY \
  hackerdogs/pinecone-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8657:8657 \
...
| `MCP_PORT` | `8657` | HTTP port (only used with `streamable-http`) |
| `PINECONE_API_KEY` | — | Pinecone API key — find at [app.pinecone.io](https://app.pinecone.io/) |
...
```bash
docker build -t hackerdogs/pinecone-mcp:latest .
```
...
```bash
docker run -d --rm --name pinecone-mcp-test -p 8657:8657 \
  -e MCP_TRANSPORT=streamable-http \
  -e PINECONE_API_KEY \
  hackerdogs/pinecone-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8657/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8657/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
