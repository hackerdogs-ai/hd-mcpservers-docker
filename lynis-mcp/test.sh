#!/bin/bash
# Test script for Lynis MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/lynis-mcp:latest"
PORT=8326
BINARY="lynis"
CONTAINER_NAME="lynis-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Lynis MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_lynis)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_lynis","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_lynis returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_lynis returned content"
else
    fail "tools/call run_lynis did not return expected response"
