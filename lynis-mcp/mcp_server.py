#!/usr/bin/env python3
"""Lynis MCP Server — Security auditing and compliance testing tool for Linux/Unix.

Wraps the lynis CLI (CISOfy/lynis) to expose
...
logger = logging.getLogger("lynis-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8326"))
...
BIN_NAME = os.environ.get("LYNIS_BIN", "lynis")


def _find_binary() -> str:
    """Locate the lynis binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("lynis binary not found on PATH")
        raise FileNotFoundError(
            f"lynis binary not found. Ensure it is installed and available "
            f"on PATH, or set LYNIS_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a lynis command and return structured output.
...
async def run_lynis(
    ...
    """Run lynis with the given arguments.
    ...
    logger.info("run_lynis called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("lynis command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"lynis failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"lynis {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting lynis-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
