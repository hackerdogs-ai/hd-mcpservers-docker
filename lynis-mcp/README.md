...
MCP server wrapper for [Lynis](https://github.com/CISOfy/lynis) — Security auditing and compliance testing tool for Linux/Unix.

## What is Lynis?

Lynis (lynis) is a security tool that provides: **Security auditing and compliance testing tool for Linux/Unix.**

See [CISOfy/lynis](https://github.com/CISOfy/lynis) for full documentation.
...
Run lynis with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "lynis output will appear here"
}
```
...
- "Run lynis with --help to see all available options."
- "Use lynis to scan the target 192.168.1.1."
- "What options does lynis support? Show me its help output."
...
```bash
docker run -i --rm hackerdogs/lynis-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8326:8326 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8326 \
  hackerdogs/lynis-mcp:latest
```
...
```json
{
  "mcpServers": {
    "lynis-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/lynis-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "lynis-mcp": {
      "url": "http://localhost:8485/lynis-mcp/mcp",
      "headers": {
...
```bash
docker build -t hackerdogs/lynis-mcp:latest .
```
...
```bash
docker run -d --rm --name lynis-mcp-test -p 8326:8326 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/lynis-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8326/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8326/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8326/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_lynis","arguments":{"arguments":"--help"}}}'
```
