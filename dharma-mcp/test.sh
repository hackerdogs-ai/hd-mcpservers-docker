#!/bin/bash
# Test script for Dharma MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/dharma-mcp:latest"
PORT=8334
BINARY="dharma"
CONTAINER_NAME="dharma-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Dharma MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_dharma)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_dharma","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_dharma returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_dharma returned content"
else
    fail "tools/call run_dharma did not return expected response"
