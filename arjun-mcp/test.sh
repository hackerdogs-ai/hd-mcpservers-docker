...
PASS=0; FAIL=0
IMAGE="hackerdogs/arjun-mcp:latest"
PORT=8383
CONTAINER_NAME="arjun-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"do_arjun","arguments":{"arguments":"-h"}}}'
echo "========== arjun-mcp test (compliance) =========="
info "[1] Install"
...
echo "$CALL_OUT" | grep -q 'result\|content' && pass "stdio tools/call do_arjun" || fail "stdio tools/call do_arjun"
...
info "[5] HTTP streamable tools/call do_arjun"
...
echo "$CALL_HTTP" | grep -q 'result\|content' && pass "HTTP tools/call do_arjun" || fail "HTTP tools/call do_arjun"
