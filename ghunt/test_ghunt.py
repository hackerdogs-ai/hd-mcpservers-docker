#!/usr/bin/env python3
"""
Test suite for GHunt MCP Server.
Tests the GHunt Google account search tool.
"""
...
def main():
    """Run tests for GHunt."""
    print("="*60)
    print("GHunt MCP Server Test Suite")
    print("="*60)
    
    docker_cmd = ["docker", "run", "--rm", "-i", "hackerdogs/ghunt-mcp-server:latest"]
    ...
    if "result" in response and response["result"].get("serverInfo", {}).get("name") == "ghunt-mcp-server":
    ...
    if "result" in response and "tools" in response["result"]:
        ...
        if len(tools) > 0 and tools[0].get("name") == "ghunt_google_search":
    ...
    # Test 3: GHunt Google search (may fail without proper setup)
    tests_total += 1
    result = test_tool(
        docker_cmd,
        "ghunt_google_search",
