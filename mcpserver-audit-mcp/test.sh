#!/bin/bash
# Test script for MCPServer Audit MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/mcpserver-audit-mcp:latest"
PORT=8340
BINARY="mcpserver-audit"
CONTAINER_NAME="mcpserver-audit-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}MCPServer Audit MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_mcpserver_audit)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_mcpserver_audit","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_mcpserver_audit returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_mcpserver_audit returned content"
else
    fail "tools/call run_mcpserver_audit did not return expected response"
