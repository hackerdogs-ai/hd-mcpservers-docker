...
MCP server wrapper for [Clair](https://github.com/quay/clair) — Container vulnerability analysis.

## What is Clair?

Clair (clair) is a security tool that provides: **Container vulnerability analysis.**

See [quay/clair](https://github.com/quay/clair) for full documentation.

**No API keys required** — Clair runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Clair](https://github.com/quay/clair) — Container vulnerability analysis.

**Tools:**
- `run_clair` — Run clair with the given arguments. Returns structured JSON output.
...
Run clair with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "clair output will appear here"
}
```
...
- "Run clair with --help to see all available options."
- "Use clair to scan the target 192.168.1.1."
- "What options does clair support? Show me its help output."
- "Run clair against example.com with default settings."
- "Execute clair with verbose output enabled."
- "Use the clair tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/clair-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8270:8270 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8270 \
  hackerdogs/clair-mcp:latest
```
...
```json
{
  "mcpServers": {
    "clair-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/clair-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "clair-mcp": {
      "url": "http://localhost:8485/clair-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8270` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/clair-mcp:latest .
```
...
```bash
docker run -d --rm --name clair-mcp-test -p 8270:8270 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/clair-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8270/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8270/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8270/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_clair","arguments":{"arguments":"--help"}}}'
```
