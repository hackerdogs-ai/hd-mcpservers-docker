#!/bin/bash
# Test script for Clair MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/clair-mcp:latest"
PORT=8270
BINARY="clair"
CONTAINER_NAME="clair-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Clair MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_clair)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_clair","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_clair returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_clair returned content"
else
    fail "tools/call run_clair did not return expected response"
