#!/usr/bin/env python3
"""Clair MCP Server — Container vulnerability analysis.

Wraps the clair CLI (quay/clair) to expose
...
logger = logging.getLogger("clair-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8270"))
...
CLAIR_BIN = os.environ.get("CLAIR_BIN", "clair")


def _find_binary() -> str:
    """Locate the clair binary, raising a clear error if missing."""
    path = shutil.which(CLAIR_BIN)
    if path is None:
        logger.error("clair binary not found on PATH")
        raise FileNotFoundError(
            f"clair binary not found. Ensure it is installed and available "
            f"on PATH, or set CLAIR_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a clair command and return structured output.
...
async def run_clair(
    ...
    """Run clair with the given arguments.
    ...
    logger.info("run_clair called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("clair command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"clair failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"clair {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting clair-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
