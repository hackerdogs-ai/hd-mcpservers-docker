#!/usr/bin/env python3
"""ggshield MCP Server — Secret detection and code security scanning by GitGuardian.

Wraps the ggshield CLI (GitGuardian/ggshield) to expose
...
logger = logging.getLogger("ggshield-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8363"))
...
BIN_NAME = os.environ.get("GGSHIELD_BIN", "ggshield")


def _find_binary() -> str:
    """Locate the ggshield binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("ggshield binary not found on PATH")
        raise FileNotFoundError(
            f"ggshield binary not found. Ensure it is installed and available "
            f"on PATH, or set GGSHIELD_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a ggshield command and return structured output.
...
@mcp.tool()
async def run_ggshield(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run ggshield with the given arguments.
    ...
    logger.info("run_ggshield called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("ggshield command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"ggshield failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"ggshield {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting ggshield-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
