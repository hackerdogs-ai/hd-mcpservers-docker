#!/bin/bash
# Test script for ggshield MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/ggshield-mcp:latest"
PORT=8363
BINARY="ggshield"
CONTAINER_NAME="ggshield-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}ggshield MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_ggshield)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_ggshield","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_ggshield returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_ggshield returned content"
else
    fail "tools/call run_ggshield did not return expected response"
