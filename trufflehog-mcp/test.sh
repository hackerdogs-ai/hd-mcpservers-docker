#!/bin/bash
# Test script for TruffleHog MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/trufflehog-mcp:latest"
PORT=8258
BINARY="trufflehog"
CONTAINER_NAME="trufflehog-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}TruffleHog MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_trufflehog)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_trufflehog","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_trufflehog returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_trufflehog returned content"
else
    fail "tools/call run_trufflehog did not return expected response"
