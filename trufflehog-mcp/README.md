...
MCP server wrapper for [Trufflehog](https://github.com/trufflesecurity/trufflehog) — Git repository secret scanning.

## What is Trufflehog?

Trufflehog (trufflehog) is a security tool that provides: **Git repository secret scanning.**

See [trufflesecurity/trufflehog](https://github.com/trufflesecurity/trufflehog) for full documentation.

**No API keys required** — Trufflehog runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Trufflehog](https://github.com/trufflesecurity/trufflehog) — Git repository secret scanning.

**Tools:**
- `run_trufflehog` — Run trufflehog with the given arguments. Returns structured JSON output.
...
Run trufflehog with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "trufflehog output will appear here"
}
```
...
- "Run trufflehog with --help to see all available options."
- "Use trufflehog to scan the target 192.168.1.1."
- "What options does trufflehog support? Show me its help output."
- "Run trufflehog against example.com with default settings."
- "Execute trufflehog with verbose output enabled."
- "Use the trufflehog tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/trufflehog-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8258:8258 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8258 \
  hackerdogs/trufflehog-mcp:latest
```
...
```json
{
  "mcpServers": {
    "trufflehog-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/trufflehog-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
| `MCP_PORT` | `8258` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/trufflehog-mcp:latest .
```
...
```bash
docker run -d --rm --name trufflehog-mcp-test -p 8258:8258 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/trufflehog-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8258/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8258/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8258/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_trufflehog","arguments":{"arguments":"--help"}}}'
```
...
```json
{
  "mcpServers": {
    "trufflehog-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "hackerdogs/trufflehog-mcp:latest"],
      "env": {}
    }
  }
}
...
```bash
docker run -d -p 8258:8258 -e MCP_TRANSPORT=streamable-http hackerdogs/trufflehog-mcp:latest
```
