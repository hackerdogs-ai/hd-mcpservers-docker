#!/usr/bin/env python3
"""TruffleHog MCP Server — Git repository secret scanning.

Wraps the trufflehog CLI (trufflesecurity/trufflehog) to expose
...
MCP_PORT = int(os.environ.get("MCP_PORT", "8258"))
...
def _find_binary() -> str:
    """Locate the trufflehog binary, raising a clear error if missing."""
    path = shutil.which(TRUFFLEHOG_BIN)
    if path is None:
        logger.error("trufflehog binary not found on PATH")
        raise FileNotFoundError(
            f"trufflehog binary not found. Ensure it is installed and available "
            f"on PATH, or set TRUFFLEHOG_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a trufflehog command and return structured output.
...
async def run_trufflehog(
    ...
    """Run trufflehog with the given arguments.
    ...
    logger.info("run_trufflehog called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("trufflehog command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"trufflehog failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"trufflehog {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting trufflehog-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
