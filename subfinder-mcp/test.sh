#!/bin/bash
# Test script for Subfinder MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/subfinder-mcp:latest"
PORT=8367
BINARY="subfinder"
CONTAINER_NAME="subfinder-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Subfinder MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
BINARY_OUTPUT=$(docker run --rm --entrypoint "$BINARY" "$IMAGE" -version 2>&1 | head -5 || docker run --rm --entrypoint "$BINARY" "$IMAGE" -h 2>&1 | head -5 || true)
...
# Test 6: MCP HTTP — tools/call (list_subfinder_sources)
info "[Test 6] MCP HTTP — tools/call (list_subfinder_sources)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"list_subfinder_sources","arguments":{}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call list_subfinder_sources returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call list_subfinder_sources returned content"
else
    fail "tools/call list_subfinder_sources did not return expected response"
    [ -n "$CALL_RESP" ] && echo "       Response: ${CALL_RESP:0:500}"
fi
echo ""

# Test 7: MCP HTTP — tools/call (run_subfinder --help)
info "[Test 7] MCP HTTP — tools/call (run_subfinder)"
CALL_REQ='{"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"run_subfinder","arguments":{"arguments":"-h","timeout_seconds":30}}}'
CALL_RESP=$(curl -s --max-time 60 -X POST "http://localhost:${PORT}/mcp" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json, text/event-stream" \
    $SESSION_HDR \
    -d "$CALL_REQ" 2>/dev/null || true)
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_subfinder returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_subfinder returned content"
else
    fail "tools/call run_subfinder did not return expected response"
    [ -n "$CALL_RESP" ] && echo "       Response: ${CALL_RESP:0:500}"
fi
echo ""
