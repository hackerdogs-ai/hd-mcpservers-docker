...
PASS=0
FAIL=0
IMAGE="hackerdogs/medusa-mcp:latest"
PORT=8261
BINARY="medusa"
CONTAINER_NAME="medusa-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Medusa MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_medusa)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_medusa","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_medusa returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_medusa returned content"
else
    fail "tools/call run_medusa did not return expected response"
