#!/bin/bash
# Test script for ARP-Scan MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/arp-scan-mcp:latest"
PORT=8205
BINARY="arp-scan"
CONTAINER_NAME="arp-scan-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}ARP-Scan MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_arp_scan)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_arp_scan","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_arp_scan returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_arp_scan returned content"
else
    fail "tools/call run_arp_scan did not return expected response"
