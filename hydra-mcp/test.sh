#!/bin/bash
# Test script for Hydra MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/hydra-mcp:latest"
PORT=8233
BINARY="hydra"
CONTAINER_NAME="hydra-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Hydra MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_hydra","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_hydra returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_hydra returned content"
else
    fail "tools/call run_hydra did not return expected response"
