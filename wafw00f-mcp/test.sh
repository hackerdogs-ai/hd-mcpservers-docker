#!/bin/bash
# Test script for Wafw00f MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/wafw00f-mcp:latest"
PORT=8230
BINARY="wafw00f"
CONTAINER_NAME="wafw00f-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Wafw00f MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_wafw00f)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_wafw00f","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_wafw00f returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_wafw00f returned content"
else
    fail "tools/call run_wafw00f did not return expected response"
