#!/bin/bash
# Test script for naabu MCP Server
...
echo "================================================================================="
echo -e "${BLUE}naabu MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (scan_ports)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"scan_ports","arguments":{"hosts":"scanme.nmap.org","top_ports":10}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call scan_ports returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call scan_ports returned content"
else
    fail "tools/call scan_ports did not return expected response"
