#!/usr/bin/env python3
"""Volatility MCP Server — Memory forensics framework (v2).

Wraps the vol.py CLI (volatilityfoundation/volatility) to expose
...
MCP_PORT = int(os.environ.get("MCP_PORT", "8248"))

mcp = FastMCP(
    "Volatility MCP Server",
    instructions=(
        "Memory forensics framework (v2)."
    ),
)

VOLATILITY_BIN = os.environ.get("VOLATILITY_BIN", "vol.py")


def _find_binary() -> str:
    """Locate the vol.py binary, raising a clear error if missing."""
    path = shutil.which(VOLATILITY_BIN)
    if path is None:
        logger.error("vol.py binary not found on PATH")
        raise FileNotFoundError(
            f"vol.py binary not found. Ensure it is installed and available "
            f"on PATH, or set VOLATILITY_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a vol.py command and return structured output.
...
async def run_volatility(
    ...
    """Run vol.py with the given arguments.
    ...
    logger.info("run_volatility called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("vol.py command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"vol.py failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"vol.py {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting volatility-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
