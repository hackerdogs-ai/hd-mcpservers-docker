...
MCP server wrapper for [Volatility](https://github.com/volatilityfoundation/volatility) — Memory forensics framework (v2).

## What is Volatility?

Volatility (vol.py) is a security tool that provides: **Memory forensics framework (v2).**

See [volatilityfoundation/volatility](https://github.com/volatilityfoundation/volatility) for full documentation.

**No API keys required** — Volatility runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Volatility](https://github.com/volatilityfoundation/volatility) — Memory forensics framework (v2).

**Tools:**
- `run_volatility` — Run vol.py with the given arguments. Returns structured JSON output.
...
Run vol.py with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "vol.py output will appear here"
}
```
...
- "Run vol.py with --help to see all available options."
- "Use volatility to scan the target 192.168.1.1."
- "What options does vol.py support? Show me its help output."
- "Run volatility against example.com with default settings."
- "Execute vol.py with verbose output enabled."
- "Use the volatility tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/volatility-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8248:8248 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8248 \
  hackerdogs/volatility-mcp:latest
```
...
```json
{
  "mcpServers": {
    "volatility-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/volatility-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
| `MCP_PORT` | `8248` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/volatility-mcp:latest .
```
...
```bash
docker run -d --rm --name volatility-mcp-test -p 8248:8248 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/volatility-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8248/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8248/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8248/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_volatility","arguments":{"arguments":"--help"}}}'
```
...
```json
{
  "mcpServers": {
    "volatility-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "hackerdogs/volatility-mcp:latest"],
      "env": {}
    }
  }
}
...
```bash
docker run -d -p 8248:8248 -e MCP_TRANSPORT=streamable-http hackerdogs/volatility-mcp:latest
```
