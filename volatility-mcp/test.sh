#!/bin/bash
# Test script for Volatility MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/volatility-mcp:latest"
PORT=8248
BINARY="vol.py"
CONTAINER_NAME="volatility-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Volatility MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_volatility)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_volatility","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_volatility returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_volatility returned content"
else
    fail "tools/call run_volatility did not return expected response"
