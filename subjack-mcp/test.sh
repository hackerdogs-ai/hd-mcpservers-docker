#!/bin/bash
# Test script for Subjack MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/subjack-mcp:latest"
PORT=8260
BINARY="subjack"
CONTAINER_NAME="subjack-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Subjack MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_subjack)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_subjack","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_subjack returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_subjack returned content"
else
    fail "tools/call run_subjack did not return expected response"
