#!/bin/bash
# Test script for NBTScan MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/nbtscan-mcp:latest"
PORT=8206
BINARY="nbtscan"
CONTAINER_NAME="nbtscan-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}NBTScan MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_nbtscan)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_nbtscan","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_nbtscan returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_nbtscan returned content"
else
    fail "tools/call run_nbtscan did not return expected response"
