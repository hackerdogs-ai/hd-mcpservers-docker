#!/bin/bash
# Test script for BruteSpray MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/brutespray-mcp:latest"
PORT=8308
BINARY="brutespray"
CONTAINER_NAME="brutespray-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}BruteSpray MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_brutespray)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_brutespray","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_brutespray returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_brutespray returned content"
else
    fail "tools/call run_brutespray did not return expected response"
