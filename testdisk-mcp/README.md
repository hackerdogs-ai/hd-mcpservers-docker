...
**Summary.** MCP server wrapper for [Testdisk](https://github.com/cgsecurity/testdisk) — Disk partition recovery and file carving.

**Tools:**
- `run_testdisk` — Run testdisk with the given arguments. Returns structured JSON output.
...
```json
{
  "mcpServers": {
    "testdisk-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/testdisk-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "testdisk-mcp": {
      "url": "http://localhost:8485/testdisk-mcp/mcp",
      "headers": {
