#!/bin/bash
# Test script for TestDisk MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/testdisk-mcp:latest"
PORT=8283
BINARY="testdisk"
CONTAINER_NAME="testdisk-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}TestDisk MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_testdisk)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_testdisk","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_testdisk returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_testdisk returned content"
else
    fail "tools/call run_testdisk did not return expected response"
