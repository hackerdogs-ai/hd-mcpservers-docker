#!/usr/bin/env python3
"""TestDisk MCP Server — Disk partition recovery and file carving.

Wraps the testdisk CLI (cgsecurity/testdisk) to expose
...
mcp = FastMCP(
    "TestDisk MCP Server",
    instructions=(
        "Disk partition recovery and file carving."
    ),
)

TESTDISK_BIN = os.environ.get("TESTDISK_BIN", "testdisk")


def _find_binary() -> str:
    """Locate the testdisk binary, raising a clear error if missing."""
    path = shutil.which(TESTDISK_BIN)
    if path is None:
        logger.error("testdisk binary not found on PATH")
        raise FileNotFoundError(
            f"testdisk binary not found. Ensure it is installed and available "
            f"on PATH, or set TESTDISK_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a testdisk command and return structured output.
...
@mcp.tool()
async def run_testdisk(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run testdisk with the given arguments.
    ...
    logger.info("run_testdisk called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("testdisk command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"testdisk failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"testdisk {' '.join(args)}",
                },
                indent=2,
            )
