#!/bin/bash
# Test script for Responder MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/responder-mcp:latest"
PORT=8207
BINARY="Responder"
CONTAINER_NAME="responder-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Responder MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_responder)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_responder","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_responder returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_responder returned content"
else
    fail "tools/call run_responder did not return expected response"
