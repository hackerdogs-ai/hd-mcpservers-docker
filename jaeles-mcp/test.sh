#!/bin/bash
# Test script for Jaeles MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/jaeles-mcp:latest"
PORT=8232
BINARY="jaeles"
CONTAINER_NAME="jaeles-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Jaeles MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_jaeles)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_jaeles","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_jaeles returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_jaeles returned content"
else
    fail "tools/call run_jaeles did not return expected response"
