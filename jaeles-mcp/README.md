...
MCP server wrapper for [Jaeles](https://github.com/jaeles-project/jaeles) — Vulnerability scanning with custom signatures.

## What is Jaeles?

Jaeles (jaeles) is a security tool that provides: **Vulnerability scanning with custom signatures.**

See [jaeles-project/jaeles](https://github.com/jaeles-project/jaeles) for full documentation.

**No API keys required** — Jaeles runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Jaeles](https://github.com/jaeles-project/jaeles) — Vulnerability scanning with custom signatures.

**Tools:**
- `run_jaeles` — Run jaeles with the given arguments. Returns structured JSON output.
...
Run jaeles with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "jaeles output will appear here"
}
```
...
- "Run jaeles with --help to see all available options."
- "Use jaeles to scan the target 192.168.1.1."
- "What options does jaeles support? Show me its help output."
- "Run jaeles against example.com with default settings."
- "Execute jaeles with verbose output enabled."
- "Use the jaeles tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/jaeles-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8232:8232 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8232 \
  hackerdogs/jaeles-mcp:latest
```
...
```json
{
  "mcpServers": {
    "jaeles-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/jaeles-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "jaeles-mcp": {
      "url": "http://localhost:8485/jaeles-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8232` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/jaeles-mcp:latest .
```
...
```bash
docker run -d --rm --name jaeles-mcp-test -p 8232:8232 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/jaeles-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8232/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8232/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8232/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_jaeles","arguments":{"arguments":"--help"}}}'
```
