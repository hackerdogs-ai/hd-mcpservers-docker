#!/usr/bin/env python3
"""Jaeles MCP Server — Vulnerability scanning with custom signatures.

Wraps the jaeles CLI (jaeles-project/jaeles) to expose
...
logger = logging.getLogger("jaeles-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8232"))

mcp = FastMCP(
    "Jaeles MCP Server",
    instructions=(
        "Vulnerability scanning with custom signatures."
    ),
)

JAELES_BIN = os.environ.get("JAELES_BIN", "jaeles")


def _find_binary() -> str:
    """Locate the jaeles binary, raising a clear error if missing."""
    path = shutil.which(JAELES_BIN)
    if path is None:
        logger.error("jaeles binary not found on PATH")
        raise FileNotFoundError(
            f"jaeles binary not found. Ensure it is installed and available "
            f"on PATH, or set JAELES_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a jaeles command and return structured output.
...
@mcp.tool()
async def run_jaeles(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run jaeles with the given arguments.
    ...
    logger.info("run_jaeles called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("jaeles command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"jaeles failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"jaeles {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting jaeles-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
