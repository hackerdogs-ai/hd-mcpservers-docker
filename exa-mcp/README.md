...
docker run -i --rm \
  -e EXA_API_KEY \
  hackerdogs/exa-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8637:8637 \
...
| `MCP_PORT` | `8637` | HTTP port (only used with `streamable-http`) |
| `EXA_API_KEY` | — | Exa API key — get one at [exa.ai](https://exa.ai/) |
...
```bash
docker build -t hackerdogs/exa-mcp:latest .
```
...
```bash
docker run -d --rm --name exa-mcp-test -p 8637:8637 \
  -e MCP_TRANSPORT=streamable-http \
  -e EXA_API_KEY \
  hackerdogs/exa-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8637/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8637/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
