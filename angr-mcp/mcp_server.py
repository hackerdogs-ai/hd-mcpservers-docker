#!/usr/bin/env python3
"""Angr MCP Server — Binary analysis with symbolic execution.

Wraps the python3 CLI (angr/angr) to expose
...
logger = logging.getLogger("angr-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8246"))
...
async def run_angr(
    ...
    logger.info("run_angr called with arguments=%s source_url=%s", arguments, source_url)
...
def main():
    logger.info("Starting angr-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
