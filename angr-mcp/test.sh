#!/bin/bash
# Test script for Angr MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/angr-mcp:latest"
PORT=8246
BINARY="python3"
CONTAINER_NAME="angr-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Angr MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_angr)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_angr","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_angr returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_angr returned content"
else
    fail "tools/call run_angr did not return expected response"
