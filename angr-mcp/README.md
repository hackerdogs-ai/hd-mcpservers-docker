...
MCP server wrapper for [Angr](https://github.com/angr/angr) — Binary analysis with symbolic execution.

## What is Angr?

Angr (python3) is a security tool that provides: **Binary analysis with symbolic execution.**

See [angr/angr](https://github.com/angr/angr) for full documentation.

**No API keys required** — Angr runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Angr](https://github.com/angr/angr) — Binary analysis with symbolic execution.

**Tools:**
- `run_angr` — Run python3 with the given arguments. Returns structured JSON output.
...
- "Use angr to scan the target 192.168.1.1."
- "What options does python3 support? Show me its help output."
- "Run angr against example.com with default settings."
- "Execute python3 with verbose output enabled."
- "Use the angr tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/angr-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8246:8246 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8246 \
  hackerdogs/angr-mcp:latest
```
...
```json
{
  "mcpServers": {
    "angr-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/angr-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "angr-mcp": {
      "url": "http://localhost:8485/angr-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8246` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/angr-mcp:latest .
```
...
```bash
docker run -d --rm --name angr-mcp-test -p 8246:8246 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/angr-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8246/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8246/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8246/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_angr","arguments":{"arguments":"--help"}}}'
```
