...
docker run -i --rm \
  ...
  -e JIRA_EMAIL \
  -e JIRA_API_TOKEN \
  hackerdogs/jira-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8649:8649 \
...
| `MCP_PORT` | `8649` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/jira-mcp:latest .
```
...
```bash
docker run -d --rm --name jira-mcp-test -p 8649:8649 \
  -e MCP_TRANSPORT=streamable-http \
  -e JIRA_URL \
  -e JIRA_EMAIL \
  -e JIRA_API_TOKEN \
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8649/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8649/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
