...
MCP server wrapper for [AIBOM](https://github.com/cisco-ai-defense/aibom) — AI Bill of Materials generator for AI model transparency.

## What is AIBOM?

AIBOM (aibom) is a security tool that provides: **AI Bill of Materials generator for AI model transparency.**

See [cisco-ai-defense/aibom](https://github.com/cisco-ai-defense/aibom) for full documentation.

**No API keys required** — AIBOM runs locally inside the Docker container.

**Summary.** MCP server wrapper for [AIBOM](https://github.com/cisco-ai-defense/aibom) — AI Bill of Materials generator for AI model transparency.

**Tools:**
- `run_aibom` — Run aibom with the given arguments. Returns structured JSON output.
...
Run aibom with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "aibom output will appear here"
}
```
...
- "Run aibom with --help to see all available options."
- "Use aibom to scan the target 192.168.1.1."
- "What options does aibom support? Show me its help output."
- "Run aibom against example.com with default settings."
- "Execute aibom with verbose output enabled."
- "Use the aibom tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/aibom-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8343:8343 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8343 \
  hackerdogs/aibom-mcp:latest
```
...
```json
{
  "mcpServers": {
    "aibom-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/aibom-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "aibom-mcp": {
      "url": "http://localhost:8485/aibom-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8343` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/aibom-mcp:latest .
```
...
```bash
docker run -d --rm --name aibom-mcp-test -p 8343:8343 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/aibom-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8343/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8343/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8343/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_aibom","arguments":{"arguments":"--help"}}}'
```
