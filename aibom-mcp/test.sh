#!/bin/bash
# Test script for AIBOM MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/aibom-mcp:latest"
PORT=8343
BINARY="aibom"
CONTAINER_NAME="aibom-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}AIBOM MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_aibom)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_aibom","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_aibom returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_aibom returned content"
else
    fail "tools/call run_aibom did not return expected response"
