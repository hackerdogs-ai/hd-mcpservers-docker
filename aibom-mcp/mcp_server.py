#!/usr/bin/env python3
"""AIBOM MCP Server — AI Bill of Materials generator for AI model transparency.

Wraps the aibom CLI (cisco-ai-defense/aibom) to expose
...
logger = logging.getLogger("aibom-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8343"))
...
BIN_NAME = os.environ.get("AIBOM_BIN", "aibom")


def _find_binary() -> str:
    """Locate the aibom binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("aibom binary not found on PATH")
        raise FileNotFoundError(
            f"aibom binary not found. Ensure it is installed and available "
            f"on PATH, or set AIBOM_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a aibom command and return structured output.
...
async def run_aibom(
    ...
    """Run aibom with the given arguments.
    ...
    logger.info("run_aibom called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("aibom command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"aibom failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"aibom {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting aibom-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
