#!/bin/bash
# Test script for SSTImap MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/sstimap-mcp:latest"
PORT=8289
BINARY="sstimap"
CONTAINER_NAME="sstimap-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}SSTImap MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_sstimap)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_sstimap","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_sstimap returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_sstimap returned content"
else
    fail "tools/call run_sstimap did not return expected response"
