...
MCP server wrapper for [Suricata](https://github.com/Medinios/SuricataMCP) — Suricata network intrusion detection and monitoring.

## What is Suricata?

Suricata (suricata-mcp) is a security tool that provides: **Suricata network intrusion detection and monitoring.**

See [Medinios/SuricataMCP](https://github.com/Medinios/SuricataMCP) for full documentation.
...
Run suricata-mcp with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "suricata-mcp output will appear here"
}
```
...
- "Run suricata-mcp with --help to see all available options."
- "Use suricata-mcp to scan the target 192.168.1.1."
- "What options does suricata-mcp support? Show me its help output."
...
curl -s -X POST http://localhost:8365/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_suricata_mcp","arguments":{"arguments":"--help"}}}'
```
