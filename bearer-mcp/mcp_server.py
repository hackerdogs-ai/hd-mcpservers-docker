#!/usr/bin/env python3
"""Bearer MCP Server — Code security scanning tool for sensitive data flows.

Wraps the bearer CLI (Bearer/bearer) to expose
...
logger = logging.getLogger("bearer-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8360"))
...
BIN_NAME = os.environ.get("BEARER_BIN", "bearer")


def _find_binary() -> str:
    """Locate the bearer binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("bearer binary not found on PATH")
        raise FileNotFoundError(
            f"bearer binary not found. Ensure it is installed and available "
            f"on PATH, or set BEARER_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a bearer command and return structured output.
...
async def run_bearer(
    ...
    """Run bearer with the given arguments.
    ...
    logger.info("run_bearer called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("bearer command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"bearer failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"bearer {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting bearer-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
