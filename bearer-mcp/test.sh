#!/bin/bash
# Test script for Bearer MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/bearer-mcp:latest"
PORT=8360
BINARY="bearer"
CONTAINER_NAME="bearer-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Bearer MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_bearer)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_bearer","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_bearer returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_bearer returned content"
else
    fail "tools/call run_bearer did not return expected response"
