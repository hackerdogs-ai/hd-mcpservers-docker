...
MCP server wrapper for [Bearer](https://github.com/Bearer/bearer) — Code security scanning tool for sensitive data flows.

## What is Bearer?

Bearer (bearer) is a security tool that provides: **Code security scanning tool for sensitive data flows.**

See [Bearer/bearer](https://github.com/Bearer/bearer) for full documentation.

**No API keys required** — Bearer runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Bearer](https://github.com/Bearer/bearer) — Code security scanning tool for sensitive data flows.

**Tools:**
- `run_bearer` — Run bearer with the given arguments. Returns structured JSON output.
...
Run bearer with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "bearer output will appear here"
}
```
...
- "Run bearer with --help to see all available options."
- "Use bearer to scan the target 192.168.1.1."
- "What options does bearer support? Show me its help output."
- "Run bearer against example.com with default settings."
- "Execute bearer with verbose output enabled."
- "Use the bearer tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/bearer-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8360:8360 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8360 \
  hackerdogs/bearer-mcp:latest
```
...
```json
{
  "mcpServers": {
    "bearer-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/bearer-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
| `MCP_PORT` | `8360` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/bearer-mcp:latest .
```
...
```bash
docker run -d --rm --name bearer-mcp-test -p 8360:8360 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/bearer-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8360/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8360/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8360/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_bearer","arguments":{"arguments":"--help"}}}'
```
...
```json
{
  "mcpServers": {
    "bearer-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "hackerdogs/bearer-mcp:latest"],
      "env": {}
    }
  }
}
...
```bash
docker run -d -p 8360:8360 -e MCP_TRANSPORT=streamable-http hackerdogs/bearer-mcp:latest
```
