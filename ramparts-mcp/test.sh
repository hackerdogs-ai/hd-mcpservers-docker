#!/bin/bash
# Test script for Ramparts MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/ramparts-mcp:latest"
PORT=8295
BINARY="ramparts"
CONTAINER_NAME="ramparts-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Ramparts MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_ramparts)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_ramparts","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_ramparts returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_ramparts returned content"
else
    fail "tools/call run_ramparts did not return expected response"
