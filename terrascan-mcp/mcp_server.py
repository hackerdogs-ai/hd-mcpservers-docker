#!/usr/bin/env python3
"""Terrascan MCP Server — Infrastructure security scanner (policy-as-code).

Wraps the terrascan CLI (tenable/terrascan) to expose
...
logger = logging.getLogger("terrascan-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8273"))
...
TERRASCAN_BIN = os.environ.get("TERRASCAN_BIN", "terrascan")


def _find_binary() -> str:
    """Locate the terrascan binary, raising a clear error if missing."""
    path = shutil.which(TERRASCAN_BIN)
    if path is None:
        logger.error("terrascan binary not found on PATH")
        raise FileNotFoundError(
            f"terrascan binary not found. Ensure it is installed and available "
            f"on PATH, or set TERRASCAN_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a terrascan command and return structured output.
...
async def run_terrascan(
    ...
    """Run terrascan with the given arguments.
    ...
    logger.info("run_terrascan called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("terrascan command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"terrascan failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"terrascan {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting terrascan-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
