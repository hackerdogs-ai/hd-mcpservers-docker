#!/bin/bash
# Test script for Terrascan MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/terrascan-mcp:latest"
PORT=8273
BINARY="terrascan"
CONTAINER_NAME="terrascan-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Terrascan MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_terrascan)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_terrascan","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_terrascan returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_terrascan returned content"
else
    fail "tools/call run_terrascan did not return expected response"
