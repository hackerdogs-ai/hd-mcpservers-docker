...
MCP server wrapper for [Terrascan](https://github.com/tenable/terrascan) — Infrastructure security scanner (policy-as-code).

## What is Terrascan?

Terrascan (terrascan) is a security tool that provides: **Infrastructure security scanner (policy-as-code).**

See [tenable/terrascan](https://github.com/tenable/terrascan) for full documentation.

**No API keys required** — Terrascan runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Terrascan](https://github.com/tenable/terrascan) — Infrastructure security scanner (policy-as-code).

**Tools:**
- `run_terrascan` — Run terrascan with the given arguments. Returns structured JSON output.
...
Run terrascan with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "terrascan output will appear here"
}
```
...
- "Run terrascan with --help to see all available options."
- "Use terrascan to scan the target 192.168.1.1."
- "What options does terrascan support? Show me its help output."
- "Run terrascan against example.com with default settings."
- "Execute terrascan with verbose output enabled."
- "Use the terrascan tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/terrascan-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8273:8273 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8273 \
  hackerdogs/terrascan-mcp:latest
```
...
```json
{
  "mcpServers": {
    "terrascan-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/terrascan-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "terrascan-mcp": {
      "url": "http://localhost:8485/terrascan-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8273` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/terrascan-mcp:latest .
```
...
```bash
docker run -d --rm --name terrascan-mcp-test -p 8273:8273 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/terrascan-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8273/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8273/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8273/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_terrascan","arguments":{"arguments":"--help"}}}'
```
