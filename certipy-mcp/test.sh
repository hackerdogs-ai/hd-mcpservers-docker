#!/bin/bash
# Test script for Certipy MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/certipy-mcp:latest"
PORT=8285
BINARY="certipy"
CONTAINER_NAME="certipy-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Certipy MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_certipy)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_certipy","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_certipy returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_certipy returned content"
else
    fail "tools/call run_certipy did not return expected response"
