...
PASS=0; FAIL=0
IMAGE="hackerdogs/pagespeed-mcp:latest"
PORT=8378
CONTAINER_NAME="pagespeed-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_pagespeed","arguments":{"url":"https://example.com","strategy":"mobile"}}}'

echo "========== pagespeed-mcp test (compliance) =========="
...
info "[3] Stdio tools/call run_pagespeed"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 8 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'result\|content\|error' && pass "stdio tools/call run_pagespeed" || fail "stdio tools/call run_pagespeed"
...
info "[5] HTTP streamable tools/call run_pagespeed"
...
echo "$CALL_HTTP" | grep -q 'result\|content\|error' && pass "HTTP tools/call run_pagespeed" || fail "HTTP tools/call run_pagespeed"
