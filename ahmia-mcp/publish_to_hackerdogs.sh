#!/bin/bash
# Build and Publish Ahmia MCP Server Docker Image to Docker Hub
# Image name: ahmia-mcp
...
if [ "$SHOW_HELP" = true ]; then
    echo "Build and Publish Ahmia MCP Server Docker Image to Docker Hub"
    echo ""
    echo "Usage:"
...
if [ "$DO_BUILD" = true ] && [ "$DO_PUBLISH" = true ]; then
    echo -e "${BLUE}Building and Publishing Ahmia MCP Server Docker Image${NC}"
elif [ "$DO_BUILD" = true ]; then
    echo -e "${BLUE}Building Ahmia MCP Server Docker Image${NC}"
else
    echo -e "${BLUE}Publishing Ahmia MCP Server Docker Image to Docker Hub${NC}"
fi
echo "================================================================================="
if [ "$DO_PUBLISH" = true ]; then
    ...
    if [ "$DO_PUBLISH" = true ]; then
        ...
        for tag in "${TAGS[@]}"; do
            if [ "$PLATFORMS_MODE" = "sequential" ]; then
                ...
                if ! do_build_push_with_retry docker buildx build --platform linux/amd64 --provenance=false --sbom=false -f "$DOCKERFILE" -t "${FULL_IMAGE_NAME}:${tag}-amd64" --push . ; then
                    echo -e "${RED}Error: Failed to build/push amd64 after $MAX_RETRIES attempts${NC}"; exit 1
                fi
                echo -e "${GREEN}Pushed ${FULL_IMAGE_NAME}:${tag}-amd64${NC}"
                if ! do_build_push_with_retry docker buildx build --platform linux/arm64 --provenance=false --sbom=false -f "$DOCKERFILE" -t "${FULL_IMAGE_NAME}:${tag}-arm64" --push . ; then
                    echo -e "${RED}Error: Failed to build/push arm64 after $MAX_RETRIES attempts${NC}"; exit 1
                fi
                ...
                if ! do_build_push_with_retry docker buildx build --platform linux/amd64,linux/arm64 --provenance=false --sbom=false -f "$DOCKERFILE" -t "${FULL_IMAGE_NAME}:${tag}" --push . ; then
                    echo -e "${RED}Error: Docker buildx build failed after $MAX_RETRIES attempts${NC}"; exit 1
                fi
            fi
