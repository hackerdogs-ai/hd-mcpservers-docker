#!/bin/bash
# Test script for IPInfo MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/ipinfo-mcp:latest"
PORT=8351
BINARY="ipinfo"
CONTAINER_NAME="ipinfo-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}IPInfo MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_ipinfo)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_ipinfo","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_ipinfo returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_ipinfo returned content"
else
    fail "tools/call run_ipinfo did not return expected response"
