#!/bin/bash
# Test script for DotDotPwn MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/dotdotpwn-mcp:latest"
PORT=8223
BINARY="dotdotpwn"
CONTAINER_NAME="dotdotpwn-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}DotDotPwn MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_dotdotpwn)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_dotdotpwn","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_dotdotpwn returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_dotdotpwn returned content"
else
    fail "tools/call run_dotdotpwn did not return expected response"
