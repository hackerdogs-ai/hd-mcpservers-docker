#!/usr/bin/env python3
"""Falco MCP Server — Runtime security monitoring (containers/K8s).

Wraps the falco CLI (falcosecurity/falco) to expose
...
logger = logging.getLogger("falco-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8271"))

mcp = FastMCP(
    "Falco MCP Server",
    instructions=(
        "Runtime security monitoring (containers/K8s)."
    ),
)

FALCO_BIN = os.environ.get("FALCO_BIN", "falco")


def _find_binary() -> str:
    """Locate the falco binary, raising a clear error if missing."""
    path = shutil.which(FALCO_BIN)
    if path is None:
        logger.error("falco binary not found on PATH")
        raise FileNotFoundError(
            f"falco binary not found. Ensure it is installed and available "
            f"on PATH, or set FALCO_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a falco command and return structured output.
...
async def run_falco(
    ...
    """Run falco with the given arguments.
    ...
    logger.info("run_falco called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("falco command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"falco failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"falco {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting falco-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
