#!/bin/bash
# Test script for Falco MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/falco-mcp:latest"
PORT=8271
BINARY="falco"
CONTAINER_NAME="falco-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Falco MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_falco)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_falco","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_falco returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_falco returned content"
else
    fail "tools/call run_falco did not return expected response"
