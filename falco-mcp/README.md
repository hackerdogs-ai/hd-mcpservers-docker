...
MCP server wrapper for [Falco](https://github.com/falcosecurity/falco) — Runtime security monitoring (containers/K8s).

## What is Falco?

Falco (falco) is a security tool that provides: **Runtime security monitoring (containers/K8s).**

See [falcosecurity/falco](https://github.com/falcosecurity/falco) for full documentation.

**No API keys required** — Falco runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Falco](https://github.com/falcosecurity/falco) — Runtime security monitoring (containers/K8s).

**Tools:**
- `run_falco` — Run falco with the given arguments. Returns structured JSON output.
...
Run falco with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "falco output will appear here"
}
```
...
- "Run falco with --help to see all available options."
- "Use falco to scan the target 192.168.1.1."
- "What options does falco support? Show me its help output."
- "Run falco against example.com with default settings."
- "Execute falco with verbose output enabled."
- "Use the falco tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/falco-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8271:8271 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8271 \
  hackerdogs/falco-mcp:latest
```
...
```json
{
  "mcpServers": {
    "falco-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/falco-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "falco-mcp": {
      "url": "http://localhost:8485/falco-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8271` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/falco-mcp:latest .
```
...
```bash
docker run -d --rm --name falco-mcp-test -p 8271:8271 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/falco-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8271/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8271/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8271/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_falco","arguments":{"arguments":"--help"}}}'
```
