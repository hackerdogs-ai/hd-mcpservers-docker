#!/usr/bin/env python3
"""Dirb MCP Server — Web content scanner with recursive scanning.

Wraps the dirb CLI (darkoperator/dirb) to expose
...
logger = logging.getLogger("dirb-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8214"))

mcp = FastMCP(
    "Dirb MCP Server",
    instructions=(
        "Web content scanner with recursive scanning."
    ),
)

DIRB_BIN = os.environ.get("DIRB_BIN", "dirb")


def _find_binary() -> str:
    """Locate the dirb binary, raising a clear error if missing."""
    path = shutil.which(DIRB_BIN)
    if path is None:
        logger.error("dirb binary not found on PATH")
        raise FileNotFoundError(
            f"dirb binary not found. Ensure it is installed and available "
            f"on PATH, or set DIRB_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a dirb command and return structured output.
...
async def run_dirb(
    ...
    """Run dirb with the given arguments.
    ...
    logger.info("run_dirb called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("dirb command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"dirb failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"dirb {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting dirb-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
