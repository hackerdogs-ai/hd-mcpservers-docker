#!/bin/bash
# Test script for Dirb MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/dirb-mcp:latest"
PORT=8214
BINARY="dirb"
CONTAINER_NAME="dirb-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Dirb MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_dirb)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_dirb","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_dirb returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_dirb returned content"
else
    fail "tools/call run_dirb did not return expected response"
