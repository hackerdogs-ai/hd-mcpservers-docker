#!/bin/bash
# Test script for OnionSearch MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/onionsearch-mcp:latest"
PORT=8372
BINARY="onionsearch"
CONTAINER_NAME="onionsearch-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}OnionSearch MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
BINARY_OUTPUT=$(docker run --rm --entrypoint $BINARY "$IMAGE" --help 2>&1 | head -5 || true)
...
info "[Test 6] MCP HTTP — tools/call (run_onionsearch --help)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_onionsearch","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_onionsearch returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_onionsearch returned content"
else
    fail "tools/call run_onionsearch did not return expected response"
