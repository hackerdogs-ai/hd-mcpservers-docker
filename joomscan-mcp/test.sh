#!/bin/bash
# Test script for JoomScan MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/joomscan-mcp:latest"
PORT=8305
BINARY="joomscan"
CONTAINER_NAME="joomscan-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}JoomScan MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_joomscan)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_joomscan","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_joomscan returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_joomscan returned content"
else
    fail "tools/call run_joomscan did not return expected response"
