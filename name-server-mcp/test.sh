...
PASS=0; FAIL=0
IMAGE="name-server-mcp"
PORT=8525
CONTAINER_NAME="name-server-mcp-test"
...
cleanup() { docker stop "$CONTAINER_NAME" 2>/dev/null || true; docker rm -f "$CONTAINER_NAME" 2>/dev/null || true; rm -f "$MCP_HDR_FILE" 2>/dev/null || true; }
trap cleanup EXIT
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"nameserver_check_ip","arguments":{"ip":"8.8.8.8"}}}'
echo "========== name-server-mcp test (compliance) =========="
info "[1] Install"
...
if "$MCP_PYTHON" "$PROJECT_DIR/../scripts/mcp_stdio_docker_tools_list.py" --check "$IMAGE" 2>/dev/null; then pass "stdio tools/list"; else fail "stdio tools/list"; fi
info "[3] Stdio tools/call"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 4 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
if echo "$CALL_OUT" | grep -q 'result\|content\|error'; then pass "stdio tools/call"; else fail "stdio tools/call"; fi
...
while [ "$WAITED" -lt "${MCP_HTTP_LIST_MAX_WAIT:-30}" ]; do
  : >"$MCP_HDR_FILE"
  curl -s -D "$MCP_HDR_FILE" -o /dev/null -X POST "http://localhost:${PORT}/mcp" -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" -d "$INIT_REQ" 2>/dev/null || true
