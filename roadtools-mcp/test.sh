#!/bin/bash
# Test script for ROADtools MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/roadtools-mcp:latest"
PORT=8331
BINARY="roadrecon"
CONTAINER_NAME="roadtools-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}ROADtools MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_roadrecon)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_roadtools","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_roadrecon returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_roadrecon returned content"
else
    fail "tools/call run_roadrecon did not return expected response"
