#!/bin/bash
# Test script for Wifiphisher MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/wifiphisher-mcp:latest"
PORT=8312
BINARY="wifiphisher"
CONTAINER_NAME="wifiphisher-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Wifiphisher MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_wifiphisher)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_wifiphisher","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_wifiphisher returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_wifiphisher returned content"
else
    fail "tools/call run_wifiphisher did not return expected response"
