#!/bin/bash
# Test script for WappalyzerGo MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/wappalyzergo-mcp:latest"
PORT=8114
BINARY="wappalyzergo-cli"
CONTAINER_NAME="wappalyzergo-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}WappalyzerGo MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
BINARY_OUTPUT=$(docker run --rm "$IMAGE" $BINARY -help 2>&1 | head -5 || true)
...
info "[Test 6] MCP HTTP — tools/call (detect_technologies)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"detect_technologies","arguments":{"urls":"https://example.com"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call detect_technologies returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call detect_technologies returned content"
else
    fail "tools/call detect_technologies did not return expected response"
