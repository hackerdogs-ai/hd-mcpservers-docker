#!/bin/bash
# Test script for Nova Proximity MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/nova-proximity-mcp:latest"
PORT=8298
BINARY="nova-proximity"
CONTAINER_NAME="nova-proximity-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Nova Proximity MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_nova_proximity)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_nova_proximity","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_nova_proximity returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_nova_proximity returned content"
else
    fail "tools/call run_nova_proximity did not return expected response"
