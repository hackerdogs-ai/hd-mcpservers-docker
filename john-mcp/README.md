...
MCP server wrapper for [John](https://github.com/openwall/john) — Password hash cracking with custom rules.

## What is John?

John (john) is a security tool that provides: **Password hash cracking with custom rules.**

See [openwall/john](https://github.com/openwall/john) for full documentation.

**No API keys required** — John runs locally inside the Docker container.

**Summary.** MCP server wrapper for [John](https://github.com/openwall/john) — Password hash cracking with custom rules.

**Tools:**
- `run_john` — Run john with the given arguments. Returns structured JSON output.
...
Run john with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "john output will appear here"
}
```
...
- "Run john with --help to see all available options."
- "Use john to scan the target 192.168.1.1."
- "What options does john support? Show me its help output."
- "Run john against example.com with default settings."
- "Execute john with verbose output enabled."
- "Use the john tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/john-mcp:latest
```
...
```json
{
  "mcpServers": {
    "john-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/john-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "john-mcp": {
      "url": "http://localhost:8485/john-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8234` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/john-mcp:latest .
```
...
```bash
docker run -d --rm --name john-mcp-test -p 8234:8234 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/john-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8234/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8234/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8234/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_john","arguments":{"arguments":"--help"}}}'
```
