#!/bin/bash
# Test script for Maigret MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/maigret-mcp:latest"
PORT=8221
BINARY="maigret"
CONTAINER_NAME="maigret-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Maigret MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_maigret --help)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_maigret","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_maigret returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_maigret returned content"
else
    fail "tools/call run_maigret did not return expected response"
