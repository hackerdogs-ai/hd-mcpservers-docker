#!/bin/bash
# Test script for Retire.js MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/retire-js-mcp:latest"
PORT=8364
BINARY="retire"
CONTAINER_NAME="retire-js-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Retire.js MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_retire)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_retire","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_retire returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_retire returned content"
else
    fail "tools/call run_retire did not return expected response"
