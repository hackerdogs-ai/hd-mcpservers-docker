#!/usr/bin/env python3
"""Retire.js MCP Server — JavaScript library vulnerability scanner for known CVEs.

Wraps the retire CLI (RetireJS/retire.js) to expose
...
logger = logging.getLogger("retire-js-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8364"))
...
BIN_NAME = os.environ.get("RETIRE_BIN", "retire")


def _find_binary() -> str:
    """Locate the retire binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("retire binary not found on PATH")
        raise FileNotFoundError(
            f"retire binary not found. Ensure it is installed and available "
            f"on PATH, or set RETIRE_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a retire command and return structured output.
...
@mcp.tool()
async def run_retire(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run retire with the given arguments.
    ...
    logger.info("run_retire called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("retire command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"retire failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"retire {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting retire-js-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
