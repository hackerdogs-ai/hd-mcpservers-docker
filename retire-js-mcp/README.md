...
MCP server wrapper for [Retire.js](https://github.com/RetireJS/retire.js) — JavaScript library vulnerability scanner for known CVEs.

## What is Retire.js?

Retire.js (retire) is a security tool that provides: **JavaScript library vulnerability scanner for known CVEs.**

See [RetireJS/retire.js](https://github.com/RetireJS/retire.js) for full documentation.
...
Run retire with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "retire output will appear here"
}
```
...
- "Run retire with --help to see all available options."
- "Use retire to scan the target 192.168.1.1."
- "What options does retire support? Show me its help output."
...
```bash
docker run -i --rm hackerdogs/retire-js-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8364:8364 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8364 \
  hackerdogs/retire-js-mcp:latest
```
...
```json
{
  "mcpServers": {
    "retire-js-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/retire-js-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "retire-js-mcp": {
      "url": "http://localhost:8485/retire-js-mcp/mcp",
      "headers": {
...
```bash
docker build -t hackerdogs/retire-js-mcp:latest .
```
...
```bash
docker run -d --rm --name retire-js-mcp-test -p 8364:8364 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/retire-js-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8364/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8364/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8364/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_retire","arguments":{"arguments":"--help"}}}'
```
