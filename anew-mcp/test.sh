#!/bin/bash
# Test script for Anew MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/anew-mcp:latest"
PORT=8229
BINARY="anew"
CONTAINER_NAME="anew-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Anew MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_anew)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_anew","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_anew returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_anew returned content"
else
    fail "tools/call run_anew did not return expected response"
