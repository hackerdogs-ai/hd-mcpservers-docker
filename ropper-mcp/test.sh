#!/bin/bash
# Test script for Ropper MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/ropper-mcp:latest"
PORT=8243
BINARY="ropper"
CONTAINER_NAME="ropper-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Ropper MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_ropper)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_ropper","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_ropper returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_ropper returned content"
else
    fail "tools/call run_ropper did not return expected response"
