...
MCP server wrapper for [Ropper](https://github.com/sashs/Ropper) — ROP gadget finder and exploit dev tool.

## What is Ropper?

Ropper (ropper) is a security tool that provides: **ROP gadget finder and exploit dev tool.**

See [sashs/Ropper](https://github.com/sashs/Ropper) for full documentation.

**No API keys required** — Ropper runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Ropper](https://github.com/sashs/Ropper) — ROP gadget finder and exploit dev tool.

**Tools:**
- `run_ropper` — Run ropper with the given arguments. Returns structured JSON output.
...
Run ropper with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "ropper output will appear here"
}
```
...
- "Run ropper with --help to see all available options."
- "Use ropper to scan the target 192.168.1.1."
- "What options does ropper support? Show me its help output."
- "Run ropper against example.com with default settings."
- "Execute ropper with verbose output enabled."
- "Use the ropper tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/ropper-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8243:8243 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8243 \
  hackerdogs/ropper-mcp:latest
```
...
```json
{
  "mcpServers": {
    "ropper-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/ropper-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "ropper-mcp": {
      "url": "http://localhost:8485/ropper-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8243` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/ropper-mcp:latest .
```
...
```bash
docker run -d --rm --name ropper-mcp-test -p 8243:8243 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/ropper-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8243/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8243/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8243/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_ropper","arguments":{"arguments":"--help"}}}'
```
