...
PASS=0; FAIL=0
IMAGE="hackerdogs/builtwith-mcp:latest"
PORT=8375
CONTAINER_NAME="builtwith-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"domain_lookup","arguments":{"domain":"example.com"}}}'

echo "========== builtwith-mcp test (compliance) =========="
...
info "[3] Stdio tools/call domain_lookup"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 4 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'result\|content\|error' && pass "stdio tools/call domain_lookup" || fail "stdio tools/call domain_lookup"
...
info "[5] HTTP streamable tools/call domain_lookup"
...
echo "$CALL_HTTP" | grep -q 'result\|content\|error' && pass "HTTP tools/call domain_lookup" || fail "HTTP tools/call domain_lookup"
