#!/usr/bin/env python3
"""Libc-Database MCP Server — Libc identification and offset lookup (use find script).

Wraps the find CLI (niklasb/libc-database) to expose
...
logger = logging.getLogger("libc-database-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8277"))

mcp = FastMCP(
    "Libc-Database MCP Server",
    instructions=(
        "Libc identification and offset lookup (use find script)."
    ),
)

LIBC_DATABASE_BIN = os.environ.get("LIBC_DATABASE_BIN", "find")


def _find_binary() -> str:
    """Locate the find binary, raising a clear error if missing."""
    path = shutil.which(LIBC_DATABASE_BIN)
    if path is None:
        logger.error("find binary not found on PATH")
        raise FileNotFoundError(
            f"find binary not found. Ensure it is installed and available "
            f"on PATH, or set LIBC_DATABASE_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a find command and return structured output.
...
@mcp.tool()
async def run_libc_database(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run find with the given arguments.
    ...
    logger.info("run_libc_database called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("find command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"find failed (exit code {result['return_code']})",
...
def main():
    logger.info("Starting libc-database-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
