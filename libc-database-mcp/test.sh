#!/bin/bash
# Test script for Libc-Database MCP Server
...
echo "================================================================================="
echo -e "${BLUE}Libc-Database MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_libc_database)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_libc_database","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_libc_database returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_libc_database returned content"
else
    fail "tools/call run_libc_database did not return expected response"
