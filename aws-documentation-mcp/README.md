...
```bash
docker run -i --rm hackerdogs/aws-documentation-mcp:latest
```
...
docker run -d -p 8610:8610 \
  ...
  -e MCP_PORT=8610 \
  hackerdogs/aws-documentation-mcp:latest
```
...
| `MCP_PORT` | `8610` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/aws-documentation-mcp:latest .
```
...
```bash
docker run -d --rm --name aws-documentation-mcp-test -p 8610:8610 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/aws-documentation-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8610/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8610/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
