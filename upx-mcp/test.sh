#!/bin/bash
# Test script for UPX MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/upx-mcp:latest"
PORT=8284
BINARY="upx"
CONTAINER_NAME="upx-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}UPX MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_upx)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_upx","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_upx returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_upx returned content"
else
    fail "tools/call run_upx did not return expected response"
