#!/usr/bin/env python3
"""UPX MCP Server — Executable packer/unpacker.

Wraps the upx CLI (upx/upx) to expose
...
logger = logging.getLogger("upx-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8284"))
...
UPX_BIN = os.environ.get("UPX_BIN", "upx")


def _find_binary() -> str:
    """Locate the upx binary, raising a clear error if missing."""
    path = shutil.which(UPX_BIN)
    if path is None:
        logger.error("upx binary not found on PATH")
        raise FileNotFoundError(
            f"upx binary not found. Ensure it is installed and available "
            f"on PATH, or set UPX_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a upx command and return structured output.
...
async def run_upx(
    ...
    """Run upx with the given arguments.
    ...
    logger.info("run_upx called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("upx command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"upx failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"upx {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting upx-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
