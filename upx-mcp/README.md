...
MCP server wrapper for [Upx](https://github.com/upx/upx) — Executable packer/unpacker.

## What is Upx?

Upx (upx) is a security tool that provides: **Executable packer/unpacker.**

See [upx/upx](https://github.com/upx/upx) for full documentation.

**No API keys required** — Upx runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Upx](https://github.com/upx/upx) — Executable packer/unpacker.

**Tools:**
- `run_upx` — Run upx with the given arguments. Returns structured JSON output.
...
Run upx with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "upx output will appear here"
}
```
...
- "Run upx with --help to see all available options."
- "Use upx to scan the target 192.168.1.1."
- "What options does upx support? Show me its help output."
- "Run upx against example.com with default settings."
- "Execute upx with verbose output enabled."
- "Use the upx tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/upx-mcp:latest
```
...
```json
{
  "mcpServers": {
    "upx-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/upx-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "upx-mcp": {
      "url": "http://localhost:8485/upx-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8284` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/upx-mcp:latest .
```
...
```bash
docker run -d --rm --name upx-mcp-test -p 8284:8284 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/upx-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8284/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8284/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8284/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_upx","arguments":{"arguments":"--help"}}}'
```
