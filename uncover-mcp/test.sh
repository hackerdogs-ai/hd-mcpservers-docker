#!/bin/bash
# Test script for uncover MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/uncover-mcp:latest"
PORT=8107
BINARY="uncover"
CONTAINER_NAME="uncover-mcp-test"
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo -e "${YELLOW}NOTE: Uncover requires API keys for each search engine (except shodan-idb).${NC}"
echo -e "${YELLOW}   Test 6 uses shodan-idb which does not require an API key.${NC}"
echo -e "${YELLOW}   See https://github.com/projectdiscovery/uncover#provider-configuration${NC}"
echo ""
...
echo "================================================================================="
echo -e "${BLUE}uncover MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (search_hosts)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"search_hosts","arguments":{"query":"nginx","engines":"shodan-idb","limit":5}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call search_hosts returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call search_hosts returned content"
else
    fail "tools/call search_hosts did not return expected response"
