#!/bin/bash
# Test script for Binwalk MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/binwalk-mcp:latest"
PORT=8241
BINARY="binwalk"
CONTAINER_NAME="binwalk-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Binwalk MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_binwalk)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_binwalk","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_binwalk returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_binwalk returned content"
else
    fail "tools/call run_binwalk did not return expected response"
