...
IMAGE="firecrawl-mcp"
PORT=8640
CONTAINER_NAME="firecrawl-mcp-test"
...
echo "========== firecrawl-mcp test (compliance) =========="
info "[1] Install"
...
while [ "$WAITED" -lt "${MCP_HTTP_LIST_MAX_WAIT:-30}" ]; do
  curl -s --max-time 5 -D "$MCP_HDR" -o /dev/null -X POST "http://localhost:${PORT}/mcp" -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" -d "$INIT_REQ" 2>/dev/null || true
