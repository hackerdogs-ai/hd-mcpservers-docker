#!/usr/bin/env python3
"""
Test suite for Sherlock MCP Server.
Tests the Sherlock username search tool.
"""
...
def main():
    ...
    if "result" in response and response["result"].get("serverInfo", {}).get("name") == "sherlock-mcp-server":
    ...
    if "result" in response and "tools" in response["result"]:
        ...
        if len(tools) > 0 and tools[0].get("name") == "sherlock_username_search":
    ...
    # Test 3: Sherlock username search
    tests_total += 1
    if test_tool(
        docker_cmd,
