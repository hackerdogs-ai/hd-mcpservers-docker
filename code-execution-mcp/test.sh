...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_python","arguments":{"code":"print(2+2)","timeout_seconds":5}}}'

echo "========== code-execution-mcp test (compliance) =========="
...
# 3. Stdio — call tool (keep stdin open)
info "[3] Stdio tools/call run_python"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 4 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'stdout' && echo "$CALL_OUT" | grep -q 'exit_code' && echo "$CALL_OUT" | grep -q '4' && pass "stdio tools/call run_python" || fail "stdio tools/call run_python"

# 4 & 5. HTTP streamable — load tools, then call tool
...
info "[5] HTTP streamable tools/call run_python"
...
echo "$CALL_HTTP" | grep -q 'result\|content' && echo "$CALL_HTTP" | grep -q '4' && pass "HTTP tools/call run_python" || fail "HTTP tools/call run_python"
