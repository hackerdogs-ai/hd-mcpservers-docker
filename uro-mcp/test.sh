#!/bin/bash
# Test script for Uro MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/uro-mcp:latest"
PORT=8228
BINARY="uro"
CONTAINER_NAME="uro-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Uro MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_uro)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_uro","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_uro returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_uro returned content"
else
    fail "tools/call run_uro did not return expected response"
