#!/bin/bash
# Test script for Ngrep MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/ngrep-mcp:latest"
PORT=8314
BINARY="ngrep"
CONTAINER_NAME="ngrep-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Ngrep MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_ngrep)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_ngrep","arguments":{"arguments":"-h"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_ngrep returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_ngrep returned content"
else
    fail "tools/call run_ngrep did not return expected response"
