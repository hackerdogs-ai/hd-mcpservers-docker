#!/usr/bin/env python3
"""Kubescape MCP Server — Kubernetes security posture management and compliance scanning.

Wraps the kubescape CLI (kubescape/kubescape) to expose
...
logger = logging.getLogger("kubescape-mcp")
...
BIN_NAME = os.environ.get("KUBESCAPE_BIN", "kubescape")


def _find_binary() -> str:
    """Locate the kubescape binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("kubescape binary not found on PATH")
        raise FileNotFoundError(
            f"kubescape binary not found. Ensure it is installed and available "
            f"on PATH, or set KUBESCAPE_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a kubescape command and return structured output.
...
async def run_kubescape(
    ...
    """Run kubescape with the given arguments.
    ...
    logger.info("run_kubescape called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("kubescape command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"kubescape failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"kubescape {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting kubescape-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
