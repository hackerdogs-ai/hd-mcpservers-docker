#!/bin/bash
# Test script for Kubescape MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/kubescape-mcp:latest"
PORT=8362
BINARY="kubescape"
CONTAINER_NAME="kubescape-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Kubescape MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_kubescape)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_kubescape","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_kubescape returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_kubescape returned content"
else
    fail "tools/call run_kubescape did not return expected response"
