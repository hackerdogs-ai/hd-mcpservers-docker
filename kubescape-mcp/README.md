...
MCP server wrapper for [Kubescape](https://github.com/kubescape/kubescape) — Kubernetes security posture management and compliance scanning.

## What is Kubescape?

Kubescape (kubescape) is a security tool that provides: **Kubernetes security posture management and compliance scanning.**

See [kubescape/kubescape](https://github.com/kubescape/kubescape) for full documentation.

**No API keys required** — Kubescape runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Kubescape](https://github.com/kubescape/kubescape) — Kubernetes security posture management and compliance scanning.

**Tools:**
- `run_kubescape` — Run kubescape with the given arguments. Returns structured JSON output.
...
Run kubescape with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "kubescape output will appear here"
}
```
...
- "Run kubescape with --help to see all available options."
- "Use kubescape to scan the target 192.168.1.1."
- "What options does kubescape support? Show me its help output."
- "Run kubescape against example.com with default settings."
- "Execute kubescape with verbose output enabled."
- "Use the kubescape tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/kubescape-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8362:8362 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8362 \
  hackerdogs/kubescape-mcp:latest
```
...
```json
{
  "mcpServers": {
    "kubescape-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/kubescape-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "kubescape-mcp": {
      "url": "http://localhost:8485/kubescape-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8362` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/kubescape-mcp:latest .
```
...
```bash
docker run -d --rm --name kubescape-mcp-test -p 8362:8362 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/kubescape-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8362/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8362/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8362/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_kubescape","arguments":{"arguments":"--help"}}}'
```
