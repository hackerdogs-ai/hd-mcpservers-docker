#!/bin/bash
# Test script for YaraFlux MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/yaraflux-mcp:latest"
PORT=8336
BINARY="yaraflux"
CONTAINER_NAME="yaraflux-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}YaraFlux MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_yaraflux)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_yaraflux","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_yaraflux returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_yaraflux returned content"
else
    fail "tools/call run_yaraflux did not return expected response"
