#!/usr/bin/env python3
"""YaraFlux MCP Server — YARA-based MCP server for malware scanning and rule management.

Wraps the yaraflux CLI (ThreatFlux/YaraFlux) to expose
...
logger = logging.getLogger("yaraflux-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8336"))
...
BIN_NAME = os.environ.get("YARAFLUX_BIN", "yaraflux")


def _find_binary() -> str:
    """Locate the yaraflux binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("yaraflux binary not found on PATH")
        raise FileNotFoundError(
            f"yaraflux binary not found. Ensure it is installed and available "
            f"on PATH, or set YARAFLUX_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a yaraflux command and return structured output.
...
@mcp.tool()
async def run_yaraflux(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run yaraflux with the given arguments.
    ...
    logger.info("run_yaraflux called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("yaraflux command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"yaraflux failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"yaraflux {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting yaraflux-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
