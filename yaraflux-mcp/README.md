...
MCP server wrapper for [YaraFlux](https://github.com/ThreatFlux/YaraFlux) — YARA-based MCP server for malware scanning and rule management.

## What is YaraFlux?

YaraFlux (yaraflux) is a security tool that provides: **YARA-based MCP server for malware scanning and rule management.**

See [ThreatFlux/YaraFlux](https://github.com/ThreatFlux/YaraFlux) for full documentation.

**No API keys required** — YaraFlux runs locally inside the Docker container.

**Summary.** MCP server wrapper for [YaraFlux](https://github.com/ThreatFlux/YaraFlux) — YARA-based MCP server for malware scanning and rule management.

**Tools:**
- `run_yaraflux` — Run yaraflux with the given arguments. Returns structured JSON output.
...
Run yaraflux with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "yaraflux output will appear here"
}
```
...
- "Run yaraflux with --help to see all available options."
- "Use yaraflux to scan the target 192.168.1.1."
- "What options does yaraflux support? Show me its help output."
- "Run yaraflux against example.com with default settings."
- "Execute yaraflux with verbose output enabled."
- "Use the yaraflux tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/yaraflux-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8336:8336 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8336 \
  hackerdogs/yaraflux-mcp:latest
```
...
```json
{
  "mcpServers": {
    "yaraflux-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/yaraflux-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "yaraflux-mcp": {
      "url": "http://localhost:8485/yaraflux-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8336` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/yaraflux-mcp:latest .
```
...
```bash
docker run -d --rm --name yaraflux-mcp-test -p 8336:8336 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/yaraflux-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8336/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8336/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8336/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_yaraflux","arguments":{"arguments":"--help"}}}'
```
