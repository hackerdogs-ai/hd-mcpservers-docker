...
MCP server wrapper for [Syft](https://github.com/anchore/syft) — Software bill of materials (SBOM) generator for container images.

## What is Syft?

Syft (syft) is a security tool that provides: **Software bill of materials (SBOM) generator for container images.**

See [anchore/syft](https://github.com/anchore/syft) for full documentation.

**No API keys required** — Syft runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Syft](https://github.com/anchore/syft) — Software bill of materials (SBOM) generator for container images.

**Tools:**
- `run_syft` — Run syft with the given arguments. Returns structured JSON output.
...
Run syft with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "syft output will appear here"
}
```
...
- "Run syft with --help to see all available options."
- "Use syft to scan the target 192.168.1.1."
- "What options does syft support? Show me its help output."
- "Run syft against example.com with default settings."
- "Execute syft with verbose output enabled."
- "Use the syft tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/syft-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8358:8358 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8358 \
  hackerdogs/syft-mcp:latest
```
...
```json
{
  "mcpServers": {
    "syft-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/syft-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "syft-mcp": {
      "url": "http://localhost:8485/syft-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8358` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/syft-mcp:latest .
```
...
```bash
docker run -d --rm --name syft-mcp-test -p 8358:8358 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/syft-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8358/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8358/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8358/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_syft","arguments":{"arguments":"--help"}}}'
```
