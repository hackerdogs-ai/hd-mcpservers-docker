#!/bin/bash
# Test script for Syft MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/syft-mcp:latest"
PORT=8358
BINARY="syft"
CONTAINER_NAME="syft-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Syft MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_syft)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_syft","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_syft returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_syft returned content"
else
    fail "tools/call run_syft did not return expected response"
