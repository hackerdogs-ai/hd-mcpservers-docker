#!/usr/bin/env python3
"""Syft MCP Server — Software bill of materials (SBOM) generator for container images.

Wraps the syft CLI (anchore/syft) to expose
...
logger = logging.getLogger("syft-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8358"))
...
BIN_NAME = os.environ.get("SYFT_BIN", "syft")


def _find_binary() -> str:
    """Locate the syft binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("syft binary not found on PATH")
        raise FileNotFoundError(
            f"syft binary not found. Ensure it is installed and available "
            f"on PATH, or set SYFT_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a syft command and return structured output.
...
async def run_syft(
    ...
    """Run syft with the given arguments.
    ...
    logger.info("run_syft called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("syft command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"syft failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"syft {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting syft-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
