#!/bin/bash
# MCP server test compliance: 1=install, 2=stdio load tools, 3=stdio call tool(s), 4=HTTP load tools, 5=HTTP call tool(s)
# See .cursor/rules/mcp-server-test-compliance.mdc
set -euo pipefail
RED='\033[0;31m'; GREEN='\033[0;32m'; BLUE='\033[0;34m'; NC='\033[0m'
PASS=0; FAIL=0
IMAGE="hackerdogs/deepwebresearch-mcp:latest"
PORT=8377
CONTAINER_NAME="deepwebresearch-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"fetch_url","arguments":{"url":"https://example.com","max_chars":2000}}}'

echo "========== deepwebresearch-mcp test (compliance) =========="
...
info "[3] Stdio tools/call fetch_url"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 6 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'result\|content' && echo "$CALL_OUT" | grep -q 'status_code\|text' && pass "stdio tools/call fetch_url" || fail "stdio tools/call fetch_url"
...
info "[5] HTTP streamable tools/call fetch_url"
...
echo "$CALL_HTTP" | grep -q 'result\|content' && pass "HTTP tools/call fetch_url" || fail "HTTP tools/call fetch_url"
