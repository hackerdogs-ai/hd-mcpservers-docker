#!/bin/bash
# Build and Publish Crawl4AI MCP Server Docker Image to Docker Hub
# Image name: crawl4ai-mcp
...
if [ "$SHOW_HELP" = true ]; then
    echo "Build and Publish Crawl4AI MCP Server Docker Image to Docker Hub"
    echo ""
    echo "Usage:"
...
if [ "$DO_BUILD" = true ] && [ "$DO_PUBLISH" = true ]; then
    echo -e "${BLUE}Building and Publishing Crawl4AI MCP Server Docker Image${NC}"
elif [ "$DO_BUILD" = true ]; then
    echo -e "${BLUE}Building Crawl4AI MCP Server Docker Image${NC}"
else
    echo -e "${BLUE}Publishing Crawl4AI MCP Server Docker Image to Docker Hub${NC}"
fi
echo "================================================================================="
if [ "$DO_PUBLISH" = true ]; then
...
if [ "$DO_BUILD" = true ]; then
    ...
    if [ "$DO_PUBLISH" = true ]; then
        ...
        for tag in "${TAGS[@]}"; do
            if [ "$PLATFORMS_MODE" = "sequential" ]; then
                ...
                if ! do_build_push_with_retry docker buildx build \
                    ...
                    -f "$DOCKERFILE" -t "${FULL_IMAGE_NAME}:${tag}-amd64" --push . ; then
                ...
                if ! do_build_push_with_retry docker buildx build \
                    ...
                    -f "$DOCKERFILE" -t "${FULL_IMAGE_NAME}:${tag}-arm64" --push . ; then
                ...
                if [ $? -ne 0 ]; then
                    echo -e "${RED}Error: Failed to create manifest for ${FULL_IMAGE_NAME}:${tag}${NC}"; exit 1
                fi
            else
                echo "Building and pushing ${FULL_IMAGE_NAME}:${tag}..."
                if ! do_build_push_with_retry docker buildx build \
                    --platform linux/amd64,linux/arm64 --provenance=false --sbom=false \
                    -f "$DOCKERFILE" -t "${FULL_IMAGE_NAME}:${tag}" --push . ; then
        ...
        if [ $? -eq 0 ]; then echo -e "${GREEN}Tagged as ${REGISTRY_TAG} (for docker-compose)${NC}"
        else echo -e "${YELLOW}Warning: Failed to tag image for docker-compose${NC}"; fi
        if [ ${#TAGS[@]} -gt 1 ]; then
