...
MCP server wrapper for [Cutter](https://github.com/rizinorg/cutter) — Reverse engineering platform powered by Rizin.

## What is Cutter?

Cutter (cutter) is a security tool that provides: **Reverse engineering platform powered by Rizin.**

See [rizinorg/cutter](https://github.com/rizinorg/cutter) for full documentation.

**No API keys required** — Cutter runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Cutter](https://github.com/rizinorg/cutter) — Reverse engineering platform powered by Rizin.

**Tools:**
- `run_cutter` — Run cutter with the given arguments. Returns structured JSON output.
...
Run cutter with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "cutter output will appear here"
}
```
...
- "Run cutter with --help to see all available options."
- "Use cutter to scan the target 192.168.1.1."
- "What options does cutter support? Show me its help output."
- "Run cutter against example.com with default settings."
- "Execute cutter with verbose output enabled."
- "Use the cutter tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/cutter-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8320:8320 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8320 \
  hackerdogs/cutter-mcp:latest
```
...
```json
{
  "mcpServers": {
    "cutter-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/cutter-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "cutter-mcp": {
      "url": "http://localhost:8485/cutter-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8320` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/cutter-mcp:latest .
```
...
```bash
docker run -d --rm --name cutter-mcp-test -p 8320:8320 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/cutter-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8320/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8320/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8320/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_cutter","arguments":{"arguments":"--help"}}}'
```
