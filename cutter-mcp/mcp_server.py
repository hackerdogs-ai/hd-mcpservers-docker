#!/usr/bin/env python3
"""Cutter MCP Server — Reverse engineering platform powered by Rizin.

Wraps the cutter CLI (rizinorg/cutter) to expose
...
logger = logging.getLogger("cutter-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8320"))
...
BIN_NAME = os.environ.get("CUTTER_BIN", "cutter")


def _find_binary() -> str:
    """Locate the cutter binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("cutter binary not found on PATH")
        raise FileNotFoundError(
            f"cutter binary not found. Ensure it is installed and available "
            f"on PATH, or set CUTTER_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a cutter command and return structured output.
...
async def run_cutter(
    ...
    """Run cutter with the given arguments.
    ...
    logger.info("run_cutter called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("cutter command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"cutter failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"cutter {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting cutter-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
