#!/bin/bash
# Test script for Cutter MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/cutter-mcp:latest"
PORT=8320
BINARY="cutter"
CONTAINER_NAME="cutter-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Cutter MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
BINARY_OUTPUT=$(docker run --rm "$IMAGE" $BINARY -v 2>&1 | head -5 || docker run --rm "$IMAGE" $BINARY --version 2>&1 | head -5 || docker run --rm "$IMAGE" $BINARY -h 2>&1 | head -5 || true)
...
info "[Test 6] MCP HTTP — tools/call (run_cutter)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_cutter","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_cutter returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_cutter returned content"
else
    fail "tools/call run_cutter did not return expected response"
