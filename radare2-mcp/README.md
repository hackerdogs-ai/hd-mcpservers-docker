...
MCP server wrapper for [Radare2](https://github.com/radareorg/radare2) — Reverse engineering framework.

## What is Radare2?

Radare2 (r2) is a security tool that provides: **Reverse engineering framework.**

See [radareorg/radare2](https://github.com/radareorg/radare2) for full documentation.

**No API keys required** — Radare2 runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Radare2](https://github.com/radareorg/radare2) — Reverse engineering framework.

**Tools:**
- `run_radare2` — Run r2 with the given arguments. Returns structured JSON output.
...
Run r2 with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "r2 output will appear here"
}
```
...
- "Run r2 with --help to see all available options."
- "Use radare2 to scan the target 192.168.1.1."
- "What options does r2 support? Show me its help output."
- "Run radare2 against example.com with default settings."
- "Execute r2 with verbose output enabled."
- "Use the radare2 tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/radare2-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8239:8239 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8239 \
  hackerdogs/radare2-mcp:latest
```
...
```json
{
  "mcpServers": {
    "radare2-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/radare2-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "radare2-mcp": {
      "url": "http://localhost:8485/radare2-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8239` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/radare2-mcp:latest .
```
...
```bash
docker run -d --rm --name radare2-mcp-test -p 8239:8239 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/radare2-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8239/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8239/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8239/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_radare2","arguments":{"arguments":"--help"}}}'
```
