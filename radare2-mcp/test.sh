#!/bin/bash
# Test script for Radare2 MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/radare2-mcp:latest"
PORT=8239
BINARY="r2"
CONTAINER_NAME="radare2-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Radare2 MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_radare2)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_radare2","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_radare2 returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_radare2 returned content"
else
    fail "tools/call run_radare2 did not return expected response"
