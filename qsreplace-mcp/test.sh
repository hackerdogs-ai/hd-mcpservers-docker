#!/bin/bash
# Test script for QSReplace MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/qsreplace-mcp:latest"
PORT=8227
BINARY="qsreplace"
CONTAINER_NAME="qsreplace-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}QSReplace MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_qsreplace)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_qsreplace","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_qsreplace returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_qsreplace returned content"
else
    fail "tools/call run_qsreplace did not return expected response"
