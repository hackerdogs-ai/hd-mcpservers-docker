#!/bin/bash
# Test script for Netdiscover MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/netdiscover-mcp:latest"
PORT=8322
BINARY="netdiscover"
CONTAINER_NAME="netdiscover-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Netdiscover MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_netdiscover)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_netdiscover","arguments":{"arguments":"-h"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_netdiscover returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_netdiscover returned content"
else
    fail "tools/call run_netdiscover did not return expected response"
