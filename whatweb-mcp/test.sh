#!/bin/bash
# Test script for WhatWeb MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/whatweb-mcp:latest"
PORT=8281
BINARY="whatweb"
CONTAINER_NAME="whatweb-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}WhatWeb MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_whatweb)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_whatweb","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_whatweb returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_whatweb returned content"
else
    fail "tools/call run_whatweb did not return expected response"
