#!/bin/bash
# Test script for Wfuzz MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/wfuzz-mcp:latest"
PORT=8224
BINARY="wfuzz"
CONTAINER_NAME="wfuzz-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Wfuzz MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_wfuzz)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_wfuzz","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_wfuzz returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_wfuzz returned content"
else
    fail "tools/call run_wfuzz did not return expected response"
