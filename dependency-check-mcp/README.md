...
MCP server wrapper for [Dependency-Check](https://github.com/jeremylong/DependencyCheck) — Software composition analysis for known vulnerabilities in dependencies.

## What is Dependency-Check?

Dependency-Check (dependency-check) is a security tool that provides: **Software composition analysis for known vulnerabilities in dependencies.**

See [jeremylong/DependencyCheck](https://github.com/jeremylong/DependencyCheck) for full documentation.

**No API keys required** — Dependency-Check runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Dependency-Check](https://github.com/jeremylong/DependencyCheck) — Software composition analysis for known vulnerabilities in dependencies.

**Tools:**
- `run_dependency_check` — Run dependency-check with the given arguments. Returns structured JSON output.
...
Run dependency-check with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "dependency-check output will appear here"
}
```
...
- "Run dependency-check with --help to see all available options."
- "Use dependency-check to scan the target 192.168.1.1."
- "What options does dependency-check support? Show me its help output."
- "Run dependency-check against example.com with default settings."
- "Execute dependency-check with verbose output enabled."
- "Use the dependency-check tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/dependency-check-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8361:8361 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8361 \
  hackerdogs/dependency-check-mcp:latest
```
...
```json
{
  "mcpServers": {
    "dependency-check-mcp": {
      "url": "http://localhost:8485/dependency-check-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8361` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/dependency-check-mcp:latest .
```
...
```bash
docker run -d --rm --name dependency-check-mcp-test -p 8361:8361 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/dependency-check-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8361/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8361/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8361/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_dependency_check","arguments":{"arguments":"--help"}}}'
```
