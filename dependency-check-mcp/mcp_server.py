#!/usr/bin/env python3
"""Dependency-Check MCP Server — Software composition analysis for known vulnerabilities in dependencies.

Wraps the dependency-check CLI (jeremylong/DependencyCheck) to expose
...
logger = logging.getLogger("dependency-check-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8361"))
...
BIN_NAME = os.environ.get("DEPENDENCY_CHECK_BIN", "dependency-check")


def _find_binary() -> str:
    """Locate the dependency-check binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("dependency-check binary not found on PATH")
        raise FileNotFoundError(
            f"dependency-check binary not found. Ensure it is installed and available "
            f"on PATH, or set DEPENDENCY_CHECK_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a dependency-check command and return structured output.
...
async def run_dependency_check(
    ...
    """Run dependency-check with the given arguments.
    ...
    logger.info("run_dependency_check called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("dependency-check command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"dependency-check failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"dependency-check {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting dependency-check-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
