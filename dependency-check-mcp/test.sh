#!/bin/bash
# Test script for Dependency-Check MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/dependency-check-mcp:latest"
PORT=8361
BINARY="dependency-check"
CONTAINER_NAME="dependency-check-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Dependency-Check MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_dependency_check)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_dependency_check","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_dependency_check returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_dependency_check returned content"
else
    fail "tools/call run_dependency_check did not return expected response"
