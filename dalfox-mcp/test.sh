#!/bin/bash
# Test script for Dalfox MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/dalfox-mcp:latest"
PORT=8221
BINARY="dalfox"
CONTAINER_NAME="dalfox-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Dalfox MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_dalfox)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_dalfox","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_dalfox returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_dalfox returned content"
else
    fail "tools/call run_dalfox did not return expected response"
