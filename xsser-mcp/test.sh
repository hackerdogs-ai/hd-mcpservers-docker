#!/bin/bash
# Test script for XSSer MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/xsser-mcp:latest"
PORT=8222
BINARY="xsser"
CONTAINER_NAME="xsser-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}XSSer MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_xsser)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_xsser","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_xsser returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_xsser returned content"
else
    fail "tools/call run_xsser did not return expected response"
