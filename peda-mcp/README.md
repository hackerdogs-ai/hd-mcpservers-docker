...
MCP server wrapper for [Peda](https://github.com/longld/peda) — GNU Debugger with PEDA (exploit development).

## What is Peda?

Peda (gdb) is a security tool that provides: **GNU Debugger with PEDA (exploit development).**

See [longld/peda](https://github.com/longld/peda) for full documentation.

**No API keys required** — Peda runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Peda](https://github.com/longld/peda) — GNU Debugger with PEDA (exploit development).

**Tools:**
- `run_peda` — Run gdb with the given arguments. Returns structured JSON output.
...
- "Use peda to scan the target 192.168.1.1."
- "What options does gdb support? Show me its help output."
- "Run peda against example.com with default settings."
- "Execute gdb with verbose output enabled."
- "Use the peda tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/peda-mcp:latest
```
...
```json
{
  "mcpServers": {
    "peda-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/peda-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
| `MCP_PORT` | `8237` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/peda-mcp:latest .
```
...
```bash
docker run -d --rm --name peda-mcp-test -p 8237:8237 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/peda-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8237/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8237/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8237/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_peda","arguments":{"arguments":"--help"}}}'
```
...
```json
{
  "mcpServers": {
    "peda-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "hackerdogs/peda-mcp:latest"],
      "env": {}
    }
  }
}
...
```bash
docker run -d -p 8237:8237 -e MCP_TRANSPORT=streamable-http hackerdogs/peda-mcp:latest
```
