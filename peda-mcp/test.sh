#!/bin/bash
# Test script for GDB-PEDA MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/peda-mcp:latest"
PORT=8237
BINARY="gdb"
CONTAINER_NAME="peda-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}GDB-PEDA MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_peda)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_peda","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_peda returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_peda returned content"
else
    fail "tools/call run_peda did not return expected response"
