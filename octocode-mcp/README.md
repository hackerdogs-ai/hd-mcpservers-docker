...
MCP server wrapper for [OctoCode](https://github.com/nicholasgasior/octocode-mcp) — upstream package `octocode-mcp`.
...
```bash
docker run -i --rm hackerdogs/octocode-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8654:8654 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8654 \
  hackerdogs/octocode-mcp:latest
```
...
| `MCP_PORT` | `8654` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/octocode-mcp:latest .
```
...
```bash
docker run -d --rm --name octocode-mcp-test -p 8654:8654 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/octocode-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8654/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8654/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
