#!/bin/bash
# Test script for dnsReaper MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/dnsreaper-mcp:latest"
PORT=8293
BINARY="dnsreaper"
CONTAINER_NAME="dnsreaper-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}dnsReaper MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_dnsreaper)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_dnsreaper","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_dnsreaper returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_dnsreaper returned content"
else
    fail "tools/call run_dnsreaper did not return expected response"
