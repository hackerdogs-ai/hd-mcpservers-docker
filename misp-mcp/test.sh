#!/bin/bash
# Test script for MISP MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/misp-mcp:latest"
PORT=8371
CONTAINER_NAME="misp-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}MISP MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
# Test 2: Python requests module available
info "[Test 2] Python requests module inside container"
IMPORT_OUTPUT=$(docker run --rm "$IMAGE" python -c "import requests; print(f'requests {requests.__version__}')" 2>&1 || true)
if echo "$IMPORT_OUTPUT" | grep -q "requests"; then
    pass "requests module available"
    echo "       ${IMPORT_OUTPUT%%$'\n'*}"
else
    fail "requests module not found"
fi
echo ""
...
# Test 6: MCP HTTP — tools/call (misp_search_attributes — will return config error)
info "[Test 6] MCP HTTP — tools/call (misp_search_attributes)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"misp_search_attributes","arguments":{"value":"example.com"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call misp_search_attributes returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call misp_search_attributes returned content"
else
    fail "tools/call misp_search_attributes did not return expected response"
