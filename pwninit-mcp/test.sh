#!/bin/bash
# Test script for Pwninit MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/pwninit-mcp:latest"
PORT=8278
BINARY="pwninit"
CONTAINER_NAME="pwninit-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Pwninit MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_pwninit)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_pwninit","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_pwninit returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_pwninit returned content"
else
    fail "tools/call run_pwninit did not return expected response"
