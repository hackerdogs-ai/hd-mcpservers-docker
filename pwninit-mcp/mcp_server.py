#!/usr/bin/env python3
"""Pwninit MCP Server — Automate binary exploitation setup.

Wraps the pwninit CLI (icecream94/pwninit) to expose
...
logger = logging.getLogger("pwninit-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8278"))

mcp = FastMCP(
    "Pwninit MCP Server",
    instructions=(
        "Automate binary exploitation setup."
    ),
)

PWNINIT_BIN = os.environ.get("PWNINIT_BIN", "pwninit")


def _find_binary() -> str:
    """Locate the pwninit binary, raising a clear error if missing."""
    path = shutil.which(PWNINIT_BIN)
    if path is None:
        logger.error("pwninit binary not found on PATH")
        raise FileNotFoundError(
            f"pwninit binary not found. Ensure it is installed and available "
            f"on PATH, or set PWNINIT_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a pwninit command and return structured output.
...
@mcp.tool()
async def run_pwninit(
    arguments: str,
    source_url: str = "",
    timeout_seconds: int = 600,
) -> str:
    """Run pwninit with the given arguments.
    ...
    logger.info("run_pwninit called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("pwninit command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"pwninit failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"pwninit {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting pwninit-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
