#!/bin/bash
# Test script for Pacu MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/pacu-mcp:latest"
PORT=8269
BINARY="pacu"
CONTAINER_NAME="pacu-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Pacu MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_pacu)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_pacu","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_pacu returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_pacu returned content"
else
    fail "tools/call run_pacu did not return expected response"
