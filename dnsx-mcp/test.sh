#!/bin/bash
# Test script for dnsx MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/dnsx-mcp:latest"
PORT=8108
BINARY="dnsx"
CONTAINER_NAME="dnsx-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}dnsx MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (resolve_domains)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"resolve_domains","arguments":{"domains":"example.com","record_types":["a"]}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call resolve_domains returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call resolve_domains returned content"
else
    fail "tools/call resolve_domains did not return expected response"
