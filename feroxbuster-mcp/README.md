...
MCP server wrapper for [Feroxbuster](https://github.com/epi052/feroxbuster) — Recursive content discovery with filtering.

## What is Feroxbuster?

Feroxbuster (feroxbuster) is a security tool that provides: **Recursive content discovery with filtering.**

See [epi052/feroxbuster](https://github.com/epi052/feroxbuster) for full documentation.

**No API keys required** — Feroxbuster runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Feroxbuster](https://github.com/epi052/feroxbuster) — Recursive content discovery with filtering.

**Tools:**
- `run_feroxbuster` — Run feroxbuster with the given arguments. Returns structured JSON output.
...
Run feroxbuster with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "feroxbuster output will appear here"
}
```
...
- "Run feroxbuster with --help to see all available options."
- "Use feroxbuster to scan the target 192.168.1.1."
- "What options does feroxbuster support? Show me its help output."
- "Run feroxbuster against example.com with default settings."
- "Execute feroxbuster with verbose output enabled."
- "Use the feroxbuster tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/feroxbuster-mcp:latest
```
...
docker run -d -p 8216:8216 \
  ...
  -e MCP_PORT=8216 \
  hackerdogs/feroxbuster-mcp:latest
```
...
```json
{
  "mcpServers": {
    "feroxbuster-mcp": {
      "url": "http://localhost:8485/feroxbuster-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8216` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/feroxbuster-mcp:latest .
```
...
```bash
docker run -d --rm --name feroxbuster-mcp-test -p 8216:8216 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/feroxbuster-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8216/mcp \
...
curl -s -X POST http://localhost:8216/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_feroxbuster","arguments":{"arguments":"--help"}}}'
```
