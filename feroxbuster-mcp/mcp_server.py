#!/usr/bin/env python3
"""Feroxbuster MCP Server — Recursive content discovery with filtering.

Wraps the feroxbuster CLI (epi052/feroxbuster) to expose
...
logger = logging.getLogger("feroxbuster-mcp")
...
mcp = FastMCP(
    ...
    instructions=(
        "Recursive content discovery with filtering."
    ),
)

FEROXBUSTER_BIN = os.environ.get("FEROXBUSTER_BIN", "feroxbuster")


def _find_binary() -> str:
    """Locate the feroxbuster binary, raising a clear error if missing."""
    path = shutil.which(FEROXBUSTER_BIN)
    if path is None:
        logger.error("feroxbuster binary not found on PATH")
        raise FileNotFoundError(
            f"feroxbuster binary not found. Ensure it is installed and available "
            f"on PATH, or set FEROXBUSTER_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a feroxbuster command and return structured output.
...
async def run_feroxbuster(
    ...
    """Run feroxbuster with the given arguments.
    ...
    logger.info("run_feroxbuster called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("feroxbuster command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"feroxbuster failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"feroxbuster {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting feroxbuster-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
