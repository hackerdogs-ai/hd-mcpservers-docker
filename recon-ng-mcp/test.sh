#!/bin/bash
# Test script for Recon-ng MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/recon-ng-mcp:latest"
PORT=8256
BINARY="recon-ng"
CONTAINER_NAME="recon-ng-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Recon-ng MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_recon_ng)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_recon_ng","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_recon_ng returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_recon_ng returned content"
else
    fail "tools/call run_recon_ng did not return expected response"
