...
PASS=0; FAIL=0
IMAGE="graphviz-dot-mcp"
PORT=8523
CONTAINER_NAME="graphviz-dot-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"render_dot","arguments":{"content":"digraph { A -> B }"}}}'

echo "========== graphviz-dot-mcp test (compliance) =========="
info "[1] Install"
...
info "[3] Stdio tools/call render_dot"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 5 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'result\|content\|error' && pass "stdio tools/call render_dot" || fail "stdio tools/call render_dot"
...
info "[5] HTTP streamable tools/call render_dot"
SESSION_HDR=()
[ -n "$SESSION_ID" ] && SESSION_HDR=(-H "Mcp-Session-Id: ${SESSION_ID}")
CALL_HTTP=$(curl -s -X POST "http://localhost:${PORT}/mcp" \
  -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" \
  "${SESSION_HDR[@]}" -d "$CALL_REQ" 2>/dev/null) || true
echo "$CALL_HTTP" | grep -q 'result\|content\|error' && pass "HTTP tools/call render_dot" || fail "HTTP tools/call render_dot"
