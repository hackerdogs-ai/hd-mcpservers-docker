#!/bin/bash
# Test script for Evil-WinRM MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/evil-winrm-mcp:latest"
PORT=8263
BINARY="evil-winrm"
CONTAINER_NAME="evil-winrm-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Evil-WinRM MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_evil_winrm)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_evil_winrm","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_evil_winrm returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_evil_winrm returned content"
else
    fail "tools/call run_evil_winrm did not return expected response"
