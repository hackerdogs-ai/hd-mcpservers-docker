#!/bin/bash
# Test script for Aquatone MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/aquatone-mcp:latest"
PORT=8259
BINARY="aquatone"
CONTAINER_NAME="aquatone-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Aquatone MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_aquatone)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_aquatone","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_aquatone returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_aquatone returned content"
else
    fail "tools/call run_aquatone did not return expected response"
