...
MCP server wrapper for [Kube Bench](https://github.com/aquasecurity/kube-bench) — CIS Kubernetes benchmark checker.

## What is Kube Bench?

Kube Bench (kube-bench) is a security tool that provides: **CIS Kubernetes benchmark checker.**

See [aquasecurity/kube-bench](https://github.com/aquasecurity/kube-bench) for full documentation.

**No API keys required** — Kube Bench runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Kube Bench](https://github.com/aquasecurity/kube-bench) — CIS Kubernetes benchmark checker.

**Tools:**
- `run_kube_bench` — Run kube-bench with the given arguments. Returns structured JSON output.
...
Run kube-bench with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "kube-bench output will appear here"
}
```
...
- "Run kube-bench with --help to see all available options."
- "Use kube-bench to scan the target 192.168.1.1."
- "What options does kube-bench support? Show me its help output."
- "Run kube-bench against example.com with default settings."
- "Execute kube-bench with verbose output enabled."
- "Use the kube-bench tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/kube-bench-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8253:8253 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8253 \
  hackerdogs/kube-bench-mcp:latest
```
...
```json
{
  "mcpServers": {
    "kube-bench-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/kube-bench-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "kube-bench-mcp": {
      "url": "http://localhost:8485/kube-bench-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8253` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/kube-bench-mcp:latest .
```
...
```bash
docker run -d --rm --name kube-bench-mcp-test -p 8253:8253 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/kube-bench-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8253/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8253/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8253/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_kube_bench","arguments":{"arguments":"--help"}}}'
```
