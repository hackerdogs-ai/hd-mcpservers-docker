#!/bin/bash
# Test script for Kube-Bench MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/kube-bench-mcp:latest"
PORT=8253
BINARY="kube-bench"
CONTAINER_NAME="kube-bench-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Kube-Bench MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_kube_bench)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_kube_bench","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_kube_bench returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_kube_bench returned content"
else
    fail "tools/call run_kube_bench did not return expected response"
