#!/bin/bash
# Test script for Titus MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/titus-mcp:latest"
PORT=8103
BINARY="titus"
CONTAINER_NAME="titus-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Titus MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (list_rules)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"list_rules","arguments":{}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call list_rules returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call list_rules returned content"
else
    fail "tools/call list_rules did not return expected response"
