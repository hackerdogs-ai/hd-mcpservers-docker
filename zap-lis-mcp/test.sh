#!/bin/bash
# Test script for ZAP Lis MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/zap-lis-mcp:latest"
PORT=8355
BINARY="zap-mcp"
CONTAINER_NAME="zap-lis-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}ZAP Lis MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_zap_mcp)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_zap_lis","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_zap_mcp returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_zap_mcp returned content"
else
    fail "tools/call run_zap_mcp did not return expected response"
