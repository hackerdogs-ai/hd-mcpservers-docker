...
MCP server wrapper for [waybackurls](https://github.com/tomnomnom/waybackurls) — fetch all URLs ever crawled by the Wayback Machine for a domain.

## What is Waybackurls?

Waybackurls is a Go tool by Tom Hudson (tomnomnom) that queries the Wayback Machine CDX API to retrieve every URL ever crawled for a given domain, making it useful for OSINT, attack surface discovery, and finding forgotten endpoints, old admin paths, or leaked files. It is commonly used in bug bounty recon pipelines alongside tools like gau and hakrawler. See [tomnomnom/waybackurls](https://github.com/tomnomnom/waybackurls) for full documentation.

**No API keys required** — waybackurls queries the public Wayback Machine API and runs locally inside the Docker container.

**Summary.** MCP server wrapper for [waybackurls](https://github.com/tomnomnom/waybackurls) — fetch all URLs ever crawled by the Wayback Machine for a domain.
...
```json
{
  "mcpServers": {
    "waybackurls-mcp": {
      "url": "http://localhost:8485/waybackurls-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8395` | HTTP port (only used with `streamable-http`) |
