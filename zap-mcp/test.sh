#!/bin/bash
# Test script for OWASP ZAP MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/zap-mcp:latest"
PORT=8231
BINARY="zap.sh"
CONTAINER_NAME="zap-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}OWASP ZAP MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_zap)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_zap","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_zap returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_zap returned content"
else
    fail "tools/call run_zap did not return expected response"
