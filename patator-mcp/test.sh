#!/bin/bash
# Test script for Patator MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/patator-mcp:latest"
PORT=8262
BINARY="patator"
CONTAINER_NAME="patator-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Patator MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_patator)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_patator","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_patator returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_patator returned content"
else
    fail "tools/call run_patator did not return expected response"
