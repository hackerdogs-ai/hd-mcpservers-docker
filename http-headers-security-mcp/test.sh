...
PASS=0; FAIL=0
IMAGE="hackerdogs/http-headers-security-mcp:latest"
PORT=8392
CONTAINER_NAME="http-headers-security-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_http_headers_security","arguments":{"arguments":"-h"}}}'
echo "========== http-headers-security-mcp test (compliance) =========="
info "[1] Install"
...
info "[3] Stdio tools/call run_http_headers_security"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 5 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'result\|content' && pass "stdio tools/call run_http_headers_security" || fail "stdio tools/call run_http_headers_security"
...
info "[5] HTTP streamable tools/call run_http_headers_security"
...
echo "$CALL_HTTP" | grep -q 'result\|content' && pass "HTTP tools/call run_http_headers_security" || fail "HTTP tools/call run_http_headers_security"
