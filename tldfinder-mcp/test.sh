#!/bin/bash
# Test script for TLDFinder MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/tldfinder-mcp:latest"
PORT=8113
BINARY="tldfinder"
CONTAINER_NAME="tldfinder-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}TLDFinder MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (list_sources)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"list_sources","arguments":{}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call list_sources returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call list_sources returned content"
else
    fail "tools/call list_sources did not return expected response"
