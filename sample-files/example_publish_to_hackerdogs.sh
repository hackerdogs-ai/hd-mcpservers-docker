#!/bin/bash
# Build and Publish Streamlit Supervisord Docker Image to Docker Hub
# Image name: hackerdogs-web-sd (with -sd suffix to avoid conflicts)
...
# Configuration
IMAGE_NAME="hackerdogs-web-sd"
DOCKERFILE="streamlit-only-supervisord-dockerfile"
DEFAULT_TAG="latest"
...
DO_PUBLISH=false
SHOW_HELP=false
# sequential = push amd64 then arm64 then create manifest (smaller uploads, better for 502 retries)
...
if [ "$SHOW_HELP" = true ]; then
    echo "Build and Publish Streamlit Supervisord Docker Image to Docker Hub"
    echo ""
    echo "Usage:"
    ...
    echo "  --platforms parallel|sequential  Push both platforms at once (default) or amd64 then arm64 (better for 502 retries)"
    ...
    echo "  $0 --build --publish hackerdogs v1.2.3           # Build and publish with tag v1.2.3"
    echo "  $0 --build --publish --platforms sequential hackerdogs 1.0.5 latest   # Push amd64 then arm64 (recommended if 502)"
    echo "  $0 --build --publish hackerdogs v1.2.3 latest     # Build and publish with tags v1.2.3 and latest"
    exit 0
fi
...
# Normalize platforms mode (default parallel if invalid or empty)
...
# Get Docker Hub username from remaining arguments (only needed for publishing)
if [ "$DO_PUBLISH" = true ]; then
    # Docker Hub username is required for publishing
    if [ ${#ARGS[@]} -eq 0 ]; then
    ...
    # For local-only builds, use local image name format
    DOCKERHUB_USERNAME=""
    FULL_IMAGE_NAME="${IMAGE_NAME}"
fi

# Get tags from remaining arguments (default to 'latest' if not provided)
TAGS=("${ARGS[@]:1}")
if [ ${#TAGS[@]} -eq 0 ]; then
    TAGS=("$DEFAULT_TAG")
fi
...
if [ "$DO_BUILD" = true ] && [ "$DO_PUBLISH" = true ]; then
    echo -e "${BLUE}Building and Publishing Streamlit Supervisord Docker Image${NC}"
elif [ "$DO_BUILD" = true ]; then
    echo -e "${BLUE}Building Streamlit Supervisord Docker Image${NC}"
else
    echo -e "${BLUE}Publishing Streamlit Supervisord Docker Image to Docker Hub${NC}"
fi
echo "================================================================================="
if [ "$DO_PUBLISH" = true ]; then
    echo "Docker Hub Username: ${GREEN}${DOCKERHUB_USERNAME}${NC}"
fi
echo "Image Name: ${GREEN}${IMAGE_NAME}${NC} (with -sd suffix)"
...
if ! docker buildx version > /dev/null 2>&1; then
    echo -e "${RED}Error: Docker Buildx is not available. Please upgrade Docker to a version that supports Buildx${NC}"
    exit 1
fi
...
# Check Docker Hub authentication if we're going to publish
if [ "$DO_PUBLISH" = true ]; then
    echo ""
...
if [ "$DO_BUILD" = true ]; then
    ...
    if [ "$DO_PUBLISH" = true ]; then
        ...
        if [ "$PLATFORMS_MODE" = "sequential" ]; then
            echo "Mode: sequential (push amd64 then arm64, then create manifest — better for 502 retries)"
        else
            echo "Mode: parallel (push both platforms at once)"
        fi
        echo "This may take 10-20+ minutes (building for multiple platforms)..."
        echo ""

        # Retry config: more attempts and exponential backoff for 502 Bad Gateway
        MAX_RETRIES=5
        do_build_push_with_retry() {
            ...
            while [ $retry -lt $MAX_RETRIES ]; do
                if [ $retry -gt 0 ]; then
                    echo -e "${YELLOW}Retry $retry/$MAX_RETRIES in ${backoff}s (502/network issues are often transient)...${NC}"
                    sleep "$backoff"
                    backoff=$((backoff * 2))
        ...
        for tag in "${TAGS[@]}"; do
            if [ "$PLATFORMS_MODE" = "sequential" ]; then
                # Push one platform at a time so each upload is smaller and retries only affect one platform
                echo "Building and pushing ${FULL_IMAGE_NAME}:${tag} (sequential: amd64 then arm64)..."
                if ! do_build_push_with_retry docker buildx build \
                    ...
                    --push \
                    . ; then
                    echo -e "${RED}Error: Failed to build/push amd64 for ${FULL_IMAGE_NAME}:${tag} after $MAX_RETRIES attempts${NC}"
                    echo "See docs/DOCKER_HUB_502_PUSH_TROUBLESHOOTING.md for 502 workarounds."
                    exit 1
                fi
                ...
                if ! do_build_push_with_retry docker buildx build \
                    ...
                    --push \
                    . ; then
                    echo -e "${RED}Error: Failed to build/push arm64 for ${FULL_IMAGE_NAME}:${tag} after $MAX_RETRIES attempts${NC}"
                    echo "See docs/DOCKER_HUB_502_PUSH_TROUBLESHOOTING.md for 502 workarounds."
                    exit 1
                fi
                echo -e "${GREEN}✅ Pushed ${FULL_IMAGE_NAME}:${tag}-arm64${NC}"
                echo "Creating multi-platform manifest ${FULL_IMAGE_NAME}:${tag}..."
                ...
                # Parallel: build and push both platforms in one go
                ...
                if ! do_build_push_with_retry docker buildx build \
                    ...
                    --push \
                    . ; then
                    echo -e "${RED}Error: Docker buildx build failed for ${FULL_IMAGE_NAME}:${tag} after $MAX_RETRIES attempts${NC}"
                    echo "Tip: Try --platforms sequential to push amd64 then arm64 (smaller uploads). See docs/DOCKER_HUB_502_PUSH_TROUBLESHOOTING.md"
                    exit 1
                fi
            fi
        ...
        # Single-platform build for local testing (faster)
        ...
        # Automatically tag with registry prefix for docker-compose compatibility
        # Docker Compose expects hackerdogs/hackerdogs-web-sd:latest format
        # This applies to both --build only and --build --publish scenarios
        ...
        if [ ${#TAGS[@]} -gt 1 ]; then
            ...
            for tag in "${TAGS[@]:1}"; do
                ...
                # Also tag with registry prefix for docker-compose
...
# Publish images to Docker Hub (if requested and not already pushed during build)
if [ "$DO_PUBLISH" = true ] && [ "$DO_BUILD" = false ]; then
    ...
    # Note: If images were built with buildx and --push, they're already published
    # This section only runs if --publish was used without --build
    echo ""
    echo -e "${YELLOW}Note: If images were built with buildx --push, they're already published.${NC}"
    echo -e "${YELLOW}Pushing any remaining images...${NC}"
    for tag in "${TAGS[@]}"; do
        echo ""
...
echo "Note: This image uses the -sd suffix to avoid conflicts with hackerdogs-web"
...
if [ "$DO_PUBLISH" = true ]; then
    # Published: tags,platforms,date,dockerhub_link
