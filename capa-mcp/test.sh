#!/bin/bash
# Test script for Capa MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/capa-mcp:latest"
PORT=8329
BINARY="capa"
CONTAINER_NAME="capa-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Capa MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_capa)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_capa","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_capa returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_capa returned content"
else
    fail "tools/call run_capa did not return expected response"
