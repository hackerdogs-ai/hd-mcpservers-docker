...
MCP server wrapper for [Capa](https://github.com/mandiant/capa) — Capability detection tool for malware triage and analysis.

## What is Capa?

Capa (capa) is a security tool that provides: **Capability detection tool for malware triage and analysis.**

See [mandiant/capa](https://github.com/mandiant/capa) for full documentation.

**No API keys required** — Capa runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Capa](https://github.com/mandiant/capa) — Capability detection tool for malware triage and analysis.

**Tools:**
- `run_capa` — Run capa with the given arguments. Returns structured JSON output.
...
Run capa with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "capa output will appear here"
}
```
...
- "Run capa with --help to see all available options."
- "Use capa to scan the target 192.168.1.1."
- "What options does capa support? Show me its help output."
- "Run capa against example.com with default settings."
- "Execute capa with verbose output enabled."
- "Use the capa tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/capa-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8329:8329 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8329 \
  hackerdogs/capa-mcp:latest
```
...
```json
{
  "mcpServers": {
    "capa-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/capa-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "capa-mcp": {
      "url": "http://localhost:8485/capa-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8329` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/capa-mcp:latest .
```
...
```bash
docker run -d --rm --name capa-mcp-test -p 8329:8329 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/capa-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8329/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8329/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8329/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_capa","arguments":{"arguments":"--help"}}}'
```
