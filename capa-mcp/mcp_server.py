#!/usr/bin/env python3
"""Capa MCP Server — Capability detection tool for malware triage and analysis.

Wraps the capa CLI (mandiant/capa) to expose
...
MCP_PORT = int(os.environ.get("MCP_PORT", "8329"))
...
BIN_NAME = os.environ.get("CAPA_BIN", "capa")


def _find_binary() -> str:
    """Locate the capa binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("capa binary not found on PATH")
        raise FileNotFoundError(
            f"capa binary not found. Ensure it is installed and available "
            f"on PATH, or set CAPA_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a capa command and return structured output.
...
async def run_capa(
    ...
    """Run capa with the given arguments.
    ...
    logger.info("run_capa called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("capa command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"capa failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"capa {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting capa-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
