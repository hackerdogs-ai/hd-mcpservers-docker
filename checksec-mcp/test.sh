#!/bin/bash
# Test script for Checksec MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/checksec-mcp:latest"
PORT=8244
BINARY="checksec"
CONTAINER_NAME="checksec-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Checksec MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_checksec)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_checksec","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_checksec returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_checksec returned content"
else
    fail "tools/call run_checksec did not return expected response"
