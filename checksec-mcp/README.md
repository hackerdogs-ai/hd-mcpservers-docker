...
MCP server wrapper for [Checksec](https://github.com/slimm609/checksec.sh) — Binary security property checker.

## What is Checksec?

Checksec (checksec) is a security tool that provides: **Binary security property checker.**

See [slimm609/checksec.sh](https://github.com/slimm609/checksec.sh) for full documentation.

**No API keys required** — Checksec runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Checksec](https://github.com/slimm609/checksec.sh) — Binary security property checker.

**Tools:**
- `run_checksec` — Run checksec with the given arguments. Returns structured JSON output.
...
Run checksec with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "checksec output will appear here"
}
```
...
- "Run checksec with --help to see all available options."
- "Use checksec to scan the target 192.168.1.1."
- "What options does checksec support? Show me its help output."
- "Run checksec against example.com with default settings."
- "Execute checksec with verbose output enabled."
- "Use the checksec tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/checksec-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8244:8244 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8244 \
  hackerdogs/checksec-mcp:latest
```
...
```json
{
  "mcpServers": {
    "checksec-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/checksec-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "checksec-mcp": {
      "url": "http://localhost:8485/checksec-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8244` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/checksec-mcp:latest .
```
...
```bash
docker run -d --rm --name checksec-mcp-test -p 8244:8244 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/checksec-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8244/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8244/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8244/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_checksec","arguments":{"arguments":"--help"}}}'
```
