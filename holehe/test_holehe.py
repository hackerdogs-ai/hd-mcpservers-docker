#!/usr/bin/env python3
"""
Test suite for Holehe MCP Server.
Tests the Holehe email search tool.
"""
...
def main():
    """Run tests for Holehe."""
    print("="*60)
    print("Holehe MCP Server Test Suite")
    print("="*60)
    
    docker_cmd = ["docker", "run", "--rm", "-i", "hackerdogs/holehe-mcp-server:latest"]
    ...
    if "result" in response and response["result"].get("serverInfo", {}).get("name") == "holehe-mcp-server":
    ...
    if "result" in response and "tools" in response["result"]:
        ...
        if len(tools) > 0 and tools[0].get("name") == "holehe_email_search":
