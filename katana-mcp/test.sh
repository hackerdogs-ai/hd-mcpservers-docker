...
PASS=0; FAIL=0
IMAGE="hackerdogs/katana-mcp:latest"
PORT=8387
CONTAINER_NAME="katana-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_katana","arguments":{"arguments":"-h"}}}'
echo "========== katana-mcp test (compliance) =========="
info "[1] Install"
...
info "[3] Stdio tools/call run_katana"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 5 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'result\|content' && pass "stdio tools/call run_katana" || fail "stdio tools/call run_katana"
...
info "[5] HTTP streamable tools/call run_katana"
...
echo "$CALL_HTTP" | grep -q 'result\|content' && pass "HTTP tools/call run_katana" || fail "HTTP tools/call run_katana"
