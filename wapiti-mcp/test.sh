#!/bin/bash
# Test script for Wapiti MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/wapiti-mcp:latest"
PORT=8288
BINARY="wapiti"
CONTAINER_NAME="wapiti-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Wapiti MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_wapiti)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_wapiti","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_wapiti returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_wapiti returned content"
else
    fail "tools/call run_wapiti did not return expected response"
