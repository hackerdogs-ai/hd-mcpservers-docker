#!/usr/bin/env bash
# Compliance sweep for mcp-servers-simple-index.txt lines 171–180.
set -u
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOCKDIR="${ROOT}/.run_index_171_180_work.lockdir"
if ! mkdir "$LOCKDIR" 2>/dev/null; then
  echo "run_index_171_180_tests.sh: another sweep is running (lock: ${LOCKDIR}). If none, remove that directory." >&2
  exit 1
fi
...
dirs=(
  gef-mcp geocoding-mcp ggshield-mcp ghidra-mcp ghunt-mcp gitlab-mcp
  gitleaks-mcp globalping-mcp gobuster-mcp google-threat-intelligence-mcp
)
