#!/usr/bin/env bash
# Compliance sweep for mcp-servers-simple-index.txt lines 181–193.
set -u
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOCKDIR="${ROOT}/.run_index_181_193_tests.lockdir"
if ! mkdir "$LOCKDIR" 2>/dev/null; then
  echo "run_index_181_193_tests.sh: another sweep is running (lock: ${LOCKDIR}). If none, remove that directory." >&2
  exit 1
fi
...
dirs=(
  gospider-mcp gowitness-mcp graphql-voyager-mcp graphviz-dot-mcp greynoise-mcp grype-mcp
  hakrawler-mcp hashcat-mcp hashid-mcp hashpump-mcp hibp-mcp holehe-mcp horusec-mcp
)
