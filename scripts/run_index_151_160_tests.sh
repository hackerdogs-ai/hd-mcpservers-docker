#!/usr/bin/env bash
# Compliance sweep for mcp-servers-simple-index.txt lines 151–160.
set -u
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOCKDIR="${ROOT}/.run_index_151_160_work.lockdir"
if ! mkdir "$LOCKDIR" 2>/dev/null; then
  echo "run_index_151_160_tests.sh: another sweep is running (lock: ${LOCKDIR}). If none, remove that directory." >&2
  exit 1
fi
cleanup_lock() { rmdir "$LOCKDIR" 2>/dev/null || true; }
trap cleanup_lock EXIT INT TERM

LOG="${ROOT}/agent-index-151-160-test.log"
...
if ! docker info >/dev/null 2>&1; then
  ...
  echo "run_index_151_160_tests.sh: Docker is not running — cannot build or test." >&2
  exit 1
fi

_PY="/c/Users/${USERNAME:-$USER}/AppData/Local/Python/pythoncore-3.14-64/python.exe"
if [[ -x "$_PY" ]]; then
  _SHIM="${TMPDIR:-/tmp}/mcp151python"
  mkdir -p "$_SHIM"
...
dirs=(
  evil-winrm-mcp exa-mcp excel-tools-mcp exiftool-agent-mcp exiftool-mcp
  exploitdb-mcp falco-mcp feroxbuster-mcp fetch-mcp ffuf-mcp
)
