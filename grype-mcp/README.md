...
MCP server wrapper for [Grype](https://github.com/anchore/grype) — Container image and filesystem vulnerability scanner.

## What is Grype?

Grype (grype) is a security tool that provides: **Container image and filesystem vulnerability scanner.**

See [anchore/grype](https://github.com/anchore/grype) for full documentation.

**No API keys required** — Grype runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Grype](https://github.com/anchore/grype) — Container image and filesystem vulnerability scanner.

**Tools:**
- `run_grype` — Run grype with the given arguments. Returns structured JSON output.
...
Run grype with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "grype output will appear here"
}
```
...
- "Run grype with --help to see all available options."
- "Use grype to scan the target 192.168.1.1."
- "What options does grype support? Show me its help output."
- "Run grype against example.com with default settings."
- "Execute grype with verbose output enabled."
- "Use the grype tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/grype-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8357:8357 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8357 \
  hackerdogs/grype-mcp:latest
```
...
```json
{
  "mcpServers": {
    "grype-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/grype-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "grype-mcp": {
      "url": "http://localhost:8485/grype-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8357` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/grype-mcp:latest .
```
...
```bash
docker run -d --rm --name grype-mcp-test -p 8357:8357 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/grype-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8357/mcp \
...
curl -s -X POST http://localhost:8357/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_grype","arguments":{"arguments":"--help"}}}'
```
