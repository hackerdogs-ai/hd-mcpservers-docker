#!/usr/bin/env python3
"""Grype MCP Server — Container image and filesystem vulnerability scanner.

Wraps the grype CLI (anchore/grype) to expose
...
logger = logging.getLogger("grype-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8357"))
...
BIN_NAME = os.environ.get("GRYPE_BIN", "grype")


def _find_binary() -> str:
    """Locate the grype binary, raising a clear error if missing."""
    path = shutil.which(BIN_NAME)
    if path is None:
        logger.error("grype binary not found on PATH")
        raise FileNotFoundError(
            f"grype binary not found. Ensure it is installed and available "
            f"on PATH, or set GRYPE_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a grype command and return structured output.
...
async def run_grype(
    ...
    """Run grype with the given arguments.
    ...
    logger.info("run_grype called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("grype command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"grype failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"grype {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting grype-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
