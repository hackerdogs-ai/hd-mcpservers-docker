#!/bin/bash
# Test script for Grype MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/grype-mcp:latest"
PORT=8357
BINARY="grype"
CONTAINER_NAME="grype-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Grype MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_grype)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_grype","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_grype returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_grype returned content"
else
    fail "tools/call run_grype did not return expected response"
