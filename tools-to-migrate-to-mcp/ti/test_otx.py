"""
Comprehensive tests for AlienVault OTX threat intelligence tools.

This test suite validates all OTX LangChain tools including:
- otx_file_report
- otx_url_report
- otx_domain_report
- otx_ip_report
...
try:
    ...
    OTX_TOOLS_AVAILABLE = True
except ImportError as e:
    OTX_TOOLS_AVAILABLE = False
    IMPORT_ERROR = str(e)


@pytest.fixture
def mock_runtime():
    """Create a mock ToolRuntime with OTX configuration."""
    runtime = Mock()
    runtime.state = {
        "api_keys": {"otx": "test_api_key_123"},
...
def mock_runtime_no_config():
    """Create a mock ToolRuntime without OTX configuration."""
    runtime = Mock()
    runtime.state = {
        "api_keys": {},
        "user_id": "test_user_001"
...
def mock_otx_response():
    """Create a mock OTX API response."""
    return {
        "data": {
            "attributes": {
...
class TestOTXHelperFunctions:
    """Test helper functions for OTX tools."""
    
    def test_calculate_threat_verdict_malicious(self):
        """Test threat verdict calculation for malicious indicator."""
        if not OTX_TOOLS_AVAILABLE:
            pytest.skip(f"OTX tools not available: {IMPORT_ERROR}")
        
        attributes = {"type": "malware", "score": 8}
    ...
    def test_calculate_threat_verdict_suspicious(self):
        ...
        attributes = {"type": "suspicious", "score": 6}
        verdict = _calculate_threat_verdict(attributes)
    ...
    def test_calculate_threat_verdict_clean(self):
        ...
        attributes = {"type": "clean", "score": 2}
        verdict = _calculate_threat_verdict(attributes)
        assert verdict == "CLEAN"


class TestOTXFileReport:
    """Test otx_file_report tool."""
    
    def test_file_report_success(self, mock_runtime, mock_otx_response):
        """Test successful file hash lookup."""
        if not OTX_TOOLS_AVAILABLE:
            pytest.skip(f"OTX tools not available: {IMPORT_ERROR}")
        
        with patch('shared.modules.tools.ti.otx.requests.get') as mock_get:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = mock_otx_response
            mock_get.return_value = mock_response
            
            result = otx_file_report.func(mock_runtime, "44d88612fea8a8f36de82e1278abb02f")
            parsed = json.loads(result)
            
            assert parsed["status"] == "success"
            assert parsed["hash"] == "44d88612fea8a8f36de82e1278abb02f"
            assert "threat_verdict" in parsed
            assert "ioc" in parsed
    ...
    def test_file_report_no_config(self, mock_runtime_no_config):
        ...
        result = otx_file_report.func(mock_runtime_no_config, "44d88612fea8a8f36de82e1278abb02f")
        parsed = json.loads(result)
    ...
    def test_file_report_not_found(self, mock_runtime):
        ...
        with patch('shared.modules.tools.ti.otx.requests.get') as mock_get:
            ...
            result = otx_file_report.func(mock_runtime, "nonexistent_hash")
            parsed = json.loads(result)
...
class TestOTXURLReport:
    """Test otx_url_report tool."""
    
    def test_url_report_success(self, mock_runtime, mock_otx_response):
        ...
        with patch('shared.modules.tools.ti.otx.requests.get') as mock_get:
            ...
            result = otx_url_report.func(mock_runtime, "https://example.com")
            parsed = json.loads(result)
    ...
    def test_url_report_no_config(self, mock_runtime_no_config):
        ...
        result = otx_url_report.func(mock_runtime_no_config, "https://example.com")
        parsed = json.loads(result)
...
class TestOTXDomainReport:
    """Test otx_domain_report tool."""
    
    def test_domain_report_success(self, mock_runtime, mock_otx_response):
        ...
        with patch('shared.modules.tools.ti.otx.requests.get') as mock_get:
            ...
            result = otx_domain_report.func(mock_runtime, "example.com")
            parsed = json.loads(result)
    ...
    def test_domain_report_no_config(self, mock_runtime_no_config):
        ...
        result = otx_domain_report.func(mock_runtime_no_config, "example.com")
        parsed = json.loads(result)
...
class TestOTXIPReport:
    """Test otx_ip_report tool."""
    
    def test_ip_report_success(self, mock_runtime, mock_otx_response):
        ...
        with patch('shared.modules.tools.ti.otx.requests.get') as mock_get:
            ...
            result = otx_ip_report.func(mock_runtime, "8.8.8.8")
            parsed = json.loads(result)
    ...
    def test_ip_report_no_config(self, mock_runtime_no_config):
        ...
        result = otx_ip_report.func(mock_runtime_no_config, "8.8.8.8")
        parsed = json.loads(result)
...
class TestOTXSubmitURL:
    """Test otx_submit_url tool."""
    ...
    def test_submit_url_success(self, mock_runtime):
        ...
        with patch('shared.modules.tools.ti.otx.requests.post') as mock_post:
            mock_response = Mock()
            mock_response.status_code = 200
            mock_response.json.return_value = mock_response_data
            mock_post.return_value = mock_response
            
            result = otx_submit_url.func(mock_runtime, "https://example.com")
            parsed = json.loads(result)
    ...
    def test_submit_url_no_config(self, mock_runtime_no_config):
        ...
        result = otx_submit_url.func(mock_runtime_no_config, "https://example.com")
        parsed = json.loads(result)
    ...
    def test_submit_url_api_error(self, mock_runtime):
        ...
        with patch('shared.modules.tools.ti.otx.requests.post') as mock_post:
            ...
            result = otx_submit_url.func(mock_runtime, "invalid-url")
            parsed = json.loads(result)
...
class TestOTXRealWorldScenarios:
    """Test real-world scenarios for OTX tools."""
    
    def test_incident_response_workflow(self, mock_runtime, mock_otx_response):
        """Test complete incident response workflow."""
        if not OTX_TOOLS_AVAILABLE:
            pytest.skip(f"OTX tools not available: {IMPORT_ERROR}")
        
        with patch('shared.modules.tools.ti.otx.requests.get') as mock_get, \
             patch('shared.modules.tools.ti.otx.requests.post') as mock_post:
            ...
            file_result = otx_file_report.func(mock_runtime, "44d88612fea8a8f36de82e1278abb02f")
            assert json.loads(file_result)["status"] == "success"
            
            # Step 2: Check URL
            url_result = otx_url_report.func(mock_runtime, "https://suspicious-site.com")
            assert json.loads(url_result)["status"] == "success"
            
            # Step 3: Check domain
            domain_result = otx_domain_report.func(mock_runtime, "suspicious-site.com")
            assert json.loads(domain_result)["status"] == "success"
            
            # Step 4: Check IP
            ip_result = otx_ip_report.func(mock_runtime, "192.168.1.1")
            assert json.loads(ip_result)["status"] == "success"
            
            # Step 5: Submit new URL for analysis
            submit_result = otx_submit_url.func(mock_runtime, "https://new-suspicious-site.com")
