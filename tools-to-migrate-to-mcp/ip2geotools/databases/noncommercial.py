...
from .interfaces import IGeoIpDatabase
from ..models import IpLocation
from ..errors import IpAddressNotFoundError, PermissionRequiredError, \
...
class DbIpCity(IGeoIpDatabase):
    ...
    def get(ip_address, api_key='free', db_path=None, username=None, password=None):
        ...
        if geocoder is None:
            raise ServiceError("Optional dependency 'geocoder' is required for DbIpCity OSM lat/lon lookup. "
                               "Install it (e.g., `pip install geocoder`) or use a different geo DB.")
...
class MaxMindGeoLite2City(IGeoIpDatabase):
    ...
    def get(ip_address, api_key=None, db_path=None, username=None, password=None):
        # process request
        if 'geoip2' not in globals() or geoip2 is None:
            raise ServiceError("Optional dependency 'geoip2' is required for MaxMindGeoLite2City. "
                               "Install it (e.g., `pip install geoip2`) and provide a valid db_path.")
        try:
...
class Ip2Location(IGeoIpDatabase):
    ...
    def get(ip_address, api_key=None, db_path=None, username=None, password=None):
        ...
        if IP2Location is None:
            raise ServiceError("Optional dependency 'IP2Location' is required for Ip2Location. "
                               "Install it (e.g., `pip install IP2Location`) and provide a valid db_path.")
        try:
