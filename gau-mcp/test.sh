#!/bin/bash
# Test script for GAU MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/gau-mcp:latest"
PORT=8218
BINARY="gau"
CONTAINER_NAME="gau-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}GAU MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_gau)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_gau","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_gau returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_gau returned content"
else
    fail "tools/call run_gau did not return expected response"
