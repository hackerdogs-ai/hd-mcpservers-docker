...
MCP server wrapper for [Octagon](https://github.com/octagon-agents/octagon-mcp) — upstream package `octagon-mcp`.
...
docker run -i --rm \
  -e OCTAGON_API_KEY \
  hackerdogs/octagon-mcp-server-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8653:8653 \
...
| `MCP_PORT` | `8653` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/octagon-mcp-server-mcp:latest .
```
...
```bash
docker run -d --rm --name octagon-mcp-server-mcp-test -p 8653:8653 \
  -e MCP_TRANSPORT=streamable-http \
  -e OCTAGON_API_KEY \
  hackerdogs/octagon-mcp-server-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8653/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8653/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
