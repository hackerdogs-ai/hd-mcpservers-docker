...
docker run -i --rm \
  -e OPENFDA_API_KEY \
  hackerdogs/openfda-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8655:8655 \
...
| `MCP_PORT` | `8655` | HTTP port (only used with `streamable-http`) |
| `OPENFDA_API_KEY` | — | OpenFDA API key — get one at [open.fda.gov/apis/authentication](https://open.fda.gov/apis/authentication/) |
...
```bash
docker build -t hackerdogs/openfda-mcp:latest .
```
...
```bash
docker run -d --rm --name openfda-mcp-test -p 8655:8655 \
  -e MCP_TRANSPORT=streamable-http \
  -e OPENFDA_API_KEY \
  hackerdogs/openfda-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8655/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8655/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
