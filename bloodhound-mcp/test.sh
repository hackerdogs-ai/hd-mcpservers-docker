#!/bin/bash
# Test script for BloodHound MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/bloodhound-mcp:latest"
PORT=8286
BINARY="bloodhound-python"
CONTAINER_NAME="bloodhound-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}BloodHound MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_bloodhound_python)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_bloodhound","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_bloodhound_python returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_bloodhound_python returned content"
else
    fail "tools/call run_bloodhound_python did not return expected response"
