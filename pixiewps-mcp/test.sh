#!/bin/bash
# Test script for Pixiewps MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/pixiewps-mcp:latest"
PORT=8311
BINARY="pixiewps"
CONTAINER_NAME="pixiewps-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Pixiewps MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_pixiewps)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_pixiewps","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_pixiewps returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_pixiewps returned content"
else
    fail "tools/call run_pixiewps did not return expected response"
