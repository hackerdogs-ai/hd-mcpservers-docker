#!/bin/bash
# Test script for Cisco AI Defense Scanner MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/cisco-mcp-scanner-mcp:latest"
PORT=8342
BINARY="mcp-scanner"
CONTAINER_NAME="cisco-mcp-scanner-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Cisco AI Defense Scanner MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_mcp_scanner)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_cisco_mcp_scanner","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_mcp_scanner returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_mcp_scanner returned content"
else
    fail "tools/call run_mcp_scanner did not return expected response"
