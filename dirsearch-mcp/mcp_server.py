#!/usr/bin/env python3
"""Dirsearch MCP Server — Directory and file discovery.

Wraps the dirsearch CLI (maurosoria/dirsearch) to expose
...
logger = logging.getLogger("dirsearch-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8215"))
...
def _find_binary() -> str:
    """Locate the dirsearch binary, raising a clear error if missing."""
    path = shutil.which(DIRSEARCH_BIN)
    if path is None:
        logger.error("dirsearch binary not found on PATH")
        raise FileNotFoundError(
            f"dirsearch binary not found. Ensure it is installed and available "
            f"on PATH, or set DIRSEARCH_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a dirsearch command and return structured output.
...
async def run_dirsearch(
    ...
    """Run dirsearch with the given arguments.
    ...
    logger.info("run_dirsearch called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("dirsearch command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"dirsearch failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"dirsearch {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting dirsearch-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
