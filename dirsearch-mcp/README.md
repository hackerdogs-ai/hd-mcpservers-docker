...
MCP server wrapper for [Dirsearch](https://github.com/maurosoria/dirsearch) — Directory and file discovery.

## What is Dirsearch?

Dirsearch (dirsearch) is a security tool that provides: **Directory and file discovery.**

See [maurosoria/dirsearch](https://github.com/maurosoria/dirsearch) for full documentation.

**No API keys required** — Dirsearch runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Dirsearch](https://github.com/maurosoria/dirsearch) — Directory and file discovery.

**Tools:**
- `run_dirsearch` — Run dirsearch with the given arguments. Returns structured JSON output.
...
Run dirsearch with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "dirsearch output will appear here"
}
```
...
- "Run dirsearch with --help to see all available options."
- "Use dirsearch to scan the target 192.168.1.1."
- "What options does dirsearch support? Show me its help output."
- "Run dirsearch against example.com with default settings."
- "Execute dirsearch with verbose output enabled."
- "Use the dirsearch tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/dirsearch-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8215:8215 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8215 \
  hackerdogs/dirsearch-mcp:latest
```
...
```json
{
  "mcpServers": {
    "dirsearch-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/dirsearch-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
| `MCP_PORT` | `8215` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/dirsearch-mcp:latest .
```
...
```bash
docker run -d --rm --name dirsearch-mcp-test -p 8215:8215 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/dirsearch-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8215/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8215/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8215/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_dirsearch","arguments":{"arguments":"--help"}}}'
```
...
```json
{
  "mcpServers": {
    "dirsearch-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "hackerdogs/dirsearch-mcp:latest"],
      "env": {}
    }
  }
}
...
```bash
docker run -d -p 8215:8215 -e MCP_TRANSPORT=streamable-http hackerdogs/dirsearch-mcp:latest
```
