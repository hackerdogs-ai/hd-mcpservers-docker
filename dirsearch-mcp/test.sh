#!/bin/bash
# Test script for Dirsearch MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/dirsearch-mcp:latest"
PORT=8215
BINARY="dirsearch"
CONTAINER_NAME="dirsearch-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Dirsearch MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_dirsearch)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_dirsearch","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_dirsearch returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_dirsearch returned content"
else
    fail "tools/call run_dirsearch did not return expected response"
