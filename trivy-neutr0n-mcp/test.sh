#!/bin/bash
# Test script for Trivy Neutr0n MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/trivy-neutr0n-mcp:latest"
PORT=8356
BINARY="trivy-mcp"
CONTAINER_NAME="trivy-neutr0n-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Trivy Neutr0n MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_trivy_mcp)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_trivy_mcp","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_trivy_mcp returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_trivy_mcp returned content"
else
    fail "tools/call run_trivy_mcp did not return expected response"
