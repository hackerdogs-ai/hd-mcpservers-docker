#!/bin/bash
# Test script for Port Scanner MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/port-scanner-mcp:latest"
PORT=8354
BINARY="port-scanner"
CONTAINER_NAME="port-scanner-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Port Scanner MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_port_scanner)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_port_scanner","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_port_scanner returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_port_scanner returned content"
else
    fail "tools/call run_port_scanner did not return expected response"
