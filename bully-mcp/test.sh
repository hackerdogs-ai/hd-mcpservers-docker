#!/bin/bash
# Test script for Bully MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/bully-mcp:latest"
PORT=8310
BINARY="bully"
CONTAINER_NAME="bully-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Bully MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_bully)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_bully","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_bully returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_bully returned content"
else
    fail "tools/call run_bully did not return expected response"
