#!/bin/bash
# Test script for asnmap MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/asnmap-mcp:latest"
PORT=8110
BINARY="asnmap"
CONTAINER_NAME="asnmap-mcp-test"
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

if [ -z "${PDCP_API_KEY:-}" ]; then
    echo -e "${YELLOW}⚠️  WARNING: PDCP_API_KEY is not set. Asnmap requires a ProjectDiscovery Cloud Platform API key.${NC}"
    echo -e "${YELLOW}   Get one free at https://cloud.projectdiscovery.io/?ref=api_key${NC}"
    echo -e "${YELLOW}   Set it: export PDCP_API_KEY=your_key${NC}"
    echo ""
fi
...
echo "================================================================================="
echo -e "${BLUE}asnmap MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
BINARY_OUTPUT=$(docker run --rm "$IMAGE" $BINARY -version 2>&1 | head -5 || true)
...
STDIO_OUT=$(python3 "$PROJECT_DIR/../scripts/mcp_stdio_docker_tools_list.py" -e PDCP_API_KEY="${PDCP_API_KEY:-}" "$IMAGE") || true
...
info "[Test 6] MCP HTTP — tools/call (lookup_asn)"
if [ -z "${PDCP_API_KEY:-}" ]; then
    echo -e "  ${YELLOW}⏭️  SKIPPED: PDCP_API_KEY not set (required for asnmap API calls)${NC}"
else
    CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"lookup_asn","arguments":{"asn":"AS13335"}}}'
    ...
    if echo "$CALL_RESP" | grep -q '"result"'; then
        pass "tools/call lookup_asn returned a result"
    elif echo "$CALL_RESP" | grep -q '"content"'; then
        pass "tools/call lookup_asn returned content"
    else
        fail "tools/call lookup_asn did not return expected response"
