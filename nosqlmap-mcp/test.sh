#!/bin/bash
# Test script for NoSQLMap MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/nosqlmap-mcp:latest"
PORT=8266
BINARY="nosqlmap"
CONTAINER_NAME="nosqlmap-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}NoSQLMap MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_nosqlmap)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_nosqlmap","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_nosqlmap returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_nosqlmap returned content"
else
    fail "tools/call run_nosqlmap did not return expected response"
