...
MCP server wrapper for [Nosqlmap](https://github.com/codingo/NoSQLMap) — NoSQL injection testing.

## What is Nosqlmap?

Nosqlmap (nosqlmap) is a security tool that provides: **NoSQL injection testing.**

See [codingo/NoSQLMap](https://github.com/codingo/NoSQLMap) for full documentation.
...
Run nosqlmap with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "nosqlmap output will appear here"
}
```
...
```bash
docker run -i --rm hackerdogs/nosqlmap-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8266:8266 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8266 \
  hackerdogs/nosqlmap-mcp:latest
```
...
```json
{
  "mcpServers": {
    "nosqlmap-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/nosqlmap-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "nosqlmap-mcp": {
      "url": "http://localhost:8485/nosqlmap-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8266` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/nosqlmap-mcp:latest .
```
...
```bash
docker run -d --rm --name nosqlmap-mcp-test -p 8266:8266 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/nosqlmap-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8266/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8266/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8266/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_nosqlmap","arguments":{"arguments":"--help"}}}'
```
