#!/bin/bash
# Test script for Gospider MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/gospider-mcp:latest"
PORT=8350
BINARY="gospider"
CONTAINER_NAME="gospider-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Gospider MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
BINARY_OUTPUT=$(docker run --rm "$IMAGE" $BINARY -h 2>&1 | head -5 || docker run --rm "$IMAGE" $BINARY --version 2>&1 | head -5 || docker run --rm "$IMAGE" $BINARY -h 2>&1 | head -5 || true)
...
while [ "$WAITED" -lt "$MAX_WAIT" ]; do
    ...
    SESSION_ID=$(grep -i 'mcp-session-id' "$MCP_HDR_FILE" 2>/dev/null | sed 's/.*:[[:space:]]*//' | tr -d '' | head -1 || true)
...
info "[Test 6] MCP HTTP — tools/call (run_gospider)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_gospider","arguments":{"arguments":"-h"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_gospider returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_gospider returned content"
else
    fail "tools/call run_gospider did not return expected response"
