...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"urlhaus_host","arguments":{"host":"example.com"}}}'

echo "========== abusech-mcp test (compliance) =========="
...
info "[3] Stdio tools/call urlhaus_host"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 4 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'result\|content\|error' && pass "stdio tools/call urlhaus_host" || fail "stdio tools/call urlhaus_host"
...
info "[5] HTTP streamable tools/call urlhaus_host"
...
echo "$CALL_HTTP" | grep -q 'result\|content\|error' && pass "HTTP tools/call urlhaus_host" || fail "HTTP tools/call urlhaus_host"
