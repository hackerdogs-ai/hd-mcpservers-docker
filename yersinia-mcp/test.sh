#!/bin/bash
# Test script for Yersinia MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/yersinia-mcp:latest"
PORT=8319
BINARY="yersinia"
CONTAINER_NAME="yersinia-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Yersinia MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_yersinia)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_yersinia","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_yersinia returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_yersinia returned content"
else
    fail "tools/call run_yersinia did not return expected response"
