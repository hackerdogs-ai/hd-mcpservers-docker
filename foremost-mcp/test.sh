#!/bin/bash
# Test script for Foremost MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/foremost-mcp:latest"
PORT=8249
BINARY="foremost"
CONTAINER_NAME="foremost-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Foremost MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_foremost)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_foremost","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_foremost returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_foremost returned content"
else
    fail "tools/call run_foremost did not return expected response"
