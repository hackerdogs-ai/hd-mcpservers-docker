...
MCP server wrapper for [Foremost](https://github.com/kdz/foremost) — File carving and data recovery.

## What is Foremost?

Foremost (foremost) is a security tool that provides: **File carving and data recovery.**

See [kdz/foremost](https://github.com/kdz/foremost) for full documentation.

**No API keys required** — Foremost runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Foremost](https://github.com/kdz/foremost) — File carving and data recovery.

**Tools:**
- `run_foremost` — Run foremost with the given arguments. Returns structured JSON output.
...
Run foremost with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "foremost output will appear here"
}
```
...
- "Run foremost with --help to see all available options."
- "Use foremost to scan the target 192.168.1.1."
- "What options does foremost support? Show me its help output."
- "Run foremost against example.com with default settings."
- "Execute foremost with verbose output enabled."
- "Use the foremost tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/foremost-mcp:latest
```
...
docker run -d -p 8249:8249 \
  ...
  -e MCP_PORT=8249 \
  hackerdogs/foremost-mcp:latest
```
...
```json
{
  "mcpServers": {
    "foremost-mcp": {
      "url": "http://localhost:8485/foremost-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8249` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/foremost-mcp:latest .
```
...
```bash
docker run -d --rm --name foremost-mcp-test -p 8249:8249 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/foremost-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8249/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8249/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8249/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_foremost","arguments":{"arguments":"--help"}}}'
```
