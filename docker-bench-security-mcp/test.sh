#!/bin/bash
# Test script for Docker Bench Security MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/docker-bench-security-mcp:latest"
PORT=8254
BINARY="docker-bench-security.sh"
CONTAINER_NAME="docker-bench-security-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Docker Bench Security MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
STDIO_OUT=$(python3 "$PROJECT_DIR/../scripts/mcp_stdio_docker_tools_list.py" -v /var/run/docker.sock:/var/run/docker.sock --group-add 0 "$IMAGE") || true
...
info "[Test 6] MCP HTTP — tools/call (run_docker_bench_security)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_docker_bench_security","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_docker_bench_security returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_docker_bench_security returned content"
else
    fail "tools/call run_docker_bench_security did not return expected response"
