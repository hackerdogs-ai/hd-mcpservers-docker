#!/usr/bin/env python3
"""Docker Bench Security MCP Server — Docker security assessment (CIS).

Wraps the docker-bench-security.sh CLI (docker/docker-bench-security) to expose
...
logger = logging.getLogger("docker-bench-security-mcp")

MCP_TRANSPORT = os.environ.get("MCP_TRANSPORT", "stdio")
MCP_PORT = int(os.environ.get("MCP_PORT", "8254"))

mcp = FastMCP(
    "Docker Bench Security MCP Server",
    instructions=(
        "Docker security assessment (CIS)."
    ),
)

DOCKER_BENCH_SECURITY_BIN = os.environ.get("DOCKER_BENCH_SECURITY_BIN", "docker-bench-security.sh")


def _find_binary() -> str:
    """Locate the docker-bench-security.sh binary, raising a clear error if missing."""
    path = shutil.which(DOCKER_BENCH_SECURITY_BIN)
    if path is None:
        logger.error("docker-bench-security.sh binary not found on PATH")
        raise FileNotFoundError(
            f"docker-bench-security.sh binary not found. Ensure it is installed and available "
            f"on PATH, or set DOCKER_BENCH_SECURITY_BIN to the full path."
        )
    return path


async def _run_command(args: list[str], timeout_seconds: int = 600) -> dict:
    """Execute a docker-bench-security.sh command and return structured output.
...
async def run_docker_bench_security(
    ...
    """Run docker-bench-security.sh with the given arguments.
    ...
    logger.info("run_docker_bench_security called with arguments=%s source_url=%s", arguments, source_url)
    ...
    try:
        ...
        if result["return_code"] != 0:
            logger.warning("docker-bench-security.sh command failed with exit code %d", result["return_code"])
            error_detail = result["stderr"] or result["stdout"] or "Unknown error"
            return json.dumps(
                {
                    "error": True,
                    "message": f"docker-bench-security.sh failed (exit code {result['return_code']})",
                    "detail": error_detail.strip(),
                    "command": f"docker-bench-security.sh {' '.join(args)}",
                },
                indent=2,
            )
...
def main():
    logger.info("Starting docker-bench-security-mcp server (transport=%s, port=%s)", MCP_TRANSPORT, MCP_PORT)
