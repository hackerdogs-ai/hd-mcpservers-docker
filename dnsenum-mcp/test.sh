#!/bin/bash
# Test script for dnsenum MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/dnsenum-mcp:latest"
PORT=8304
BINARY="dnsenum"
CONTAINER_NAME="dnsenum-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}dnsenum MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_dnsenum)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_dnsenum","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_dnsenum returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_dnsenum returned content"
else
    fail "tools/call run_dnsenum did not return expected response"
