#!/bin/bash
# Test script for ParamSpider MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/paramspider-mcp:latest"
PORT=8226
BINARY="paramspider"
CONTAINER_NAME="paramspider-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}ParamSpider MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_paramspider)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_paramspider","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_paramspider returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_paramspider returned content"
else
    fail "tools/call run_paramspider did not return expected response"
