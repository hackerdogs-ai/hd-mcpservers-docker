#!/bin/bash
# Test script for GHunt MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/ghunt-mcp:latest"
PORT=8218
BINARY="ghunt"
CONTAINER_NAME="ghunt-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}GHunt MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
BINARY_OUTPUT=$(docker run --rm "$IMAGE" $BINARY --help 2>&1 | head -5 || docker run --rm "$IMAGE" $BINARY -h 2>&1 | head -5 || true)
...
if env "MCP_STDIO_DOCKER_TIMEOUT=${MCP_STDIO_DOCKER_TIMEOUT:-360}" \
    "$MCP_PYTHON" "$PROJECT_DIR/../scripts/mcp_stdio_docker_tools_list.py" --check "$IMAGE" 2>/dev/null; then
...
while [ "$WAITED" -lt "$MAX_WAIT" ]; do
    ...
    HTTP_CODE=$(head -1 "$MCP_HDR_FILE" 2>/dev/null | grep -oE '[0-9]{3}' | head -1 || echo "000")
...
info "[Test 6] MCP HTTP — tools/call (run_ghunt --help)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_ghunt","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_ghunt returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_ghunt returned content"
else
    fail "tools/call run_ghunt did not return expected response"
