#!/bin/bash
# Test script for Ghidra MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/ghidra-mcp:latest"
PORT=8240
BINARY="analyzeHeadless"
CONTAINER_NAME="ghidra-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Ghidra MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_ghidra)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_ghidra","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_ghidra returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_ghidra returned content"
else
    fail "tools/call run_ghidra did not return expected response"
