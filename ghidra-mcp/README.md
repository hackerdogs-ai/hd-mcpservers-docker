...
MCP server wrapper for [Ghidra](https://github.com/NationalSecurityAgency/ghidra) — NSA reverse engineering suite (headless).

## What is Ghidra?

Ghidra (analyzeHeadless) is a security tool that provides: **NSA reverse engineering suite (headless).**

See [NationalSecurityAgency/ghidra](https://github.com/NationalSecurityAgency/ghidra) for full documentation.

**No API keys required** — Ghidra runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Ghidra](https://github.com/NationalSecurityAgency/ghidra) — NSA reverse engineering suite (headless).

**Tools:**
- `run_ghidra` — Run analyzeHeadless with the given arguments. Returns structured JSON output.
...
Run analyzeHeadless with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "analyzeHeadless output will appear here"
}
```
...
- "Run analyzeHeadless with --help to see all available options."
- "Use ghidra to scan the target 192.168.1.1."
- "What options does analyzeHeadless support? Show me its help output."
- "Run ghidra against example.com with default settings."
- "Execute analyzeHeadless with verbose output enabled."
- "Use the ghidra tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/ghidra-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8240:8240 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8240 \
  hackerdogs/ghidra-mcp:latest
```
...
```json
{
  "mcpServers": {
    "ghidra-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/ghidra-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "ghidra-mcp": {
      "url": "http://localhost:8485/ghidra-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8240` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/ghidra-mcp:latest .
```
...
```bash
docker run -d --rm --name ghidra-mcp-test -p 8240:8240 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/ghidra-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8240/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8240/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8240/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_ghidra","arguments":{"arguments":"--help"}}}'
```
