#!/bin/bash
# Test script for GraphQL-Voyager MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/graphql-voyager-mcp:latest"
PORT=8282
BINARY="graphql-voyager"
CONTAINER_NAME="graphql-voyager-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}GraphQL-Voyager MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_graphql_voyager)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_graphql_voyager","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_graphql_voyager returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_graphql_voyager returned content"
else
    fail "tools/call run_graphql_voyager did not return expected response"
