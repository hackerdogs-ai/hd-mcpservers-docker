...
export FASTMCP_PORT=${MCP_PORT:-8623}
if [ "$MCP_TRANSPORT" = "stdio" ]; then
  exec awslabs.aws-serverless-mcp-server
else
  export FASTMCP_TRANSPORT=stdio
  exec python /mcp_http_proxy.py --port ${MCP_PORT:-8623} -- awslabs.aws-serverless-mcp-server
fi
