#!/bin/bash
# Test script for Ettercap MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/ettercap-mcp:latest"
PORT=8313
BINARY="ettercap"
CONTAINER_NAME="ettercap-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Ettercap MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_ettercap)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_ettercap","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_ettercap returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_ettercap returned content"
else
    fail "tools/call run_ettercap did not return expected response"
