#!/bin/bash
# Test script for Commix MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/commix-mcp:latest"
PORT=8225
BINARY="commix"
CONTAINER_NAME="commix-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Commix MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""

# Test 1: Verify Docker image exists (build separately: docker build -t $IMAGE .)
info "[Test 1] Docker image"
if ! docker image inspect "$IMAGE" > /dev/null 2>&1; then
    fail "Docker image $IMAGE missing"
...
info "[Test 6] MCP HTTP — tools/call (run_commix)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_commix","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_commix returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_commix returned content"
else
    fail "tools/call run_commix did not return expected response"
