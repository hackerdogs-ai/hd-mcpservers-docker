...
PASS=0; FAIL=0
IMAGE="hackerdogs/alterx-mcp:latest"
PORT=8380
CONTAINER_NAME="alterx-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"do_alterx","arguments":{"domain":"example.com","pattern":"{{word}}.{{suffix}}"}}}'

echo "========== alterx-mcp test (compliance) =========="
...
echo "$CALL_OUT" | grep -q 'result\|content\|completed' && pass "stdio tools/call do_alterx" || fail "stdio tools/call do_alterx"
...
info "[5] HTTP streamable tools/call do_alterx"
...
echo "$CALL_HTTP" | grep -q 'result\|content' && pass "HTTP tools/call do_alterx" || fail "HTTP tools/call do_alterx"
