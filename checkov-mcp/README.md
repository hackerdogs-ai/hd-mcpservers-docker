...
MCP server wrapper for [Checkov](https://github.com/bridgecrewio/checkov) — Infrastructure as code security scanning.

## What is Checkov?

Checkov (checkov) is a security tool that provides: **Infrastructure as code security scanning.**

See [bridgecrewio/checkov](https://github.com/bridgecrewio/checkov) for full documentation.

**No API keys required** — Checkov runs locally inside the Docker container.

**Summary.** MCP server wrapper for [Checkov](https://github.com/bridgecrewio/checkov) — Infrastructure as code security scanning.

**Tools:**
- `run_checkov` — Run checkov with the given arguments. Returns structured JSON output.
...
Run checkov with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "checkov output will appear here"
}
```
...
- "Run checkov with --help to see all available options."
- "Use checkov to scan the target 192.168.1.1."
- "What options does checkov support? Show me its help output."
- "Run checkov against example.com with default settings."
- "Execute checkov with verbose output enabled."
- "Use the checkov tool to analyze the target and report findings."
...
```bash
docker run -i --rm hackerdogs/checkov-mcp:latest
```

### Docker Run (HTTP streamable mode)

```bash
docker run -d -p 8272:8272 \
  -e MCP_TRANSPORT=streamable-http \
  -e MCP_PORT=8272 \
  hackerdogs/checkov-mcp:latest
```
...
```json
{
  "mcpServers": {
    "checkov-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/checkov-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "checkov-mcp": {
      "url": "http://localhost:8485/checkov-mcp/mcp",
      "headers": {
...
| `MCP_PORT` | `8272` | HTTP port (only used with `streamable-http`) |
...
```bash
docker build -t hackerdogs/checkov-mcp:latest .
```
...
```bash
docker run -d --rm --name checkov-mcp-test -p 8272:8272 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/checkov-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8272/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"test","version":"0.1"}}}' \
  2>&1 | grep -i mcp-session-id | awk '{print $2}' | tr -d '\r\n')

curl -s -X POST http://localhost:8272/mcp \
  -H "Content-Type: application/json" \
  -H "Accept: application/json, text/event-stream" \
...
curl -s -X POST http://localhost:8272/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_checkov","arguments":{"arguments":"--help"}}}'
```
