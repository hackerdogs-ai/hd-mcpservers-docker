#!/bin/bash
# Test script for Checkov MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/checkov-mcp:latest"
PORT=8272
BINARY="checkov"
CONTAINER_NAME="checkov-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Checkov MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_checkov)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_checkov","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_checkov returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_checkov returned content"
else
    fail "tools/call run_checkov did not return expected response"
