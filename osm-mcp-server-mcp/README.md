...
MCP server wrapper for [OpenStreetMap](https://github.com/nicholasgasior/osm-mcp-server) — upstream package `osm-mcp-server`.

## What is OpenStreetMap?

MCP server for [OpenStreetMap](https://www.openstreetmap.org/). Query geographic data, find points of interest, perform spatial searches, and access mapping data from the open-source map project.
...
- "Find coffee shops near Times Square on OpenStreetMap."
- "Search for parks in Berlin."
...
```json
{
  "mcpServers": {
    "osm-mcp-server-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "hackerdogs/osm-mcp-server-mcp:latest"],
      "env": {}
    }
  }
}
...
```bash
docker run -d -p 8656:8656 -e MCP_TRANSPORT=streamable-http hackerdogs/osm-mcp-server-mcp:latest
```
