...
PASS=0; FAIL=0
IMAGE="hackerdogs/abuseipdb-mcp:latest"
PORT=8374
CONTAINER_NAME="abuseipdb-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"check_ip","arguments":{"ip_address":"127.0.0.1"}}}'

echo "========== abuseipdb-mcp test (compliance) =========="
...
echo "$CALL_OUT" | grep -q 'result\|content\|error' && pass "stdio tools/call check_ip" || fail "stdio tools/call check_ip"
...
info "[5] HTTP streamable tools/call check_ip"
...
echo "$CALL_HTTP" | grep -q 'result\|content\|error' && pass "HTTP tools/call check_ip" || fail "HTTP tools/call check_ip"
