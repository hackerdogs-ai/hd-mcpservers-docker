#!/bin/bash
# Test script for Fping MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/fping-mcp:latest"
PORT=8309
BINARY="fping"
CONTAINER_NAME="fping-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Fping MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_fping)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_fping","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_fping returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_fping returned content"
else
    fail "tools/call run_fping did not return expected response"
