...
PASS=0; FAIL=0
IMAGE="hackerdogs/secops-mcp:latest"
PORT=8379
CONTAINER_NAME="secops-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"list_tools","arguments":{}}}'
...
info "[3] Stdio tools/call list_tools"
CALL_OUT=$( ( printf '%s\n%s\n%s\n' "$INIT_REQ" "$INIT_NOTIF" "$CALL_REQ"; sleep 4 ) | docker run -i --rm -e MCP_TRANSPORT=stdio "$IMAGE" 2>/dev/null) || true
echo "$CALL_OUT" | grep -q 'allowed_tools' && pass "stdio tools/call list_tools" || fail "stdio tools/call list_tools"
...
info "[5] HTTP streamable tools/call list_tools"
...
echo "$CALL_HTTP" | grep -q 'allowed_tools' && pass "HTTP tools/call list_tools" || fail "HTTP tools/call list_tools"
