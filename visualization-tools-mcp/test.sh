...
PASS=0; FAIL=0
IMAGE="visualization-tools-mcp"
PORT=8506
CONTAINER_NAME="visualization-tools-mcp-test"
...
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"create_bar_chart","arguments":{"data_json":"{\"labels\":[\"A\",\"B\"],\"values\":[1,2]}"}}}'

echo "========== visualization-tools-mcp test (compliance) =========="
...
while [ $WAITED -lt 30 ]; do
  ...
  curl -s -X POST "http://localhost:${PORT}/mcp" \
    -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" \
    $SESS_HDR -d "$INIT_NOTIF" >/dev/null 2>&1 || true
  TOOLS_RESP=$(curl -s -X POST "http://localhost:${PORT}/mcp" \
    -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" \
    $SESS_HDR -d "$LIST_REQ" 2>/dev/null) || true
...
CALL_HTTP=$(curl -s -X POST "http://localhost:${PORT}/mcp" \
  -H "Content-Type: application/json" -H "Accept: application/json, text/event-stream" \
  $SESS_HDR -d "$CALL_REQ" 2>/dev/null) || true
if echo "$CALL_HTTP" | grep -q 'result\|content\|error'; then
  pass "HTTP tools/call"
else
  fail "HTTP tools/call"
fi
