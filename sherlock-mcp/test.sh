#!/bin/bash
# Test script for Sherlock MCP Server
...
PASS=0
FAIL=0
IMAGE="hackerdogs/sherlock-mcp:latest"
PORT=8317
BINARY="sherlock"
CONTAINER_NAME="sherlock-mcp-test"
...
echo "================================================================================="
echo -e "${BLUE}Sherlock MCP Server — Test Suite${NC}"
echo "================================================================================="
echo ""
...
info "[Test 6] MCP HTTP — tools/call (run_sherlock)"
CALL_REQ='{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"run_sherlock","arguments":{"arguments":"--help"}}}'
...
if echo "$CALL_RESP" | grep -q '"result"'; then
    pass "tools/call run_sherlock returned a result"
elif echo "$CALL_RESP" | grep -q '"content"'; then
    pass "tools/call run_sherlock returned content"
else
    fail "tools/call run_sherlock did not return expected response"
