...
MCP server wrapper for [Sherlock](https://github.com/sherlock-project/sherlock) — Username hunting across social networks and websites.

## What is Sherlock?

Sherlock (sherlock) is a security tool that provides: **Username hunting across social networks and websites.**

See [sherlock-project/sherlock](https://github.com/sherlock-project/sherlock) for full documentation.
...
Run sherlock with the given arguments. Returns structured JSON output.
...
```json
{
  "raw": "sherlock output will appear here"
}
```
...
```bash
docker run -i --rm hackerdogs/sherlock-mcp:latest
```
...
docker run -d -p 8317:8317 \
  ...
  -e MCP_PORT=8317 \
  hackerdogs/sherlock-mcp:latest
```
...
```json
{
  "mcpServers": {
    "sherlock-mcp": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "MCP_TRANSPORT", "hackerdogs/sherlock-mcp:latest"],
      "env": {
        "MCP_TRANSPORT": "stdio"
      }
    }
...
```json
{
  "mcpServers": {
    "sherlock-mcp": {
      "url": "http://localhost:8485/sherlock-mcp/mcp",
      "headers": {
...
```bash
docker build -t hackerdogs/sherlock-mcp:latest .
```
...
```bash
docker run -d --rm --name sherlock-mcp-test -p 8317:8317 \
  -e MCP_TRANSPORT=streamable-http \
  hackerdogs/sherlock-mcp:latest
```
...
```bash
SESSION_ID=$(curl -s -D - -X POST http://localhost:8317/mcp \
...
curl -s -X POST http://localhost:8317/mcp \
  ...
  -H "Accept: application/json, text/event-stream" \
  -H "mcp-session-id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"run_sherlock","arguments":{"arguments":"--help"}}}'
```
